...
- Milestone 5 当前 Schema/引擎为 `1.28.0` / `1.28.0-crystallize-shards`：除具名目标、逐目标 Aura/ICD/冻结抗性、三层策略、阶段窗口和显式 fanout 外，命中可用二维圆形、可旋转矩形、有限线段胶囊或填充扇形范围自动求交全部圆形碰撞体目标；几何可使用世界坐标，或通过场景静态 `actorPoses` 将施放者局部坐标旋转/平移到世界。目标可沿 60 FPS 声明式线性分段移动，核心按命中帧插值并把坐标空间、施放者姿态、解析后形状、最近距离和阈值写入逐击日志。扩散传播复用同一逐目标二维求交与阶段策略；结晶碎片在生成帧读取目标位置。跨目标命中确认产球仍聚合为一次。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
