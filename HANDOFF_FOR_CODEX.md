...
### 2026-07-29 权威 Current State

以下状态优先于后面的历史里程碑描述：

- 当前输入契约为 `schemaVersion: 1.40.0`、`engineVersion: 1.40.0-ec-next-target-tick-cleanup`；反应核心仍须由 `reactionEngine.mode` 显式选择，目标任务边界由 `targetTaskModel.mode` 显式选择，反应伤害交付顺序另由 `reactionDeliveryModel.mode` 显式选择。1.35→1.36 的 identity-only 迁移、1.36→1.37 的 legacy 注入和 1.37→1.38 的目标任务模式保留继续作为历史契约；1.38→1.39 会补入 `reactionDeliveryModel: { mode: "deferred-event-heap-v1" }`，不会自动启用递归碎冰；1.39→1.40 只升级身份，原样保留 v7、目标任务、目标时钟、玩家模型及 deferred/recursive 选择，不会自动启用 `aura-v8`。历史版本夹带未来字段会 fail-closed。所有迁移继续保留 `reactionEngine`、`playerDamageModel`、`targetClockModel`、`targetTaskModel`、逐元素敌方抗性和其余配置，不会静默切换 Aura 模式；玩家反应自伤、敌方目标本地时钟、目标任务相位、递归伤害交付和 v8 cleanup 继续分别 opt-in。
- `aura-v5` 已把正/反融化、正/反蒸发、超载、超导、感电、冻结/碎冰、火/水/冰/雷扩散、火/水/冰/雷结晶、原激化/超激化/蔓激化、燃烧，以及绽放/草原核/烈绽放/超绽放接入无 DOM 的核心。`aura-v6` 沿用这些主体语义，并对雷元素来袭按固定 gcsim 提交 `b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541` 以共享来袭 Gauge 依次检查 `超激化 → 超载 → 感电 → 冻结底超导 → 普通超导 → 原激化 → 绽放`。`aura-v7` 继承 v6，但把 Quicken 后的水草绽放跟进改为 core zero-delay task：任务继承触发事件的 frame/priority、取得更晚的全局 sequence，因而在已入队的同帧命中后按 FIFO 执行，并在执行点重读实时 Hydro/Quicken Aura。缺失任一状态时任务明确跳过，不使用触发时的陈旧快照伪造 Bloom；它不属于敌方 per-target queue，冻结的 1.37 v1 和 1.38 v2 都不会把它重新分类为 target-owned task。1.40 的 `aura-v8` 继承 v7，并在 follow-up 恰好耗尽同代最后 Hydro 时，把 EC cleanup 延迟到下一次有效目标 Tick：无 Hitlag 时 F1 停止，5 帧 Hitlag 时 F6/TF1 停止，已经排队的 F10 首次伤害保留，后续 F16 Wane/F70 Tick 被抑制；同代恢复、新代次替换、自然到期碰撞和模拟末端 pending 都有显式结果。精确 v8 只允许 `legal-frame-v1 + 60 FPS + target-phase-v2`。这些经典反应路径仍全部是 `fixed-gcsim-provisional`，不能视为官服真值或完整 gcsim 精度；v1–v7 的历史语义和 Golden 保持不变。
...
- 每次模拟返回 `runManifest`，固定配置哈希、Schema/引擎/数据版本、解析后的 crit/energy/兼容模式、随机种子和有序插件身份；复现键使用 `gdl-v2-fnv1a32-*`。声明式插件效果由核心生成内容哈希；任意代码插件的 descriptor/contentHash 仍是插件作者提供的受信声明，FNV-1a 也不是密码学签名。
...
- Zod 已严格覆盖输入、`SimulationRunManifest`、Burning/Quicken/Bloom 审计、核心 `reactionTaskLog`、冻结 v1 的 `targetTaskPhaseLog`、v2 的 `targetPhaseLog`、1.39 反应交付父链/碎冰日志引用、1.40 `electroChargedCleanup` 的任务/相位/周期/目标时间线 reciprocal 引用、ReactionA/B、草原核生命周期/接触/时间线和跨日志引用、玩家命中/自伤/HP/结晶盾吸收引用、目标时钟/Hitlag 日志及回放守恒、`TargetStateTimeline` 等关键输出。v7 的结果边界会把核心任务与触发命中、Quicken mutation、Bloom 审计、草原核预约和目标状态时间线做双向引用校验；v8 另核对 cleanup 的目标帧 deadline、代次、stop/retained/superseded/natural-expiry/pending 结果和不可伪造的 depletion 候选。两种目标相位日志分别核对自身版本身份、目标注册顺序、目标本地时钟、Aura 连续性及关联日志，并保持互斥。递归碎冰结果边界另拒绝错误身份、非碎冰前向引用、跨帧/跨目标/跨来源父子链、重复/缺失日志和循环引用。1.35 引入的逐元素抗性结果校验继续保留。完整 `SimulationResult` 仍没有覆盖所有字段的单一顶层运行时 Zod Schema。
- `ampBase` 输入必须是有限正数。1.35 及更早兼容配置中的 `reaction: "none" + ampBase` 继续按冻结的显式旧版倍率覆盖回放，但不会投影成正式反应；Aura 模式只允许显式 debug 且非 `none` 的 `reactionOverride`。公式层拒绝非正、非有限底数，并对转化/加算公式的非法运行时输入 fail-closed。
...
- 性能门保留 3 项 120 秒回归：默认兼容预设、带能量前缀探测的合法时间线、持续 Burning 刷新流；每项预热后运行 20 次，并要求最大值 `<100ms`。阈值是当前桌面回归门，不是跨设备 SLA。
- UI 继续只消费核心的敌方逐击、构成、时间线和曲线结构；本轮先保证反应核心，没有新增 EC cleanup、玩家自伤/HP/盾破裂或草原核专用面板。现有全队、个人、技能、时间轴、逐段伤害和曲线保持不变；展示扩展延后，后续 UI 只能读取新增核心结果，不能自行补算伤害。
- 明确未完成：除列明的 v6–v8 代表性来袭链之外的其他来袭元素全组合、一般化 Aura overlap、所有特殊/多 Aura 可达排列、附近潮湿目标 EC 连锁、跨过 F70 callback 的长 Hitlag Golden、Lunar 反应、完整角色特有 ICD、真实粒子/掉球与飞行、全角色动作/取消帧、从角色/武器/攻击类型自动推导 Hitlag、玩家自身 Hitlag/动作冻结、通用敌方任务暂停与任务所有权、Burning callback 内同步跨目标 Aura/反应命中、固定 gcsim 的 `DoNotLog` “Freeze Broken” 合成攻击、玩家 Aura/敌袭/治疗/死亡/复活、完整三维命中，以及全角色/武器/圣遗物/敌人的可执行机制数据库。冻结的 `target-phase-v1` 只覆盖第一批 Burning callback→Aura 衰减，1.38 `target-phase-v2` 也只覆盖 callback→该目标 `Reactable.Tick`；v8 cleanup 也不是完整 gcsim target phase。可查询角色/武器/技能目录只表示 `provisional + metadata-only` 覆盖，绝不等于完整机制覆盖；UID 展示柜映射仍有限，尚不能把任意账号可靠编译为 `SimConfig`，“毕业站位”也仍是不可模拟占位。默认杜林预设的示例魔法数继续是 `provisional`。本项目仍未达到 gcsim 精度。
- 玩家八项抗性是用户显式输入和本项目的公式约定；固定 gcsim 提交用于列明路径的交叉参考，但没有提供可直接当作官服正式角色抗性数据库的真值。测试向量、草原核寿命和杜林示例数值都不得包装为正式官服数据或完整 gcsim 精度。
- Burning 的 `legacy-event-heap-v1` 有意保留 1.30 兼容相位：Fuel 于第 121 个目标帧清理，且同一目标帧普通命中先于 Tick；v7 的 refresh 计数修正不会反向改写它。1.37 的 `target-phase-v1` 冻结第一批 Burning callback→Aura 衰减；1.38 的 `target-phase-v2` 只保证同一目标 callback 先于该目标 `Reactable.Tick`，启用目标时钟时陈旧唤醒会先重投影。实际 Burning 范围伤害仍在后续全局 core `reactionDamage` 处理，callback 内同步跨目标 Aura/反应命中尚未实现。所有这些顺序仅为 `fixed-gcsim-provisional`，不是官服真值、完整 gcsim target phase 或完整 gcsim 精度。
...
- 反应核心优先阶段已推进到 1.40：在既有 Bloom、草原核、烈绽放、超绽放、玩家反应自伤、目标本地 Hitlag、`aura-v6` 雷来袭链、逐元素抗性和 1.36 opt-in `aura-v7` 的 Quicken→Bloom core zero-delay task 之上，1.37 新增并冻结独立 `targetTaskModel`、`target-phase-v1`、第一批 Burning callback/Aura 衰减和 `targetTaskPhaseLog`；1.38 再增加显式 opt-in `target-phase-v2` 与 `targetPhaseLog`，只建模 callback→该目标 `Reactable.Tick`；1.39 新增默认 deferred、可显式递归的碎冰交付边界，并为 v2 Miss 保留 `TARGET_MISS` Hitlag 审计；1.40 新增 `aura-v8`，完成 Quicken→Bloom 后 EC next-target-Tick cleanup 的 F1/Hitlag5→F6、F10 保留、后续 cadence 抑制和跨日志审计。下一顺序应补附近湿目标 EC 连锁、长 Hitlag、其他来袭元素与多 Aura 排列，再实现 Burning callback 内同步跨目标 Aura/反应命中和通用目标任务所有权；随后再考虑玩家 Aura、敌袭、治疗/死亡等生存机制，`OnBurning` 角色回调和纳西妲 C2 仍后置。
...
- 当前 Schema/引擎为 `1.40.0` / `1.40.0-ec-next-target-tick-cleanup`：除具名目标、逐目标 Aura/ICD/冻结抗性、共享/逐目标八项基础抗性、三层策略、阶段窗口和显式 fanout 外，命中可用二维圆形、可旋转矩形、有限线段胶囊或填充扇形范围自动求交全部圆形碰撞体目标；几何可使用世界坐标，或通过场景静态 `actorPoses` 将施放者局部坐标旋转/平移到世界。目标可沿 60 FPS 声明式线性分段移动，核心按命中帧插值并把坐标空间、施放者姿态、解析后形状、最近距离和阈值写入逐击日志，但目标移动/阶段窗口仍是全局帧，不随 Hitlag 暂停。扩散传播、燃烧 Tick 和草原核爆炸复用逐目标二维求交和阶段策略；结晶碎片与草原核在各自生成帧读取目标位置。玩家反应自伤只使用配置中的静态玩家位置/圆形碰撞体。跨目标命中确认产球仍聚合为一次。冻结的 `target-phase-v1` 只改变第一批 callback/Aura 衰减边界；`target-phase-v2` 只声明 callback→该目标 `Reactable.Tick`，精确 v8 则在这一 Tick 上增加 EC cleanup 决议。兼容预设和迁移输入继续保留既有 Aura/目标任务/反应交付模式。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
...
- 当前事件优先级：
  ...
  4. `hit`
  5. `reactionDamage`
- 冻结的 1.37 `target-phase-v1` 在 `action` 后、同帧 buff/energy/hit/core reaction damage 前注册目标任务优先级，并按目标注册顺序处理第一批 Burning target callback 与该目标 Aura 衰减。1.38 `target-phase-v2` 只把每个目标 callback→同一目标 `Reactable.Tick` 固定在该边界，不建立全目标 barrier，也不把感电 `+10/+60` 伤害 Tick、`+6` Wane、ICD、ReactionA/B 或其他 core work 搬入目标相位。`legacy-event-heap-v1` 保持既有优先级和 Golden；`aura-v7/v8` 的 `quickenBloomFollowup` 仍是继承触发 frame/priority、取得更晚全局 sequence 的 core zero-delay task，在 v1/v2 中都不属于 target-owned queue，v8 只把 depletion cleanup 决议放到下一目标 Tick。继续扩展通用目标任务阶段时仍须另升引擎版本，不能静默重排 v1–v8 或历史目标任务模式。
...
技能基线保存在 `packages/test-vectors/fixtures/legacy-default-120s.golden.json`。1.30–1.36 的历史升级继续按原契约保留；1.36 新增显式 opt-in `aura-v7` 和核心反应任务日志，1.37 再新增独立 `targetTaskModel` 与 v1 目标相位日志。1.35→1.36 是 identity-only 迁移，必须保留 v6、玩家模型、目标时钟和逐元素抗性，绝不能自动启用 v7；1.36→1.37 必须为历史输入补入 `legacy-event-heap-v1`，绝不能自动启用 target phase；1.37→1.38 必须原样保留 `legacy-event-heap-v1` 或 `target-phase-v1`，只有显式 1.38 输入才能选择 `target-phase-v2`；1.38→1.39 必须补入 `deferred-event-heap-v1`，绝不能自动启用递归碎冰；1.39→1.40 必须保留 v7、目标任务、目标时钟与伤害交付选择，绝不能自动启用 v8 cleanup。1.37–1.39 的十个冻结 Fixture 与 1.40 的三个冻结 Fixture 及 SHA-256 以本文件顶部权威清单为准；任何后续工作都不得覆盖它们。1.40 分别冻结不变的兼容默认、identity-only Quicken→Bloom 和 v8 EC cleanup 的 F1/Hitlag5→F6，默认数值仍与 Vanilla 浮点基线一致。`legacy-default-120s-1.36.golden.json`、`quicken-bloom-task-order-1.36.golden.json` 和 `reaction-matrix-1.35.golden.json` 继续保留为更早历史证据。Golden 相等只证明迁移兼容和对应结构化结果没有漂移，不证明测试抗性、杜林预设中的手工反应、装备系数、固定 gcsim 提交或其他示例魔法数是官方真值；这些边界继续标记 `provisional` 或 `fixed-gcsim-provisional`。

1.36 继续保留独立的 `QUICKEN_BLOOM_TASK_*` 历史版本常量，专用 Golden 不再依赖未来会变化的 `CURRENT_*` 名称。基础反应发布门由 `aura-v7-order-release.test.ts` 和 `aura-v7-public-grid.test.ts` 负责代表性多 Aura 顺序、Frozen 火蒸发 guard、ICD/到期边界、有限性、非负性、来源槽/消费守恒、重放确定性和输入数组换序。冻结的 1.37 v1 门由 `target-task-phase.test.ts` 与 `target-task-phase-log.test.ts` 锁定 callback-before-decay、Hitlag 唤醒重投影、多目标注册顺序和 `targetTaskPhaseLog`；1.38 v2 门另由 `aura-reactable-boundary.test.ts`、`target-phase-v2-reaction-gate.test.ts` 与 `target-reactable-phase-v2.test.ts` 锁定 17 类代表性反应、callback→同一目标 `Reactable.Tick` 和 `targetPhaseLog`；1.40 v8 门由 `aura-v8-ec-cleanup.test.ts`、`electro-charged-quicken-cleanup.test.ts` 和 `electro-charged-cleanup-golden.test.ts` 锁定 cleanup 的结果分支、Hitlag 重投影、F10 保留和跨日志引用。这些门不得外推为附近湿目标 EC 连锁、Burning callback 内同步跨目标 Aura/反应命中、全目标 barrier、完整 target task system 或完整 gcsim parity；未实际执行这些命令前也不得写入通过结论或测试计数。

当前验证命令：

```bash
npm run typecheck
npx vitest run packages/schemas/src/schema.test.ts packages/sim-core/src/__tests__/formulas.test.ts packages/sim-core/src/__tests__/amplifying.test.ts packages/sim-core/src/__tests__/reaction-a.test.ts packages/sim-core/src/__tests__/reaction-b.test.ts packages/sim-core/src/__tests__/bloom-gauge.test.ts packages/sim-core/src/__tests__/bloom-aura.test.ts packages/sim-core/src/__tests__/bloom-integration.test.ts packages/sim-core/src/__tests__/dendro-core.test.ts packages/sim-core/src/__tests__/aura-v6-electro.test.ts packages/sim-core/src/__tests__/aura-v6-simulator.test.ts packages/sim-core/src/__tests__/hydro-order.test.ts packages/sim-core/src/__tests__/quicken-bloom-task-order.test.ts packages/sim-core/src/__tests__/aura-v8-ec-cleanup.test.ts packages/sim-core/src/__tests__/electro-charged-quicken-cleanup.test.ts packages/sim-core/src/__tests__/electro-charged-cleanup-golden.test.ts packages/sim-core/src/__tests__/burning.test.ts packages/sim-core/src/__tests__/burning-v7-refresh.test.ts packages/sim-core/src/__tests__/target-task-phase.test.ts packages/sim-core/src/__tests__/target-task-phase-log.test.ts packages/sim-core/src/__tests__/aura-reactable-boundary.test.ts packages/sim-core/src/__tests__/target-phase-v2-reaction-gate.test.ts packages/sim-core/src/__tests__/target-reactable-phase-v2.test.ts packages/sim-core/src/__tests__/shatter-recursive-delivery.test.ts packages/sim-core/src/__tests__/enemy-elemental-resistance.test.ts packages/sim-core/src/__tests__/crystallize.test.ts packages/sim-core/src/__tests__/player-damage.test.ts packages/sim-core/src/__tests__/player-reaction-damage.test.ts packages/sim-core/src/__tests__/target-clock.test.ts packages/sim-core/src/__tests__/aura-target-clock.test.ts packages/sim-core/src/__tests__/target-clock-integration.test.ts packages/sim-core/src/__tests__/target-hitlag-status.test.ts packages/sim-core/src/__tests__/reaction-matrix-golden.test.ts packages/sim-core/src/__tests__/golden.test.ts packages/sim-core/src/__tests__/performance.test.ts
...
3. 使用 Zod 校验输入JSON并提供字段路径错误。
   当前还为 `SimulationRunManifest`、Burning/Quicken/Bloom 审计、核心 `reactionTaskLog` 及其到命中/Quicken/草原核/目标时间线的双向引用、冻结 v1 的 `targetTaskPhaseLog`、v2 的 `targetPhaseLog` 及各自目标顺序/目标时钟/Aura 链/关联日志、v8 EC cleanup 的任务/相位/周期/目标时间线 reciprocal 引用、ReactionA/B 伤害组、草原核生命周期/接触/时间线、玩家伤害/HP/结晶盾引用、目标时钟/Hitlag 日志及回放守恒、`TargetStateTimeline` 等关键输出提供严格 Zod 校验，并以模拟器真实日志做回归。两种目标相位日志按模式互斥；Aura 结果在来源槽存在时还强制 actor 唯一、聚合 Gauge 等于最大槽、逐条 mutation 守恒；无来源槽的历史投影继续兼容。完整 `SimulationResult` 的顶层运行时 Zod Schema 尚未完成，不得把这些关键契约外推为“全部结果均已运行时校验”。
4. 加入 Schema 迁移系统，例如：
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.40 权威 Current State 为起点：先保持全部历史 Golden、三份 1.40 Golden、两种目标相位日志、`runManifest`、核心 `reactionTaskLog`、1.39 反应交付引用和 1.40 EC cleanup reciprocal 引用的一致性与性能门；随后补附近湿目标 EC 连锁、长 Hitlag、其他来袭元素的有序反应、完整特殊/多 Aura 排列、Burning callback 内同步跨目标 Aura/反应命中和通用目标任务所有权。感电 Tick/Wane 与 Quicken→Bloom 等 core work 在另立版本契约前继续保持全局。基础机制稳定后才处理玩家 Aura/敌袭生存模型、角色机制映射、UID 配置编译和专用 UI。
