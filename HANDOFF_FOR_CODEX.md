...
- Milestone 5 已建立首个逐技能部分机制闸门：Schema/引擎升级到 `1.5.0` / `1.5.0-followup-cancels`，支持声明式角色特有 ICD Profile、行动状态的授予/前置/消耗/刷新/精确到期，以及按紧随其后的普攻/重击/战技/爆发/切人选择取消帧；`AbilityBlueprint` 1.2 与通用编译器会从固定目录解析倍率、保留证据/前置条件/未实现项，并默认拒绝 `partial`。
