...
- 当前输入契约为 `schemaVersion: 1.51.0`、`engineVersion: 1.51.0-basic-reaction-scheduler`，当前 `runManifest.version` 为 `1.7.0`；反应核心、目标任务、交付顺序、感电传播、经典反应公式、普通直伤组、数字元素施加、Burning/Swirl 反应所有施加策略、ReactionA/B 伤害组策略和基础反应调度策略分别由显式版本字段固定。V1.50 的 `reactionDamageGroupModel`、Manifest 1.6.0、wire 与 Fixture 完整冻结。所有 `1.50.0` 及更早输入迁移到 1.51 时为新增 `basicReactionSchedulerModel` 选择 V1，不静默升级到 V2；只有原生 current 默认预设显式选择 V2。基础反应调度 V1 root 为 `legacy-partial-basic-reaction-scheduler-immediate-attachment-v1` / `sha256:2acb593bd32d61e908558d3d8fe8ffa0c36facff0c9f414c411af6cdf0c00ec8`，V2 root 为 `gcsim-b4ae769-basic-reaction-scheduler-provenance-provisional-v2` / `sha256:59b8f34401ceaf16ea65482c80e7f481629fceb5857900ed594c950db6954534`。迁移不猜测 fixed group、不自动绑定未核验技能、不改变默认伤害，也不启用策略外的派生施加。精确 V150/V149/V148/V147/V146/V145/V144/V142 wire、Manifest 和 Fixture 保持冻结；精确 1.44–1.51 均可在既有版本门下显式选择 v3。历史版本夹带未来 model/selector 会 fail-closed。
- `aura-v5` 已把正/反融化、正/反蒸发、超载、超导、感电、冻结/碎冰、火/水/冰/雷扩散、火/水/冰/雷结晶、原激化/超激化/蔓激化、燃烧，以及绽放/草原核/烈绽放/超绽放接入无 DOM 的核心。`aura-v6`–`aura-v9` 继续冻结共享 Gauge 有序链、Quicken→Bloom core task、EC cleanup 与全局 cadence/Hitlag/Wane 所有权。精确 1.42 历史身份下 v9 只允许 `legal-frame-v1 + 60 FPS + target-phase-v2`；精确 1.44–1.51 可在同样的帧率/时间线门下与 v2 或显式 v3 组合。这些经典反应路径仍全部是 `fixed-gcsim-provisional`，不能视为官服真值或完整 gcsim 精度；v1–v8 的历史语义和 Golden 保持不变。
...
- 当前 UI 不扩展；既有全队、个人、技能、时间轴、逐段伤害、构成和曲线继续只读核心返回值。尚未完整实现的边界包括 `DoNotLog` Freeze Broken 合成攻击、Lunar 反应族、特殊/多 Aura 可达空间、全技能 ICD 绑定、真实粒子与产球数据、完整动作帧/快照、全角色/武器可执行机制库，以及可靠的 `ShowcaseSnapshot -> ResolvedLoadout -> SimConfig` UID 管线；“毕业站位”仍是不可执行占位，冻结杜林预设的示例魔法数仍是 provisional。
...
- 当前 V151 完整性门继承冻结 V150 的全部证明，再按配置重放基础反应调度 V1/V2 root、attack/commit 配对、task sequence、`basicReactionSchedulerLog`、`reactionAuraAttachment` 和与 DamageEvent/HitResolution/TargetStateTimeline 的反链。V150 及更早历史证明保持原样；V151 新增层不等于从完整 scheduler trace、Ability/Aura、动作快照或角色数据库重放全链，也不证明其他反应应该施加元素。
...
- 每次 current 模拟返回 Manifest 1.7.0，固定配置哈希、Schema/引擎/数据版本、解析后的运行选项、随机种子、有序插件身份、完整 `reactionFormulaRoot`、`directDamageGroupRoot`、`elementalApplicationIcdRoot`、已选 `reactionOwnedElementalApplicationRoot`、已选 `reactionDamageGroupRoot` 与已选 `basicReactionSchedulerRoot`；复现键使用 `gdl-v2-fnv1a32-*`。冻结 V150 Manifest 仍是 1.6.0，V149 仍是 1.5.0，V148 仍是 1.4.0。当前 32-bit FNV-1a 已知可碰撞，只是 drift detector，不能作为密码学身份或签名。
...
- Zod 已严格覆盖当前 V151 输入与结果；`simulationResultV151Schema` 和 `assertTrustedSimulationResultV151()` 在冻结 V150 全部证明上增加第六根、attack/commit scheduler log、`reactionAuraAttachment` 与目标时间线重放。`simulationResultV150Schema` 和 `assertTrustedSimulationResultV150()` 以及所有更早精确 Schema/断言仍保持冻结身份。外部或持久化完整结果必须走完整 Zod；trusted assertion 不是外部 wire 验证边界。
...
- 当前 Schema/引擎为 `1.51.0` / `1.51.0-basic-reaction-scheduler`，Manifest 为 1.7.0。既有具名目标、Aura/反应、几何、时钟、EC cleanup/传播、普通直伤组、配置直接 application、Burning/Swirl 反应所有施加与冻结 V150 ReactionA/B 边界保持不变；V151 只新增独立基础反应调度 V1/V2 root、同帧 Swirl attack/commit 顺序和结果完整性证明。原生 current 默认预设使用 V2，所有历史迁移固定使用 V1；两者继续保留既有 Aura/目标任务/反应交付模式。精确 1.44–1.51 输入均可在版本门下 opt-in `target-phase-v3`。`AbilityBlueprint` 当前为 1.9，1.0–1.8 可迁移；编译器可显式传递两类 descriptor/application selector，但不会自动猜测 fixed group 或反应所有施加。
...
3. 使用 Zod 校验输入 JSON 并提供字段路径错误。当前精确 1.51 输出由 `simulationResultV151Schema` 覆盖全部已声明顶层字段和叶节点；冻结 `simulationResultV150Schema` 及更早精确 Schema 仍只接受各自历史身份。外部或持久化结果必须完整解析，核心内部当前结果运行 `assertTrustedSimulationResultV151()`。V145 公式根、V146 Damage Group root/log、V147 配置直接 application root/log、V148 反应所有策略 root/log、V149 的 V1/V2 reset-policy/decision 与 parent sequence 所有权、V150 ReactionA/B damage group，以及 V151 的 `basicReactionSchedulerModel`、Manifest root、attack/commit、`reactionAuraAttachment` 和 `basicReactionSchedulerLog` 都必须 strict 校验；trusted assertion 不能作为外部 wire 的验证边界。
4. 加入 Schema 迁移系统，例如：
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.51 权威 Current State 为起点：保持全部历史 Golden、Manifest 1.7 六 root、独立的 1.51 基础调度 Golden、冻结 V150 Manifest 1.6/ReactionA/B Golden 与全部更早 wire/Fixture、核心 `reactionTaskLog` / `reactionDamageGroupResetLog` / `basicReactionSchedulerLog`、反应交付/EC cleanup 引用、1.41/1.42 传播与 cadence 向量、24 标签/16 类经典反应发布门，以及 1.44–1.51 v3 callback delivery/attempt 完整性契约。下一步先继续补 Burning restart/cascade、双 Anemo、混合同帧 Swirl 与更完整的版本化 scheduler provenance，再逐技能绑定经过来源核验的两类 Tag/Group；任何新的反应所有施加都必须先核对固定来源并升版策略根。Freeze Broken 与 Lunar 反应另立版本实现。基础机制稳定后才处理玩家 Aura/敌袭生存模型、全角色机制映射、UID 配置编译和专用 UI。
