...
- Milestone 3 下一条反应切片应实现结晶：先冻结同一 gcsim 提交的岩元素消耗顺序、结晶元素与拾取/护盾语义，再定义掉落实体、位置、生成/拾取帧、同元素队列限制、护盾刷新与逐事件审计；不要只添加一个伤害为零的反应标签。
...
- Milestone 5 当前 Schema/引擎为 `1.27.0` / `1.27.0-swirl-propagation`：除具名目标、逐目标 Aura/ICD/冻结抗性、三层策略、阶段窗口和显式 fanout 外，命中可用二维圆形、可旋转矩形、有限线段胶囊或填充扇形范围自动求交全部圆形碰撞体目标；几何可使用世界坐标，或通过场景静态 `actorPoses` 将施放者局部坐标旋转/平移到世界。目标可沿 60 FPS 声明式线性分段移动，核心按命中帧插值并把坐标空间、施放者姿态、解析后形状、最近距离和阈值写入逐击日志。扩散传播复用同一逐目标二维求交与阶段策略。跨目标命中确认产球仍聚合为一次。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
