...
- Milestone 3 的第二批第一条转化反应切片已完成：Schema/引擎升级到 `1.22.0` / `1.22.0-overload-reaction`，`aura-v2` 加入雷普通 Aura与双向超载。超载独立伤害按等级基准、精通、反应增伤与火抗计算，延迟 1 帧，使用同一触发目标 6 帧伤害 GCD，并以触发目标位置为圆心、半径 3 对注册目标逐一求交；排队、GCD、未解析坐标、逐目标伤害及触发反链均进入结构化结果和网页。击退/韧性、三维范围以及其余反应仍未实现。
...
- Milestone 5 当前 Schema/引擎为 `1.22.0` / `1.22.0-overload-reaction`：除具名目标、逐目标 Aura/ICD、三层策略、阶段窗口和显式 fanout 外，命中可用二维圆形、可旋转矩形、有限线段胶囊或填充扇形范围自动求交全部圆形碰撞体目标；几何可使用世界坐标，或通过场景静态 `actorPoses` 将施放者局部坐标旋转/平移到世界。目标可沿 60 FPS 声明式线性分段移动，核心按命中帧插值并把坐标空间、施放者姿态、解析后形状、最近距离和阈值写入逐击日志。跨目标命中确认产球仍聚合为一次。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
