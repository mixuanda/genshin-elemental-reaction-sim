...
- Milestone 3 第二批的冻结切片已完成：Schema/引擎升级到 `1.25.0` / `1.25.0-freeze-state`。`aura-v2` 可由水/冰双向生成独立冻元素耐久，并按固定 gcsim 的逐帧加速速率与目标 `freezeResistance` 精确衰减；冻结抗性为 1 时保留反应/元素消耗审计但不生成耐久。冻元素会阻止火打水反向蒸发、冰打雷普通超导和新感电等错误分支，可被火正向融化或雷冻结底超导消耗。生成、刷新、免疫、消耗和自然到期进入 `frozenStateLog`、逐击详情、冻结状态表与敌方 Aura 曲线。尚未实现敌人定身、钝击削冻、碎冰、冻结气泡破裂、Hitlag 或冻结抗性数据库。
...
- Milestone 5 当前 Schema/引擎为 `1.25.0` / `1.25.0-freeze-state`：除具名目标、逐目标 Aura/ICD/冻结抗性、三层策略、阶段窗口和显式 fanout 外，命中可用二维圆形、可旋转矩形、有限线段胶囊或填充扇形范围自动求交全部圆形碰撞体目标；几何可使用世界坐标，或通过场景静态 `actorPoses` 将施放者局部坐标旋转/平移到世界。目标可沿 60 FPS 声明式线性分段移动，核心按命中帧插值并把坐标空间、施放者姿态、解析后形状、最近距离和阈值写入逐击日志。跨目标命中确认产球仍聚合为一次。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
