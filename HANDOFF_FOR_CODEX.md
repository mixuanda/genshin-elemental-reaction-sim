...
- 当前输入契约为 `schemaVersion: 1.32.0`、`engineVersion: 1.32.0-player-reaction-damage`；反应核心仍由 `reactionEngine.mode: aura-v5` 显式选择，玩家反应自伤则由 `playerDamageModel.mode: reaction-self-v1` 单独 opt-in。迁移自 1.31 及更早版本的配置保留原反应模式并得到 `playerDamageModel: { mode: "disabled" }`，不会静默改变历史回放或敌方伤害。
...
- 当前 1.32 兼容 Golden 身份为：`legacy-v0.1` → `gdl-v2-fnv1a32-1c8d1772`，1.30 Burning → `gdl-v2-fnv1a32-0ed897b9`。`reaction-matrix-1.31.golden.json` 继续冻结历史 14 个 `legal-frame-v1 + aura-v5` 反应向量；`reaction-matrix-1.32.golden.json` 以相同集合冻结当前契约及四类玩家反应自伤投影，SHA-256 为 `4089587e77765a655ccf6f776a32e1bc7e259a10ba3b532fb273274d814962e4`。新 Fixture 不改写旧 Fixture 的敌方语义。
- Zod 已严格覆盖输入、`SimulationRunManifest`、Burning/Quicken/Bloom 审计、ReactionA/B、草原核生命周期/接触/时间线和跨日志引用、玩家命中/自伤/HP/结晶盾吸收引用、`TargetStateTimeline` 等关键输出；完整 `SimulationResult` 仍没有覆盖所有字段的单一顶层运行时 Zod Schema。
...
- 明确未完成：敌人 Hitlag/目标本地时钟、玩家 Aura 与被敌攻击触发的反应、敌方攻击、治疗/死亡/复活、动态 Max HP、玩家移动、非结晶盾/护盾强效、完整 Aura 来源重叠、全角色 ICD/粒子/动作帧/可执行角色与武器库、角色回调、完整三维命中和 Lunar 反应。可查询目录的 120 角色、125 套天赋、762 条技能/被动、237 把武器只表示目录覆盖，绝不等于完整机制覆盖；UID 展示柜尚不能可靠编译任意账号，默认杜林预设的示例魔法数继续是 `provisional`。
- 玩家八项抗性是用户显式输入和本项目的公式约定；固定 gcsim 提交用于列明路径的交叉参考，但没有提供可直接当作官服正式角色抗性数据库的真值。测试向量、草原核寿命和杜林示例数值都不得包装为正式官服数据或完整 gcsim 精度。
- Burning 在 `aura-v4/v5` 有意保留 1.30 兼容相位：Fuel 于 `F+121` 清理，且同帧普通命中先于 Tick；固定参考敌方任务路径为 `F+120` 最后 Tick、Fuel 清理、再处理普通命中。这项差异与无敌方 Hitlag 一样必须显式记录，不能在现有引擎版本中静默改写。
...
- 反应核心优先阶段已完成到 1.32：Bloom、草原核、烈绽放、超绽放、14 向量历史 Golden，以及四类玩家反应自伤/结晶盾/HP 基础切片已落地。下一顺序先做敌人 Hitlag 目标本地时钟和更多基础反应边界核验，再扩展玩家 Aura、敌袭、治疗/死亡等生存机制；`OnBurning` 回调和纳西妲 C2 仍后置。
...
- 当前 Schema/引擎为 `1.32.0` / `1.32.0-player-reaction-damage`：除具名目标、逐目标 Aura/ICD/冻结抗性、三层策略、阶段窗口和显式 fanout 外，命中可用二维圆形、可旋转矩形、有限线段胶囊或填充扇形范围自动求交全部圆形碰撞体目标；几何可使用世界坐标，或通过场景静态 `actorPoses` 将施放者局部坐标旋转/平移到世界。目标可沿 60 FPS 声明式线性分段移动，核心按命中帧插值并把坐标空间、施放者姿态、解析后形状、最近距离和阈值写入逐击日志。扩散传播、燃烧 Tick 和草原核爆炸复用逐目标二维求交和阶段策略；结晶碎片与草原核在各自生成帧读取目标位置。玩家反应自伤只使用配置中的静态玩家位置/圆形碰撞体。跨目标命中确认产球仍聚合为一次。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
...
技能基线保存在 `packages/test-vectors/fixtures/legacy-default-120s.golden.json`。1.30 迁移只增加 opt-in `aura-v4`，1.31 再增加 opt-in `aura-v5`，1.32 则增加显式 `playerDamageModel`；历史迁移和内置兼容预设都使用 `disabled`，不得改变上述敌方伤害结果。当前 1.32 `runManifest` 锁定的 legacy key 为 `gdl-v2-fnv1a32-1c8d1772`、Burning key 为 `gdl-v2-fnv1a32-0ed897b9`。`packages/test-vectors/fixtures/reaction-matrix-1.31.golden.json` 继续冻结 14 个历史反应向量；`reaction-matrix-1.32.golden.json` 以相同集合锁定当前 Schema/引擎、显式 10% 玩家抗性、四类玩家自伤、空间命中、独立 ReactionA、结晶盾/HP 守恒及结构化引用。Golden 相等只证明迁移兼容和当前结构化结果没有漂移，不证明测试抗性、杜林预设中的手工反应、装备系数或其他示例魔法数是正式游戏数据，它们必须继续标记 `provisional`。

当前验证命令：

```bash
npm run typecheck
npx vitest run packages/schemas/src/schema.test.ts packages/sim-core/src/__tests__/amplifying.test.ts packages/sim-core/src/__tests__/reaction-a.test.ts packages/sim-core/src/__tests__/reaction-b.test.ts packages/sim-core/src/__tests__/bloom-gauge.test.ts packages/sim-core/src/__tests__/bloom-aura.test.ts packages/sim-core/src/__tests__/bloom-integration.test.ts packages/sim-core/src/__tests__/dendro-core.test.ts packages/sim-core/src/__tests__/player-damage.test.ts packages/sim-core/src/__tests__/player-reaction-damage.test.ts packages/sim-core/src/__tests__/reaction-matrix-golden.test.ts packages/sim-core/src/__tests__/golden.test.ts packages/sim-core/src/__tests__/performance.test.ts
npm test
npm run check
...
3. 使用 Zod 校验输入JSON并提供字段路径错误。
   当前还为 `SimulationRunManifest`、Burning/Quicken/Bloom 审计、ReactionA/B 伤害组、草原核生命周期/接触/时间线及跨日志引用、`TargetStateTimeline` 等关键输出提供严格 Zod 校验，并以模拟器真实日志做回归；完整 `SimulationResult` 的顶层运行时 Zod Schema 尚未完成，不得把这些关键契约外推为“全部结果均已运行时校验”。
4. 加入 Schema 迁移系统，例如：
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.32 权威 Current State 为起点：先保持全部 Golden、`runManifest` 和性能门，再推进目标本地时钟/Hitlag、未覆盖基础反应与 Aura overlap、玩家 Aura/敌袭生存模型、角色机制映射与 UID 配置编译；玩家自伤和草原核的专用 UI 展示后置。
