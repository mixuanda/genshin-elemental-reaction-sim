...
- 当前输入契约为 `schemaVersion: 1.38.0`、`engineVersion: 1.38.0-target-reactable-phase`；反应核心仍须由 `reactionEngine.mode` 显式选择，目标任务边界另由 `targetTaskModel.mode` 显式选择。1.35→1.36 的 identity-only 迁移和 1.36→1.37 的 legacy 注入继续作为历史契约保留；1.37→1.38 精确保留 `targetTaskModel` 的 `legacy-event-heap-v1` 或 `target-phase-v1`，不会自动启用 `target-phase-v2`。只有 1.38 输入可显式选择 v2。所有迁移继续保留 `reactionEngine`、`playerDamageModel`、`targetClockModel`、逐元素敌方抗性和其余配置，不会静默切换 Aura 模式；玩家反应自伤、敌方目标本地时钟和目标任务相位继续分别 opt-in。
...
- 1.33–1.36 的兼容 Golden 身份与 SHA-256 继续作为历史证据保留；`legacy-default-120s-1.36.golden.json` 和 `quicken-bloom-task-order-1.36.golden.json` 均不被改写。`legacy-default-120s-1.37.golden.json`、`quicken-bloom-task-order-1.37.golden.json` 与 `target-task-phase-1.37.golden.json` 冻结完整 1.37 身份、v1 目标相位和原始浮点基线。1.38 另用 `legacy-default-120s-1.38.golden.json`、`quicken-bloom-task-order-1.38.golden.json` 与 `target-reactable-phase-1.38.golden.json` 锁定新身份和 v2 边界，不得覆盖旧 Fixture；`reaction-matrix-1.35.golden.json` 的 17 个向量继续作为反应矩阵语义 Golden 复用。版本字段、配置哈希和复现键只按各自身份合法更新。
...
- 明确未完成：除列明的 v6/v7 代表性来袭链之外的其他来袭元素全组合、一般化 Aura overlap、所有特殊/多 Aura 可达排列、Lunar 反应、完整角色特有 ICD、真实粒子/掉球与飞行、全角色动作/取消帧、从角色/武器/攻击类型自动推导 Hitlag、玩家自身 Hitlag/动作冻结、通用敌方任务暂停与任务所有权、Burning callback 内同步跨目标 Aura/反应命中、玩家 Aura/敌袭/治疗/死亡/复活、完整三维命中，以及全角色/武器/圣遗物/敌人的可执行机制数据库。冻结的 `target-phase-v1` 只覆盖第一批 Burning callback→Aura 衰减，1.38 `target-phase-v2` 也只覆盖 callback→该目标 `Reactable.Tick`；两者都不是完整 gcsim target phase。可查询目录只表示 provisional metadata 覆盖，绝不等于完整机制覆盖；UID 展示柜尚不能可靠编译任意账号，默认杜林预设的示例魔法数继续是 `provisional`。本项目仍未达到 gcsim 精度。
...
- 反应核心优先阶段已推进到 1.38：在既有 Bloom、草原核、烈绽放、超绽放、玩家反应自伤、目标本地 Hitlag、`aura-v6` 雷来袭链、逐元素抗性和 1.36 opt-in `aura-v7` 的 Quicken→Bloom core zero-delay task 之上，1.37 新增并冻结独立 `targetTaskModel`、`target-phase-v1`、第一批 Burning callback/Aura 衰减和 `targetTaskPhaseLog`；1.38 再增加显式 opt-in `target-phase-v2` 与 `targetPhaseLog`，只建模 callback→该目标 `Reactable.Tick`。下一顺序仍包括其他来袭元素、多 Aura 排列、Burning callback 内同步跨目标 Aura/反应命中和通用目标任务所有权；随后再考虑玩家 Aura、敌袭、治疗/死亡等生存机制，`OnBurning` 角色回调和纳西妲 C2 仍后置。
...
- 当前 Schema/引擎为 `1.38.0` / `1.38.0-target-reactable-phase`：除具名目标、逐目标 Aura/ICD/冻结抗性、共享/逐目标八项基础抗性、三层策略、阶段窗口和显式 fanout 外，命中可用二维圆形、可旋转矩形、有限线段胶囊或填充扇形范围自动求交全部圆形碰撞体目标；几何可使用世界坐标，或通过场景静态 `actorPoses` 将施放者局部坐标旋转/平移到世界。目标可沿 60 FPS 声明式线性分段移动，核心按命中帧插值并把坐标空间、施放者姿态、解析后形状、最近距离和阈值写入逐击日志，但目标移动/阶段窗口仍是全局帧，不随 Hitlag 暂停。扩散传播、燃烧 Tick 和草原核爆炸复用逐目标二维求交和阶段策略；结晶碎片与草原核在各自生成帧读取目标位置。玩家反应自伤只使用配置中的静态玩家位置/圆形碰撞体。跨目标命中确认产球仍聚合为一次。冻结的 `target-phase-v1` 只改变第一批 callback/Aura 衰减边界；`target-phase-v2` 也只声明 callback→该目标 `Reactable.Tick`。兼容预设和 legacy 输入继续选 legacy，迁移来的 v1 输入仍保持 v1。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
...
技能基线保存在 `packages/test-vectors/fixtures/legacy-default-120s.golden.json`。1.30–1.36 的历史升级继续按原契约保留；1.36 新增显式 opt-in `aura-v7` 和核心反应任务日志，1.37 再新增独立 `targetTaskModel` 与 v1 目标相位日志。1.35→1.36 是 identity-only 迁移，必须保留 v6、玩家模型、目标时钟和逐元素抗性，绝不能自动启用 v7；1.36→1.37 必须为历史输入补入 `legacy-event-heap-v1`，绝不能自动启用 target phase；1.37→1.38 必须原样保留 `legacy-event-heap-v1` 或 `target-phase-v1`，只有显式 1.38 输入才能选择 `target-phase-v2`。`legacy-default-120s-1.37.golden.json`、`quicken-bloom-task-order-1.37.golden.json` 与 `target-task-phase-1.37.golden.json` 是冻结的 1.37 Fixture；1.38 必须另写 `legacy-default-120s-1.38.golden.json`、`quicken-bloom-task-order-1.38.golden.json` 与 `target-reactable-phase-1.38.golden.json`，不能覆盖旧文件。`legacy-default-120s-1.36.golden.json`、`quicken-bloom-task-order-1.36.golden.json` 和 `reaction-matrix-1.35.golden.json` 继续保留为更早历史证据。Golden 相等只证明迁移兼容和对应结构化结果没有漂移，不证明测试抗性、杜林预设中的手工反应、装备系数、固定 gcsim 提交或其他示例魔法数是官方真值；这些边界继续标记 `provisional` 或 `fixed-gcsim-provisional`。
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.38 权威 Current State 为起点：先保持冻结的 1.37 Golden 与 `targetTaskPhaseLog`、独立的 1.38 Golden 与 `targetPhaseLog`、`runManifest`、核心 `reactionTaskLog` 的跨日志一致性和性能门，再补 Burning callback 内同步跨目标 Aura/反应命中、其他来袭元素的有序反应、完整多 Aura 排列和通用目标任务所有权；感电 Tick/Wane 与 Quicken→Bloom 等 core work 在另立版本契约前继续保持全局。之后才处理玩家 Aura/敌袭生存模型、角色机制映射、UID 配置编译和专用 UI。
