...
- Milestone 5 当前 Schema/引擎为 `1.16.0` / `1.16.0-circle-geometry`：除具名目标、逐目标 Aura/ICD、三层策略、阶段窗口和显式 fanout 外，命中可用静态二维圆形范围自动求交全部注册目标，并把距离/阈值写入逐击日志；跨目标命中确认产球仍聚合为一次。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
...
- 冻结的杜林预设仍为 `legacy-v0.1` Golden 兼容配置，手工反应和示例魔法数没有被包装为正式数据。多目标 AoE 已有显式扇出、静态二维圆形自动求交与一次产球语义；但具体技能范围仍需来源核验，且尚无其他形状、移动/索敌、真实 Boss AI、黑/白爆发、状态驱动的被动、命座和装备。
...
14. 已支持具名多目标、逐目标 Aura/ICD、显式 fanout 和静态二维圆形 AoE 自动扇出；尚无矩形/扇形/线段、高度、索敌、目标移动、护盾和特殊易伤窗口。
