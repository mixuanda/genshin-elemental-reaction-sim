...
- Milestone 3 第二批的超导切片已完成：Schema/引擎升级到 `1.23.0` / `1.23.0-superconduct-reaction`，同一通用转化反应调度/范围管线支持冰雷双向超导。独立冰伤使用 `1.5` 倍等级/精通公式；每个被反应伤害命中的目标获得 720 帧 `-40%` 物抗。目标级状态按半开区间作用于后续物理逐击，独立反应伤害之后才施加，同帧更早的普通命中不提前获益；刷新、截断、精确到期和来源伤害 ID 均进入 `reactionStatusLog`。Hitlag 延长与冻结底超导尚未实现。
...
- Milestone 5 当前 Schema/引擎为 `1.24.0` / `1.24.0-electro-charged-reaction`：除具名目标、逐目标 Aura/ICD、三层策略、阶段窗口和显式 fanout 外，命中可用二维圆形、可旋转矩形、有限线段胶囊或填充扇形范围自动求交全部圆形碰撞体目标；几何可使用世界坐标，或通过场景静态 `actorPoses` 将施放者局部坐标旋转/平移到世界。目标可沿 60 FPS 声明式线性分段移动，核心按命中帧插值并把坐标空间、施放者姿态、解析后形状、最近距离和阈值写入逐击日志。跨目标命中确认产球仍聚合为一次。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
