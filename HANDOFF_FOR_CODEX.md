...
- Milestone 5 已建立首个逐技能部分机制闸门：Schema/引擎升级到 `1.14.0` / `1.14.0-multi-target-registry`，支持声明式角色特有附着 ICD Profile、按角色/键共享的固定回能 ICD、逐击命中产球及其粒子 ICD、行动状态无前置清除、显式冲刺/跳跃命令、分后续动作取消帧、能量失败后的完整副作用回滚，以及具名多目标注册、逐目标 Aura/ICD、脚本化 `landed / miss`、三层目标策略和 60 FPS 半开目标阶段窗口；`AbilityBlueprint` 1.7 与通用编译器会从固定目录解析倍率、保留证据/前置条件/未实现项，并默认拒绝 `partial`。
...
- 冻结的杜林预设仍为 `legacy-v0.1` Golden 兼容配置，手工反应和示例魔法数没有被包装为正式数据。新增黑/白 E 审计向量不能替代它；当前已映射普攻/重击/战技/爆发/冲刺/跳跃/切人取消，但冲刺/跳跃仍不含耐力和移动物理。多目标注册、显式 Miss、三层免疫策略与按帧阶段已有共同结算门，但每段命中仍只选择一个目标；几何 AoE 扇出、一次产球语义、真实 Boss AI、黑/白爆发、状态驱动的被动、命座和装备尚未完成。
...
14. 已支持具名多目标的顺序逐击和逐目标 Aura/ICD，但同一攻击尚不能按 AoE/命中范围自动扇出，也没有索敌、移动、护盾和特殊易伤窗口。
