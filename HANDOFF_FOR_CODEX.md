...
- 当前输入契约为 `schemaVersion: 1.48.0`、`engineVersion: 1.48.0-reaction-owned-application-root`，当前 `runManifest.version` 为 `1.4.0`；反应核心、目标任务、交付顺序、感电传播、经典反应公式、普通直伤组、数字元素施加和反应所有施加策略分别由显式版本字段固定。1.46→1.47 的显式 selector 迁移保持冻结；1.47→1.48 只更新身份、注入 `reactionOwnedElementalApplicationModel`，不猜测 fixed group、不自动绑定未核验技能、不改变默认伤害，也不启用策略范围外的派生施加。精确 V147/V146/V145/V144/V142 wire、Manifest 和 Fixture 保持冻结；精确 1.44–1.48 均可在既有版本门下显式选择 v3。历史版本夹带未来 model/selector 会 fail-closed。
- `aura-v5` 已把正/反融化、正/反蒸发、超载、超导、感电、冻结/碎冰、火/水/冰/雷扩散、火/水/冰/雷结晶、原激化/超激化/蔓激化、燃烧，以及绽放/草原核/烈绽放/超绽放接入无 DOM 的核心。`aura-v6`–`aura-v9` 继续冻结共享 Gauge 有序链、Quicken→Bloom core task、EC cleanup 与全局 cadence/Hitlag/Wane 所有权。精确 1.42 历史身份下 v9 只允许 `legal-frame-v1 + 60 FPS + target-phase-v2`；精确 1.44–1.48 可在同样的帧率/时间线门下与 v2 或显式 v3 组合。这些经典反应路径仍全部是 `fixed-gcsim-provisional`，不能视为官服真值或完整 gcsim 精度；v1–v8 的历史语义和 Golden 保持不变。
...
- 当前 V148 完整性门继承 V147 的基础反应、公式、普通直伤组与配置直接 application 证明，再从第四策略根独立重放 Burning/Swirl 目标 attempt、独立状态窗口、数字倍率、名义/有效 Gauge 与 reciprocal 外键。V147 及更早历史证明保持原样；V148 新增层不等于从完整 Ability/Aura、动作快照或角色数据库重放全链，也不证明其他反应应该施加元素。
...
- 每次模拟返回 Manifest 1.4.0，固定配置哈希、Schema/引擎/数据版本、解析后的运行选项、随机种子、有序插件身份、完整 `reactionFormulaRoot`、`directDamageGroupRoot`、`elementalApplicationIcdRoot` 与 `reactionOwnedElementalApplicationRoot`；复现键使用 `gdl-v2-fnv1a32-*`。冻结 V147 Manifest 仍是 1.3.0。当前 32-bit FNV-1a 已知可碰撞，只是 drift detector，不能作为密码学身份或签名。
...
- 当前现场结果：完整 unit 为 117 个文件、1550 项测试；data check 确认 120 个角色、125 套天赋、762 个技能/被动、237 把武器；typecheck、build 与 Playwright Chromium 36/36 通过。性能探针的伤害/事件/hash 均未漂移，但最终墙钟门在高系统负载下为 2/3：持续 Burning 中位数 `144.942ms` 超过未放宽的 `<100ms` 桌面门，需在低负载主机复验 3/3。`npm run check` 因此尚未全绿，且本来不包含 E2E；浏览器门仍须单独运行。本轮没有扩展 UI，浏览器只验证既有指标、角色/技能构成与伤害曲线。build 仍只有主 JS chunk 超过 500 kB 的非阻断警告。
...
- Zod 已严格覆盖当前 V148 输入与结果；当前 `simulationResultV148Schema` 和 `assertTrustedSimulationResultV148()` 在 V147 全部证明上增加反应所有施加策略根、Burning/Swirl attempt 重放和 reciprocal 外键闭合。冻结 `simulationResultV147Schema`、`simulationResultV146Schema`、`simulationResultV145Schema`、`simulationResultV144Schema`、`simulationResultV142Schema` 仍保持精确身份并由各自断言接纳。外部或持久化完整结果必须走完整 Zod；trusted assertion 不是外部 wire 验证边界。
...
- Burning 的 `legacy-event-heap-v1` 有意保留 1.30 兼容相位；1.37 v1 与 1.38 v2 也继续冻结。精确 1.44–1.48 的显式 v3 可在 owner callback 内交付列明的零延迟跨目标伤害/Aura；启用目标时钟时陈旧唤醒会先重投影。1.48 只将实际 Burning Tick 的 target-global observable projection 纳入统一日志，不改写 1.47 直接命中 wire。所有这些顺序仅为 `fixed-gcsim-provisional`，不是官服真值、完整 target phase 或完整 gcsim 精度。
...
- 当前 Schema/引擎为 `1.48.0` / `1.48.0-reaction-owned-application-root`，Manifest 为 1.4.0。既有具名目标、Aura/反应、几何、时钟、EC cleanup/传播、普通直伤组和 V147 配置直接 application 边界保持不变；第四份策略 root 只追加 Burning Tick 与 Swirl AoE 传播。默认预设和迁移继续保留既有 Aura/目标任务/反应交付模式；精确 1.44–1.48 输入均可在版本门下 opt-in `target-phase-v3`。`AbilityBlueprint` 当前为 1.9，1.0–1.8 可迁移；编译器可显式传递两类 descriptor/application selector，但不会自动猜测 fixed group 或反应所有施加。
...
3. 使用 Zod 校验输入 JSON 并提供字段路径错误。当前精确 1.48 输出由 `simulationResultV148Schema` 覆盖全部已声明顶层字段和叶节点；冻结 `simulationResultV147Schema` / `simulationResultV146Schema` / `simulationResultV145Schema` / `simulationResultV144Schema` / `simulationResultV142Schema` 仍只接受各自精确历史身份。外部或持久化结果必须完整解析，核心内部当前结果运行 `assertTrustedSimulationResultV148()`。V145 公式根、V146 Damage Group root/log、V147 配置直接 application root/log、V148 反应所有策略 root/log 与相关外键都必须 strict 校验；trusted assertion 不能作为外部 wire 的验证边界。
4. 加入 Schema 迁移系统，例如：
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.48 权威 Current State 为起点：保持全部历史 Golden、Manifest 1.4 四 root、两份 1.48 Fixture、冻结 V147 wire/Fixture、核心 `reactionTaskLog`、反应交付/EC cleanup 引用、1.41/1.42 传播与 cadence 向量、24 标签/16 类经典反应发布门，以及 1.44–1.48 v3 callback delivery/attempt 完整性契约。下一步先扩展 Burning/Swirl 的特殊 Aura、多目标、多代次与同帧向量，再逐技能绑定经过来源核验的两类 Tag/Group；任何新的反应所有施加都必须先核对固定来源并升版策略根。Lunar 反应另立版本实现。基础机制稳定后才处理玩家 Aura/敌袭生存模型、全角色机制映射、UID 配置编译和专用 UI。
