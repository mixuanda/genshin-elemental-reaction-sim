...
- Milestone 5 当前 Schema/引擎为 `1.15.0` / `1.15.0-aoe-fanout`：除具名目标、逐目标 Aura/ICD、三层策略和阶段窗口外，同一逻辑命中已可扇出到多个目标，并把命中确认产球聚合为一次；`AbilityBlueprint` 1.7 与通用编译器仍保留来源、前置条件和未实现项闸门。
...
- 冻结的杜林预设仍为 `legacy-v0.1` Golden 兼容配置，手工反应和示例魔法数没有被包装为正式数据。多目标 AoE 已有显式扇出与一次产球语义，但扇出列表仍是场景输入；几何命中、真实 Boss AI、黑/白爆发、状态驱动的被动、命座和装备尚未完成。
