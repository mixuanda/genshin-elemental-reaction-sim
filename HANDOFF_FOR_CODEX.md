...
- 当前输入契约为 `schemaVersion: 1.41.0`、`engineVersion: 1.41.0-ec-secondary-wet-propagation`；反应核心、目标任务边界和伤害交付顺序仍分别由 `reactionEngine.mode`、`targetTaskModel.mode` 与 `reactionDeliveryModel.mode` 显式选择。1.41 新增独立 `electroChargedPropagationModel`：内置预设与 1.40→1.41 迁移结果必须使用 `{ mode: "single-target-v1" }`；只有精确 1.41 输入才能显式选择 `{ mode: "nearby-wet-radius-v1", radius, verificationStatus: "provisional" }`。历史版本夹带该未来字段会 fail-closed。迁移保留全部历史 Aura、玩家、目标时钟/任务和反应交付选择，绝不自动启用附近湿目标传播。
...
- Zod 已严格覆盖输入、`SimulationRunManifest`、Burning/Quicken/Bloom 审计、核心 `reactionTaskLog`、冻结 v1 的 `targetTaskPhaseLog`、v2 的 `targetPhaseLog`、1.39 反应交付父链/碎冰日志引用、1.40 `electroChargedCleanup` reciprocal 引用、1.41 `electroChargedPropagationAudit` 的候选顺序/选择理由/逐目标伤害反链、ReactionA/B、草原核、玩家伤害、目标时钟/Hitlag 和 `TargetStateTimeline` 等关键输出。1.41 结果门还必须拒绝旧身份夹带传播字段、`single-target-v1` 伪造传播审计、候选排序/几何不守恒、缺失或重复伤害反链，以及传播伤害与源 Tick owner/snapshot 不一致。完整 `SimulationResult` 仍没有覆盖所有字段的单一顶层运行时 Zod Schema。
...
- 明确未完成：除列明的 v6–v8 代表性来袭链之外的其他来袭元素全组合、一般化 Aura overlap、所有特殊/多 Aura 可达排列、附近传播的官服半径/目标上限/层数/选择规则、跨过 F70 callback 的长 Hitlag Golden、Lunar/月反应、完整角色特有 ICD、真实粒子/掉球与飞行、全角色动作/取消帧、从角色/武器/攻击类型自动推导 Hitlag、玩家自身 Hitlag/动作冻结、通用敌方任务暂停与任务所有权、Burning callback 内同步跨目标 Aura/反应命中、固定 gcsim 的 `DoNotLog` “Freeze Broken” 合成攻击、玩家 Aura/敌袭/治疗/死亡/复活、完整三维命中，以及全角色/武器/圣遗物/敌人的可执行机制数据库。可查询目录只表示 `provisional + metadata-only` 覆盖；UID 展示柜仍不能可靠编译任意账号为 `SimConfig`，“毕业站位”也仍是不可模拟占位。默认杜林预设的示例魔法数继续是 `provisional`。本项目仍未达到 gcsim 精度。
...
- 反应核心优先阶段已推进到 1.41：1.40 的 `aura-v8` 已冻结 Quicken→Bloom 后 EC next-target-Tick cleanup；1.41 再以独立、默认关闭的 `nearby-wet-radius-v1` 实现附近湿目标逐目标伤害传播和候选审计，两份 1.41 Golden 已冻结。当前顺序是补长 Hitlag、其他来袭元素与多 Aura 排列，再实现 Burning callback 内同步跨目标 Aura/反应命中和通用目标任务所有权；随后才考虑玩家 Aura、敌袭、治疗/死亡等生存机制和新增展示。
...
- 当前 Schema/引擎为 `1.41.0` / `1.41.0-ec-secondary-wet-propagation`。既有具名目标、逐目标 Aura/ICD/抗性、二维几何、目标移动/阶段、目标时钟和 EC cleanup 边界保持不变；传播半径只在 `nearby-wet-radius-v1` 的 Tick 交付点读取源/目标位置与圆形 hurtbox。默认预设和迁移输入继续使用 `single-target-v1` 并保留既有 Aura/目标任务/反应交付模式。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.41 权威 Current State 为起点：保持全部历史 Golden、两种目标相位日志、`runManifest`、核心 `reactionTaskLog`、1.39 反应交付引用、1.40 EC cleanup reciprocal 引用，以及两份已冻结的 1.41 Fixture。随后补长 Hitlag、其他来袭元素的有序反应、完整特殊/多 Aura 排列、Burning callback 内同步跨目标 Aura/反应命中和通用目标任务所有权。基础机制稳定后才处理玩家 Aura/敌袭生存模型、角色机制映射、UID 配置编译和专用 UI。
