...
- Zod 已严格覆盖输入、`SimulationRunManifest`、Burning/Quicken/Bloom 审计、核心 `reactionTaskLog`、冻结 v1 的 `targetTaskPhaseLog`、v2 的 `targetPhaseLog`、1.39 反应交付父链/碎冰日志引用、1.40 `electroChargedCleanup` reciprocal 引用、1.41 `electroChargedPropagationAudit` 的候选顺序/选择理由/逐目标伤害反链，以及 1.42 `aura-v9` 的 `tick-skipped`、cadence status、Wane listener、逐来源 Wane 与 `ended-before-deadline` reciprocal 审计；ReactionA/B、草原核、玩家伤害、目标时钟/Hitlag 和 `TargetStateTimeline` 等关键输出也继续受严格校验。v9 结果门必须按 EC 所有权过滤周期记录，既允许无 EC 的 Melt/Overload 等 1.42 结果，也继续严格验证纯 EC 和混合 EC + 非 EC 的反链。完整 `SimulationResult` 仍没有覆盖所有字段的单一顶层运行时 Zod Schema。
