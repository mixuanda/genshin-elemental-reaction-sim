...
- 当前输入契约为 `schemaVersion: 1.45.0`、`engineVersion: 1.45.0-reaction-formula-root`，当前 `runManifest.version` 为 `1.1.0`；反应核心、目标任务边界、伤害交付顺序、感电目标范围和经典反应公式选择分别由 `reactionEngine.mode`、`targetTaskModel.mode`、`reactionDeliveryModel.mode`、`electroChargedPropagationModel.mode` 与 `reactionFormulaModel` 显式固定。1.44→1.45 迁移只更新运行身份并注入固定公式选择，原样保留 Aura、传播、目标任务、目标时钟、玩家伤害与反应交付模式，不改变伤害数值，也不自动启用 `target-phase-v3`、`aura-v9` 或附近湿目标传播。冻结的精确 1.44 V144 wire 和 Manifest 1.0.0 保持原语义；精确 1.44 与当前 1.45 均可在 `legal-frame-v1 + 60 FPS + aura-v7/v8/v9` 门下显式选择 v3。1.43 仍只是尚未发布的 energy wire 保留版本号。历史版本夹带 `reactionFormulaModel` 等未来字段或未来模式会 fail-closed。
- `aura-v5` 已把正/反融化、正/反蒸发、超载、超导、感电、冻结/碎冰、火/水/冰/雷扩散、火/水/冰/雷结晶、原激化/超激化/蔓激化、燃烧，以及绽放/草原核/烈绽放/超绽放接入无 DOM 的核心。`aura-v6` 沿用这些主体语义，并对雷元素来袭按固定 gcsim 提交 `b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541` 以共享来袭 Gauge 依次检查 `超激化 → 超载 → 感电 → 冻结底超导 → 普通超导 → 原激化 → 绽放`。`aura-v7` 继承 v6，但把 Quicken 后的水草绽放跟进改为 core zero-delay task：任务继承触发事件的 frame/priority、取得更晚的全局 sequence，因而在已入队的同帧命中后按 FIFO 执行，并在执行点重读实时 Hydro/Quicken Aura。缺失任一状态时任务明确跳过，不使用触发时的陈旧快照伪造 Bloom；它不属于敌方 per-target queue，冻结的 1.37 v1 和 1.38 v2 都不会把它重新分类为 target-owned task。1.40 的 `aura-v8` 继承 v7，并在 follow-up 恰好耗尽同代最后 Hydro 时，把 EC cleanup 延迟到下一次有效目标 Tick。1.42 的 `aura-v9` 再继承 v8 cleanup，并把 F+10 首伤与 F+70 callback 独立排在全局帧：目标 Hitlag 只冻结 `Reactable.Tick` / Aura；F70 缺水雷共存写 `tick-skipped` 并进入 dormant，F70 前恢复可正常结算，F71 后恢复不重启 cadence；非零伤害才维持 `+6f` Wane listener，Wane 对每个 Hydro/Electro 来源槽各扣至多 `0.4U`；流在 cleanup 截止前已终止时以 `ended-before-deadline` 和 reciprocal target-phase / timeline / periodic 引用闭合，旧代 Wane 不得污染替换代次。精确 1.42 历史身份下 v9 只允许 `legal-frame-v1 + 60 FPS + target-phase-v2`；精确 1.44 与当前 1.45 可在同样的帧率/时间线门下与 v2 或显式 v3 组合。这些经典反应路径仍全部是 `fixed-gcsim-provisional`，不能视为官服真值或完整 gcsim 精度；v1–v8 的历史语义和 Golden 保持不变。
...
- 当前 1.45 完整性门继承并加固 1.44 的基础反应链输入根：`enemyTargets.initialAura` 必须按所选 Aura 耐久版本精确规范化为 simulation-start 状态（v1/v2 `1U -> 0.8U / 426f`，v3–v9 `1U -> 0.8U / 570f`），不能只让初始状态与时间线互相自证。所有转化反应 DamageEvent 的 `sourceActorId / scalingOwnerId / creditOwnerId` 及名称投影必须归属于同一个反应来源，禁止把感电等逐击面板或伤害归属协调转嫁给旁观角色；V145 还必须由固定公式 profile 重算反应等级基准与倍率。
...
- 1.45 已新增并现场复核只读兼容 Fixture：`legacy-default-120s-1.45.golden.json` = `ce59efca02ea2a895195139a3775ec0eeefe6b73414603ee8650e46b2e3c2167`，`configHash = fnv1a32:e53f9200`，`reproducibilityKey = gdl-v2-fnv1a32-b696a75d`。它冻结当前身份、Manifest 1.1.0 与上述公式根；总伤 `41410555.13728799`、DPS `345087.9594773999`、269 命中、129 反应命中、3 跳过行动、角色/技能浮点汇总和逐击 digest `b3bddf486cf85967f8be689ccad860a450377fab5f3e2318655430324348652f` 与 1.44/1.42/Vanilla 一致。V144 和所有更早 Fixture SHA 保持不变。
- 当前 1.45 全量 Vitest 已实际运行并通过 `87` 个测试文件、`1205/1205` 项测试；该记录不代表本轮最终 build、性能门或 Playwright 已完成，三者结果必须以后续现场命令为准。
...
- Zod 已严格覆盖输入和当前精确 1.45 `SimulationResult` 的全部 65 个顶层字段；当前 `simulationResultV145Schema`、冻结 `simulationResultV144Schema` 与冻结 `simulationResultV142Schema` 分别保持精确身份。V145 继承 1.44 的 v3 callback、delivery/attempt 与所有既有跨日志完整性门，并新增配置公式选择、Manifest 公式根、来源角色等级、增幅/转化/扩散/加算反应交付的固定 profile 证明。外部或持久化的完整结果必须走完整 Zod；核心内部当前结果在公开返回前使用 `assertTrustedSimulationResultV145()`（当前别名同样指向 V145）做零拷贝跨字段断言。冻结 V144 仍由其精确 Schema/断言独立接纳；内部可信断言不等同于完整 Zod parse，也不能用于接纳外部或持久化 wire。
- 精确 V145 的所有命中都禁止输入 `ampBase`，增幅底数只能由固定公式 profile 决定；冻结 V144 及更早 wire 继续按各自历史契约接受已经合法的显式 legacy 覆盖。Aura 模式只允许显式 debug 且非 `none` 的 `reactionOverride`。公式层拒绝非正、非有限底数，并对转化/加算公式的非法运行时输入 fail-closed。
...
- Burning 的 `legacy-event-heap-v1` 有意保留 1.30 兼容相位：Fuel 于第 121 个目标帧清理，且同一目标帧普通命中先于 Tick；v7 的 refresh 计数修正不会反向改写它。1.37 v1 与 1.38 v2 也继续冻结，其实际 Burning 范围伤害仍在后续全局 `reactionDamage` 处理。精确 1.44 与当前 1.45 的显式 v3 可在 owner callback 内交付零延迟跨目标伤害/Aura；尚未运行当帧 Tick 的接收目标按 F-1/当前 Aura→F `Reactable.Tick` 结算，已运行 Tick 的目标在其后应用。启用目标时钟时陈旧唤醒会先重投影。所有这些顺序仅为 `fixed-gcsim-provisional`，不是官服真值、完整 gcsim target phase 或完整 gcsim 精度。
...
- 当前 Schema/引擎为 `1.45.0` / `1.45.0-reaction-formula-root`，Manifest 为 1.1.0。既有具名目标、逐目标 Aura/ICD/抗性、二维几何、目标移动/阶段、目标时钟、EC cleanup 和传播边界保持不变。默认预设和迁移输入继续使用 `legacy-event-heap-v1`、`single-target-v1` 并保留既有 Aura/反应交付模式；精确 1.44 或 1.45 输入均可在版本门下 opt-in `target-phase-v3`。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
...
3. 使用 Zod 校验输入 JSON 并提供字段路径错误。当前精确 1.45 输出由 `simulationResultV145Schema` 覆盖全部 65 个顶层字段和所有已声明叶节点；冻结 `simulationResultV144Schema` / `simulationResultV142Schema` 仍只接受各自精确历史身份。外部或持久化结果必须完整解析，核心内部当前结果则运行 `assertTrustedSimulationResultV145()`。v3 delivery/attempt 与 V145 公式根/反应交付外键都必须 strict 校验；内部可信断言不等同于完整 Zod parse，不能作为外部 wire 的验证边界。
4. 加入 Schema 迁移系统，例如：
