...
- Milestone 5 当前 Schema/引擎为 `1.19.0` / `1.19.0-capsule-geometry`：除具名目标、逐目标 Aura/ICD、三层策略、阶段窗口和显式 fanout 外，命中可用二维圆形、可旋转矩形或有限线段胶囊范围自动求交全部圆形碰撞体目标；目标可沿 60 FPS 声明式线性分段移动，核心按命中帧插值并把位置、形状端点/尺寸/旋转、距离和阈值写入逐击日志。跨目标命中确认产球仍聚合为一次。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
...
- 冻结的杜林预设仍为 `legacy-v0.1` Golden 兼容配置，手工反应和示例魔法数没有被包装为正式数据。多目标 AoE 已有显式扇出、二维圆形/旋转矩形/胶囊自动求交、声明式线性目标移动与一次产球语义；但具体技能范围和实战移动轨迹仍需来源核验，且尚无扇形、高度、角色移动/索敌、真实 Boss AI、黑/白爆发、状态驱动的被动、命座和装备。
...
14. 已支持具名多目标、逐目标 Aura/ICD、显式 fanout、二维圆形/旋转矩形/胶囊 AoE 自动扇出和声明式目标线性移动；尚无扇形、高度、角色移动、AI/命令驱动敌人移动、索敌、护盾和特殊易伤窗口。
