...
### 2026-08-02 权威 Current State

以下状态优先于后面的历史里程碑描述：

- 当前输入契约为 `schemaVersion: 1.53.0`、`engineVersion: 1.53.0-callback-bus`，当前 `runManifest.version` 为 `1.9.0`；V1.52 的七份信任根、wire、Manifest 1.8 和 Fixture 完整冻结，新增第八根 `callbackBusRoot`。精确 1.52 输入迁移到 1.53 时原样保留 Freeze Broken V1/V2 并注入 callback bus V1；更早输入先走冻结迁移链，再注入 bus V1。迁移绝不静默启用 Freeze Broken V3 或 callback bus V2。原生 current 只允许 V1/V2 + bus V1，或 V3 + bus V2；交叉配对 fail-closed。迁移不猜测 fixed group、不自动绑定未核验技能、不改变默认伤害，也不物化参考合成攻击。精确 V152/V151/V150/V149/V148/V147/V146/V145/V144/V142 wire、Manifest 和 Fixture 保持冻结；精确 1.44–1.53 均可在既有版本门下显式选择 `target-phase-v3`。历史版本夹带未来 model/selector 会 fail-closed。公开仓库为 <https://github.com/mixuanda/genshin-elemental-reaction-sim>。
- `aura-v5` 已把正/反融化、正/反蒸发、超载、超导、感电、冻结/碎冰、火/水/冰/雷扩散、火/水/冰/雷结晶、原激化/超激化/蔓激化、燃烧，以及绽放/草原核/烈绽放/超绽放接入无 DOM 的核心。`aura-v6`–`aura-v9` 继续冻结共享 Gauge 有序链、Quicken→Bloom core task、EC cleanup 与全局 cadence/Hitlag/Wane 所有权。精确 1.42 历史身份下 v9 只允许 `legal-frame-v1 + 60 FPS + target-phase-v2`；精确 1.44–1.53 可在同样的帧率/时间线门下与 v2 或显式 v3 组合。这些经典反应路径仍全部是 `fixed-gcsim-provisional`，不能视为官服真值或完整 gcsim 精度；v1–v8 的历史语义和 Golden 保持不变。
...
- V147 的 `elementalApplicationIcdLog` 为每个配置应用的普通直接命中目标尝试恰写一行。`miss`、`target-aura-blocked`、`no-aura-engine`、`mechanics-truncated` 都是明确 skip reason 且不推进窗口；evaluated 行闭合 selector、窗口、序列、名义/有效 Gauge 和 reciprocal 命中/伤害外键。带应用的 physical 命中会拒绝，包括继承出的 physical 元素。该历史 wire 保持冻结。
- 1.48 新增第四份无 DOM 策略根 `gcsim-b4ae769-reaction-owned-elemental-application-policy-provisional-v1`，固定同一 gcsim 提交，内容 SHA-256 为 `sha256:50abcc04ad7bc55510e5786cbc3ace5105238e5fb2dea8e0944cd95708c80acc`。它只覆盖 Burning Tick 的 `ICDTagBurningDamage / burning / 1U Pyro` 和火/水/冰/雷 Swirl AoE 传播的 `ICDTagSwirl* / reaction-a / propagated Gauge`。Swirl 源目标的伤害段不携带 Durability，不生成施加行。配置直接命中和反应所有命中使用隔离的 configured/reaction-owned 状态命名空间；Burning 使用受信任 target-global observable projection，Swirl 使用 per-target/source/tag 窗口。
- V148 将 `elementalApplicationIcdLog` 扩展为 `configured-direct-hit | burning-tick | swirl-propagation` 统一日志，并与 DamageEvent、HitResolution、ReactionDamage 和 `target-phase-v3` attempt 建立 reciprocal 引用。public Zod 与 trusted 完整性门从第四 root 重放来源、selector、状态窗口、数字倍率和 Gauge。EC/Wane、Overload、Superconduct、Shatter、Bloom/Burgeon/Hyperbloom、Quicken 后续、core damage 与 Crystallize damage/lifecycle 不得生成反应所有施加行。策略根显式标记 `officialServerTruth: false`、`completeGcsimParity: false` 与 `sameFrameOrdering: provisional-source-task-insertion-dependent`，不得宣称官服同帧真值或完整 gcsim parity。
- 1.49/V2 只收紧两个已有 channel 的重置边界：Burning Tick 由 `enemy-target-task` 所有，精确截止帧按 `attempt-before-core-reset` 先尝试后重置；Swirl 是 follow-up core propagation，按 `reset-before-attempt` 先重置后尝试。V2 仍显式标记 `provisional`、`officialServerTruth: false`、`completeGcsimParity: false`，不能外推为所有同帧任务或官服真值。
- V149 完整性门要求同一反应 parent 的所有 resolution 共享 priority/sequence，并拒绝两个独立 parent 复用同一 `eventSequence`。结果尚未包含完整 scheduler trace，因此一整条合法事件链的协同 `+1000` sequence 偏移仍不能仅凭公开结果证伪。
- 1.50 把四种扩散伤害、碎冰、超导、绽放/烈绽放/超绽放绑定到 ReactionA `[1, 1, 0…]`，把超载与感电绑定到 ReactionB `[1, 0…]`。状态按“接收目标 + 来源角色 + icdTag”隔离。V1 冻结 F+30 lazy reset；V2 在开窗帧 F 排定 F+29 reset task，并与同帧伤害尝试按全局任务插入序/`eventSequence` FIFO 决议。`reactionDamageGroupResetLog` 与每条 decision 记录 generation、窗口、序列槽、排定/执行先后和 reciprocal backlink。递归零延迟 Shatter 使用 parent 任务序号，能分辨 hit-before-reset 与 reset-before-hit。该局部 scheduler 仍是 `fixed-gcsim-provisional`，不是官服真值或通用 gcsim scheduler parity。
- 1.51 新增第六信任根 `basicReactionSchedulerRoot`。V1 保留历史 Swirl 传播命中结算后即时提交未反应附着的兼容语义；V2 先完整结算同帧 Swirl attack cohort，再按全局 task insertion order 提交未反应附着，提交不重复运行 ICD 或反应判定。每个 attack/commit 对写入 `basicReactionSchedulerLog`，commit 另生成 `reactionAuraAttachment` 事件并双向回链攻击、Aura 变更和目标状态时间线。该调度根只窄覆盖当前审计的 Burning/Swirl 路径，不是通用全目标 barrier，也不表示完整 gcsim scheduler parity。
- 1.52 新增第七信任根 `freezeBrokenAttackRoot`。V1 冻结 1.51 的“无 callback、无日志”兼容语义；V2 只在 Frozen 从正值耗尽到零时写一条规范化审计行。五类本地正触发是自然衰减、韧性削冻、碎冰削冻、冰扩散和冰结晶；mixed Hydro + Frozen Swirl 向量保留有序的 Hydro/Cryo Swirl，并把审计归因于耗尽 Frozen 的 Cryo 分支。Melt、Superconduct 和部分 Frozen 消耗是明确负例。只要有行，顶层 `mechanicsStatus` 为 `partial`；零行结果为 `complete`。
- 冻结 V1.52 的 V2 固定参考 `genshinsim/gcsim@b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541`，但执行状态严格为 `reference-audit-only-not-dispatched`。行内保留 `sourceIsSim: true`、`doNotLog: true` provenance，`damageEventId` / `hitResolutionLogId` 固定为 null；本地不消费 RNG，不物化 DamageEvent/HitResolution，不增加命中、伤害曲线、总伤或 DPS。该冻结路径没有 callback bus、Mona bubble/impulse、全局/订阅者副作用或参考 crit RNG draw；固定参考中的 `doNotLog` 在该路径上是未读取字段。不得把这份 audit root 写成已执行合成攻击、官服真值或完整 gcsim parity。
- 1.53 新增 Freeze Broken V3 root `fixed-gcsim-freeze-broken-callback-dispatch-v3` / `gcsim-b4ae769-freeze-broken-callback-dispatch-provisional-v3` / `sha256:7c6b09c56e2e70fcdee5907045cdd29e1c81474c700c0685c6b3684a34eb298b`。它只与 callback bus V2 合法配对，并将每个正值到耗尽 transition 投影为五阶段：前三阶段同步，`on-enemy-damage` 与 attack callback 在参考命名的零延迟 `end-of-frame` phase 中依次交付。本地 heap 只保证该内部任务排在 trigger 之后，不建立整帧全局 barrier。五阶段仍是非伤害 sidecar，不物化合成 DamageEvent/HitResolution，不消费参考 crit RNG，不改变命中、曲线、总伤或 DPS。
- callback bus V1 root 为 `legacy-no-versioned-callback-bus-v1` / `legacy-no-versioned-callback-bus-v1` / `sha256:2b4941332f75b605e86b3fd7bf2169e3455ed88fa9cf353f8a5d9f3104e691ce`；V2 root 为 `fixed-gcsim-versioned-callback-bus-v2` / `gcsim-b4ae769-versioned-callback-bus-normalized-provisional-v2` / `sha256:e9a07c467716a1b5bf63859945262a3abc3c54ae7407f124d0a68cc7ec380696`。V2 是无 DOM 的 typed deterministic bus：按事件内插入顺序交付，duplicate key 在原槽替换而不重排，unsubscribe 保留 tombstone，dispatch 中 mutation/reentrancy 拒绝。`callbackRegistrationLog`、`callbackDeliveryLog` 与 V3 audit 必须 reciprocal 闭合。
- callback subscriber context 只读，不能修改状态、生成伤害、调度任务或消费 RNG。V1.53 Manifest 的 `pluginCapabilities` 与 `plugins` 严格 1:1 对齐；`pluginCallbackSubscriptions` 再逐槽保存有序 `{eventKind, subscriberKey}`，damage 槽必须为空，startup registration 必须逐位置反链声明。权限与订阅顺序都纳入复现键；registration/attempt 外键只能指向 `callback-subscriber` 槽。outcome 验证等级固定为 `structural-only-unverified-runtime-output-v1`：public Zod 只证明结构、顺序、外键和下游不变量，插件代码与 `contentHash` 仍是信任声明，不能从公开 JSON 重放任意运行时代码或证明插件真实返回值。callback-only plugin 在既有 direct-damage plugin trace 中只能产生显式 no-change placeholder，不能调用 `modifyDamage`。V1.52 及更早 plugin-entry/Manifest wire 不增加这两个字段。
- Mona bubble/Omen、impulse/一般敌人物理、有状态订阅者副作用、参考 crit RNG draw、合成攻击 DamageEvent/HitResolution 和一般 gcsim callback/event/task parity 仍未实现。不得把 V1.53 写成完整 Freeze Broken 执行、完整 gcsim callback bus 或官服真值。
- 当前开发优先级是基础经典反应、元素附着 ICD、Aura 生命周期和结果完整性，不是展示扩张。1.44 的 Burning 结果门现在反向要求 start/stop 审计，重放 generation、首 Tick、15f cadence、tickIndex、Fuel 自然到期、callback、反应伤害与时间线反链，并把 Marker/Fuel 标量和 Fuel-expire 的 Aura 消费扎回权威快照；普通 stop 必须有精确 audit、callback、直接 Aura 消费、目标相位衰减或显式机制截断所有者。每个目标的首个物化 Burning 流固定为 generation 1，终止流保留退出 generation，内部 stop 递增一次后下一次 start 再递增一次，因此合法重启为退出 generation `+2`；这套 monotonic generation guard 是本地避免陈旧 callback 复活的安全偏差，不声称复制固定 gcsim 的 frame-token 重启细节。目标时钟模式下，所有 lifecycle row 直接绑定当前事件切点重放出的目标帧。v3 `before-reactable-tick` 附着的 audit 快照绑定上一全局帧末尾的目标本地帧：无 Hitlag 时它比 lifecycle 帧小 1，跨越该 callback 的 Hitlag 下两者可以相同，禁止机械地做 `-1/+1`。仍未从输入配置完整重放每个 Burning Gauge、全部特殊 Aura 和所有可达反应链，因此不能把这些门称为完整 Aura 证明。
- 当前 UI 不扩展；既有全队、个人、技能、时间轴、逐段伤害、构成和曲线继续只读核心返回值。V1.52 Freeze Broken audit 和 V1.53 callback-bus sidecar 都不进入伤害展示。尚未完整实现的边界包括 Mona bubble/Omen、impulse/一般敌人物理、有状态 subscriber/RNG 行为、Lunar 反应族、特殊/多 Aura 可达空间、全技能 ICD 绑定、真实粒子与产球数据、完整动作帧/快照、全角色/武器可执行机制库，以及可靠的 `ShowcaseSnapshot -> ResolvedLoadout -> SimConfig` UID 管线；“毕业站位”仍是不可执行占位，冻结杜林预设的示例魔法数仍是 provisional。
- 历史 `defReduction` 是有符号 defense adjustment：负值表示减防，正值表示增防；当前只为冻结 wire 兼容保留，后续必须通过版本迁移改成无歧义字段名。旧 `ReactionALimiter` / `ReactionBLimiter` 已 deprecated，不应成为新接口或新数据的命名依据。
...
- 当前 V153 完整性门继承冻结 V152 的全部证明，再按配置重放 callback bus V1/V2 root、合法 Freeze Broken 配对、注册操作、五阶段 delivery、subscriber attempt 和 reciprocal references。V1/V2 + bus V1 保持冻结结果和空 callback 日志；V3 + bus V2 的正触发必须恰好 3+2 个阶段，Melt/Superconduct/部分消耗须零行。所有 V3 行都不得链接 DamageEvent/HitResolution 或消费 RNG。V152 及更早历史证明保持原样；V153 新增层不等于实现 Mona/Omen、物理、有状态 subscriber，或从完整 scheduler trace、Ability/Aura、动作快照、角色数据库重放全链。
...
- 每次 current 模拟返回 Manifest 1.9.0，固定配置哈希、Schema/引擎/数据版本、解析后的运行选项、随机种子、有序插件身份、与插件 1:1 对齐的 capability、前七份冻结 root、已选 `freezeBrokenAttackRoot` 与 `callbackBusRoot`；复现键使用 `gdl-v2-fnv1a32-*`。冻结 V152 Manifest 仍是 1.8.0，V151 仍是 1.7.0，V150 仍是 1.6.0，V149 仍是 1.5.0。当前 32-bit FNV-1a 已知可碰撞，只是 drift detector，不能作为密码学身份或签名。
...
- 1.48 新增两份只读 Fixture，不回写 1.47 或更早文件。`legacy-default-120s-1.48.golden.json` = `563c417efe82582c9647670104b39e0c34074ceb18259a8aaa36e9c997079d5c`，`configHash = fnv1a32:6d2a6835`、`reproducibilityKey = gdl-v2-fnv1a32-e69ac333`；它保持总伤 `41410555.13728799`、DPS `345087.9594773999`、269 命中、129 反应命中、3 跳过行动、全部角色/技能汇总和伤害 digest `b3bddf486cf85967f8be689ccad860a450377fab5f3e2318655430324348652f`。`reaction-owned-application-1.48.golden.json` = `704c5db38dda87802aa000d664812b63673ea9498981ed21f26a21eac5c620bd`，冻结 36 条 Burning 行、16 条 Swirl 行和 digest `388ad56a6bc0c98c056fddbe29caf39ee4f950d66c291762cfabe86ece904d0a` / `7f18c61188e4ac0493e585e3e3e20cca1262d0b69ad290248dd3b454d538197b`。
- 1.49 新增两份只读 Fixture，不回写 1.48 或更早文件。`legacy-default-120s-1.49.golden.json` = `961505ccb95b536c3563ebeb95ec114f236f3872850df2cb98e5bc8bb5218931`；原生默认预设选择 V2，但总伤 `41410555.13728799`、DPS `345087.9594773999`、269 命中、129 反应命中、3 跳过行动及全部角/技能汇总不变。`burning-reset-boundary-1.49.golden.json` = `3e89c431c3b277fd1dc52881f7ea048b39060e0c16c5230af9c1a73b624e0e10`，冻结 F15/F134/F135 的 V1 允许序列 `true / true / false` 与 V2 `true / false / true`。
- 1.50 新增 `reaction-damage-group-reset-boundary-1.50.golden.json` = `f58cdac88ec2395239fc5f8c4818adff92e563479268ee5c4aa5a75639ae06d1`，不回写 1.49 或更早文件。它冻结 ReactionA/Superconduct 与 ReactionB/Overload 的 F+29 reset-before-attempt 实际模拟结果；纯引擎与递归 Shatter 模拟器测试另行锁定 attempt-before-reset。历史 Fixture 由逐字节完整性门保护。
- 1.51 新增 `basic-reaction-scheduler-1.51.golden.json` = `25cf50a6f39eb9bf4de2d709c896dc74e079493ef2b4e81dfad8d65d17fa4424`，不回写 V150 或更早文件。它冻结 V1 即时附着兼容与 V2 同帧 attack cohort 后按 task order commit 的差异，并锁定两次攻击、两次 `reactionAuraAttachment`、日志、根身份和结果外键；这只是窄切片的回归证据。
- 1.52 新增 `freeze-broken-attack-1.52.golden.json` = `d9a8811a46efb2ed839fac111a4e796d308323f25f3ce0fe7b53c225664f01d4`，不回写 V151 或更早文件。它冻结自然衰减、韧性削冻、碎冰削冻、冰扩散、mixed Hydro/Frozen Swirl、冰结晶六个正向场景，以及 Melt、Superconduct、部分消耗三个负向场景；并锁定 V1 零行、V2 audit-only、第七根和无 DamageEvent/HitResolution/RNG/命中/曲线/DPS 副作用。这只是本地规范化回归证据。
- 1.53 新增严格 V153→V152 projector。只有 callback bus V1、空 callback 注册/交付日志、Freeze Broken 仍为 V1/V2 且所有插件 capability 均为 `damage-modifier` 时才可降级；projector 移除 V1.53 bus 字段、capability 与 subscription 平行数组，恢复 `1.52.0 / 1.52.0-freeze-broken-attack`、Manifest 1.8，并重算哈希后交给冻结 V152 门。V3+V2、`callback-subscriber`、非空 callback 日志、伪造 root 或错误身份必须 fail-closed。现场冻结的 `legacy-default-120s-1.53.golden.json` 字节 SHA-256 为 `617edf8482f3e212d6d78dbead3df484c2665e5169add9279ac2edd26182b45b`，canonical SHA-256 为 `7ecdf28372cb0c7776a103be2fdae621c7eaf9eb32547e683ebd4bc54c2e8d14`，`configHash = fnv1a32:db7a4a8e`、`reproducibilityKey = gdl-v2-fnv1a32-584649fa`，完整战斗投影 digest 为 `c01d9d4100d79cbccbcaf4a2a7a38b7f88d42915832fdc34a8b23880a8380345`。默认 Melt 没有 Frozen 终止转换，因此三组 callback sidecar 均为空；该 Golden 只能证明兼容战斗投影与身份冻结，不能证明 V3 正向触发或完整 gcsim parity。
- 1.51 发布前现场门已完成：基础反应聚合门 22 文件/300 项、完整 unit 139 文件/1679 项、性能 3/3、数据目录、typecheck、build 与 Chromium Playwright 36/36 全部通过。完整 `npm run check` 顺序中的默认/运行时能量/持续 Burning 中位数分别为 29.370ms、5.195ms、93.471ms；墙钟不是跨设备 SLA。当前没有扩展 UI，浏览器验证的是既有指标、角色/技能构成、逐击、时间线与伤害曲线不回退。
- V1.52 本轮现场通过 `npm run test:reactions` 29 个文件/372 项、完整 unit 146 个文件/1752 项、性能 3/3、数据目录 120 角色/125 天赋集/762 技能与被动/237 武器、typecheck、production build 和 Chromium Playwright 36/36。最终 `npm run check` 三项性能中位数为 25.975ms、4.971ms、79.773ms；build 仍有主 JS chunk 超过 500kB 的非阻断警告。默认 120 秒仍为总伤 `41410555.13728799`、DPS `345087.9594773999`、269 命中、129 反应命中和 3 跳过行动；角色、技能、逐段与曲线均不变，`freezeBrokenAttackLog` 为 0 行。
- V1.53 现场发布门已完成：`npm run test:reactions` 为 51 个文件、607/607；完整 unit 为 155 个文件、1858/1858；性能 3/3；数据目录为 120 个角色、125 套天赋、762 个技能与被动、237 把武器；typecheck、production build 与 Chromium Playwright 36/36 全部通过。最终 `npm run check` 中默认、运行时能量、持续 Burning 的中位数分别为 25.985ms、5.504ms、82.953ms；阈值未放宽，墙钟不是跨设备 SLA。构建仍有主 JS chunk 1746.85kB 超过 500kB 的非阻断警告。默认 120 秒保持总伤 `41410555.13728799`、DPS `345087.9594773999`、269 命中、129 反应命中和 3 跳过行动，完整角色/技能/逐击/曲线投影与冻结 V1.52 相同。
...
- Zod 已严格覆盖当前 V153 输入与结果；current `simulationResultSchema`/trusted assertion 在冻结 V152 全部证明上增加第八根、合法模型配对、callback 注册/交付、V3 五阶段和 reciprocal references。`simulationResultV152Schema`/`assertTrustedSimulationResultV152()` 以及所有更早精确 Schema/断言仍保持冻结身份。外部或持久化完整结果必须走完整 Zod；trusted assertion 不是外部 wire 验证边界，subscriber outcome 仍只做结构验证。
...
- 当前基础反应正确性优先于展示扩张。UI 继续只消费核心的敌方逐击、构成、时间线和曲线结构；本轮没有新增 UI 面板。实际伤害仍通过结构化 `DamageEvent` 自然进入现有全队、个人、技能、时间轴、逐段伤害、伤害构成和曲线；V1.53 callback delivery 不是 DamageEvent，不进入这些伤害展示。展示扩展延后，UI 不能自行补算 callback、候选、伤害、Aura 或事件顺序。
- 明确未完成：24 标签/16 类经典反应门之外的 Lunar/月反应；完整特殊/多 Aura 可达空间；官服核验的感电传播；通用 callback/目标任务所有权与全目标 barrier；Mona bubble/Omen、impulse/一般敌人物理、有状态 subscriber、参考 crit RNG 与合成 Freeze Broken DamageEvent；Burning Tick/Swirl 传播之外的反应所有施加；把 55 个公共 application group 与普通直伤组逐技能绑定到全角色；真实粒子/掉球/飞行、完整动作帧/Hitlag、玩家 Aura/敌袭/治疗/死亡/复活、三维命中，以及全角色/武器/圣遗物/敌人的可执行机制数据库。两份 58 组来源表都不等于全角色覆盖。UID 展示柜仍不能可靠编译任意账号为 `SimConfig`，“毕业站位”仍是不可模拟占位。草原核 `300f`、附近传播、固定参考数据和默认杜林示例魔法数均为 provisional。本项目仍未达到 gcsim 精度。
...
- Burning 的 `legacy-event-heap-v1` 有意保留 1.30 兼容相位；1.37 v1 与 1.38 v2 也继续冻结。精确 1.44–1.53 的显式 v3 可在 owner callback 内交付列明的零延迟跨目标伤害/Aura；启用目标时钟时陈旧唤醒会先重投影。1.48 只将实际 Burning Tick 的 target-global observable projection 纳入统一日志，1.49 只给该 channel 与 Swirl 增加显式 V2 重置边界，1.50 的 ReactionA/B 伤害组任务与这套施加状态独立，1.51 的基础反应调度根也只覆盖列明的窄切片。所有这些顺序仅为 `fixed-gcsim-provisional`，不是官服真值、完整 target phase 或完整 gcsim 精度。
...
- V1.52 发布时的 Schema/引擎为 `1.52.0` / `1.52.0-freeze-broken-attack`，Manifest 为 1.8.0；该历史身份只新增 Freeze Broken V1/V2 audit root 和结果证明，现已完整冻结。V1.53 当前身份、callback bus 配对和迁移规则以文件顶部权威 Current State 为准。精确 1.44–1.53 输入均可在各自版本门下 opt-in `target-phase-v3`。`AbilityBlueprint` 当前为 1.9，1.0–1.8 可迁移；编译器可显式传递两类 descriptor/application selector，但不会自动猜测 fixed group 或反应所有施加。
...
npm run check
npm run test:e2e
npm run golden:v153:default:preview --workspace @genshin-dps-lab/test-vectors
# 以下 npx/node 命令保留为历史版本与窄机制的定向验证入口。
...
3. 使用 Zod 校验输入 JSON 并提供字段路径错误。当前精确 1.53 输出由 current `simulationResultSchema` 覆盖全部已声明顶层字段和叶节点；冻结 `simulationResultV152Schema` 及更早精确 Schema 仍只接受各自历史身份。外部或持久化结果必须完整解析，核心内部当前结果运行 current trusted assertion。V145–V152 的公式、Damage Group、application、ReactionA/B、调度与 Freeze Broken root/log 保持 strict；V153 另严格校验 callback bus 模型/root、合法配对、注册/交付、五阶段顺序、subscriber attempt、结构化 outcome 和空 DamageEvent/HitResolution/RNG 副作用。trusted assertion 不能作为外部 wire 的验证边界。
4. 加入 Schema 迁移系统，例如：
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.53 权威 Current State 为起点：保持 Manifest 1.9 八 root、callback bus/V3 结果门、V153→V152 projector、全部历史 Golden、冻结 V152 Manifest 1.8/Freeze Broken audit Golden、V151 Manifest 1.7/基础调度 Golden、V150 Manifest 1.6/ReactionA/B Golden 与全部更早 wire/Fixture，以及核心所有已有日志/引用和 24 标签/16 类经典反应发布门。下一步先加固 callback bus 与 scheduler 在跨目标、同帧 reset、Hitlag、递归碎冰和异常 subscriber 下的证明，再以新版本定义状态权限、Mona bubble/Omen、impulse/物理、明确 RNG 与 DamageEvent 语义；不得把当前 sidecar 直接当作已执行伤害攻击。随后继续 Burning restart/cascade、双 Anemo、混合同帧 Swirl 与更完整 scheduler provenance，再逐技能绑定来源核验后的 Tag/Group。Lunar 反应另立版本实现；基础机制稳定后才处理玩家 Aura/敌袭生存模型、全角色机制映射、UID 配置编译和专用 UI。
