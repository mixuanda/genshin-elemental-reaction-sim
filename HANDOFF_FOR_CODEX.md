...
- 当前输入契约为 `schemaVersion: 1.39.0`、`engineVersion: 1.39.0-shatter-recursive-delivery`；反应核心仍须由 `reactionEngine.mode` 显式选择，目标任务边界由 `targetTaskModel.mode` 显式选择，反应伤害交付顺序另由 `reactionDeliveryModel.mode` 显式选择。1.35→1.36 的 identity-only 迁移、1.36→1.37 的 legacy 注入和 1.37→1.38 的目标任务模式保留继续作为历史契约；1.38→1.39 会补入 `reactionDeliveryModel: { mode: "deferred-event-heap-v1" }`，不会自动启用递归碎冰。历史版本夹带该未来字段会 fail-closed。所有迁移继续保留 `reactionEngine`、`playerDamageModel`、`targetClockModel`、`targetTaskModel`、逐元素敌方抗性和其余配置，不会静默切换 Aura 模式；玩家反应自伤、敌方目标本地时钟、目标任务相位和递归伤害交付继续分别 opt-in。
- `aura-v5` 已把正/反融化、正/反蒸发、超载、超导、感电、冻结/碎冰、扩散、结晶、原激化/超激化/蔓激化、燃烧，以及绽放/草原核/烈绽放/超绽放接入无 DOM 的核心。`aura-v6` 沿用这些主体语义，并对雷元素来袭按固定 gcsim 提交 `b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541` 以共享来袭 Gauge 依次检查 `超激化 → 超载 → 感电 → 冻结底超导 → 普通超导 → 原激化 → 绽放`。`aura-v7` 继承 v6，但把 Quicken 后的水草绽放跟进改为 core zero-delay task：任务继承触发事件的 frame/priority、取得更晚的全局 sequence，因而在已入队的同帧命中后按 FIFO 执行，并在执行点重读实时 Hydro/Quicken Aura。缺失任一状态时任务明确跳过，不使用触发时的陈旧快照伪造 Bloom；它不属于敌方 per-target queue，冻结的 1.37 v1 和 1.38 v2 都不会把它重新分类为 target-owned task。该切片仍是 `fixed-gcsim-provisional`，不能视为官服真值；v1–v6 的历史语义和 Golden 保持不变。
...
- 1.39 的 `reactionDeliveryModel` 提供两种严格模式：所有现有预设、兼容配置和迁移结果继续使用 `deferred-event-heap-v1`；`shatter-recursive-zero-delay-v1` 只允许精确 1.39 身份并要求 `legal-frame-v1 + 60 FPS + aura-v7` 显式 opt-in。递归模式只把零延迟碎冰子伤害在同帧、同目标、同来源上下文中先于直接或嵌套反应父段交付，允许 `parentDamageEventId` 指向稍后编号的父段；它不会同步化其他反应或敌方任务。结果门会验证连续 ID、无环父链、同帧/目标/来源约束，以及每个递归碎冰子段与唯一 `reactionDamageLog` 的双向引用。固定 gcsim 还会建立一个 `DoNotLog` 的零伤害 “Freeze Broken” 合成攻击，本项目尚未实现。
...
- Zod 已严格覆盖输入、`SimulationRunManifest`、Burning/Quicken/Bloom 审计、核心 `reactionTaskLog`、冻结 v1 的 `targetTaskPhaseLog`、v2 的 `targetPhaseLog`、1.39 反应交付父链/碎冰日志引用、ReactionA/B、草原核生命周期/接触/时间线和跨日志引用、玩家命中/自伤/HP/结晶盾吸收引用、目标时钟/Hitlag 日志及回放守恒、`TargetStateTimeline` 等关键输出。v7 的结果边界会把核心任务与触发命中、Quicken mutation、Bloom 审计、草原核预约和目标状态时间线做双向引用校验；两种目标相位日志分别核对自身版本身份、目标注册顺序、目标本地时钟、Aura 连续性及关联日志，并保持互斥。递归碎冰结果边界另拒绝错误身份、非碎冰前向引用、跨帧/跨目标/跨来源父子链、重复/缺失日志和循环引用。1.35 引入的逐元素抗性结果校验继续保留。完整 `SimulationResult` 仍没有覆盖所有字段的单一顶层运行时 Zod Schema。
...
- UI 继续只消费核心的敌方逐击、构成、时间线和曲线结构；本轮先保证反应核心，没有新增玩家自伤/HP/盾破裂或草原核专用面板。现有全队、个人、技能、时间轴、逐段伤害和曲线保持不变；后续 UI 只能读取新增核心结果，不能自行补算伤害。
- 明确未完成：除列明的 v6/v7 代表性来袭链之外的其他来袭元素全组合、一般化 Aura overlap、所有特殊/多 Aura 可达排列、Lunar 反应、完整角色特有 ICD、真实粒子/掉球与飞行、全角色动作/取消帧、从角色/武器/攻击类型自动推导 Hitlag、玩家自身 Hitlag/动作冻结、通用敌方任务暂停与任务所有权、Burning callback 内同步跨目标 Aura/反应命中、固定 gcsim 的 `DoNotLog` “Freeze Broken” 合成攻击、Quicken→Bloom 消耗水 Aura 后的感电 next-tick cleanup、玩家 Aura/敌袭/治疗/死亡/复活、完整三维命中，以及全角色/武器/圣遗物/敌人的可执行机制数据库。感电清理计划进入新的 `aura-v8` / 1.40 边界，不能静默修改 v7。冻结的 `target-phase-v1` 只覆盖第一批 Burning callback→Aura 衰减，1.38 `target-phase-v2` 也只覆盖 callback→该目标 `Reactable.Tick`；两者都不是完整 gcsim target phase。可查询目录只表示 provisional metadata 覆盖，绝不等于完整机制覆盖；UID 展示柜尚不能可靠编译任意账号，默认杜林预设的示例魔法数继续是 `provisional`。本项目仍未达到 gcsim 精度。
...
- v2 可把列明的普通 Aura、Frozen、Quicken、Burning Fuel 和感电共存自然到期纳入该目标 `Reactable.Tick` 日志，但没有建立全目标 barrier；感电 Tick/Wane、Quicken→Bloom、ICD、ReactionA/B、草原核/结晶实体和其他 core work 继续按全局队列运行，通用 target-owned task 仍未实现。
...
- 反应核心优先阶段已推进到 1.39：在既有 Bloom、草原核、烈绽放、超绽放、玩家反应自伤、目标本地 Hitlag、`aura-v6` 雷来袭链、逐元素抗性和 1.36 opt-in `aura-v7` 的 Quicken→Bloom core zero-delay task 之上，1.37 新增并冻结独立 `targetTaskModel`、`target-phase-v1`、第一批 Burning callback/Aura 衰减和 `targetTaskPhaseLog`；1.38 再增加显式 opt-in `target-phase-v2` 与 `targetPhaseLog`，只建模 callback→该目标 `Reactable.Tick`；1.39 新增默认 deferred、可显式递归的碎冰交付边界，并为 v2 Miss 保留 `TARGET_MISS` Hitlag 审计。下一顺序优先修复 Quicken→Bloom 后的 EC next-tick cleanup（新 `aura-v8` / 1.40），再补其他来袭元素、多 Aura 排列、Burning callback 内同步跨目标 Aura/反应命中和通用目标任务所有权；随后再考虑玩家 Aura、敌袭、治疗/死亡等生存机制，`OnBurning` 角色回调和纳西妲 C2 仍后置。
...
- 当前 Schema/引擎为 `1.39.0` / `1.39.0-shatter-recursive-delivery`：除具名目标、逐目标 Aura/ICD/冻结抗性、共享/逐目标八项基础抗性、三层策略、阶段窗口和显式 fanout 外，命中可用二维圆形、可旋转矩形、有限线段胶囊或填充扇形范围自动求交全部圆形碰撞体目标；几何可使用世界坐标，或通过场景静态 `actorPoses` 将施放者局部坐标旋转/平移到世界。目标可沿 60 FPS 声明式线性分段移动，核心按命中帧插值并把坐标空间、施放者姿态、解析后形状、最近距离和阈值写入逐击日志，但目标移动/阶段窗口仍是全局帧，不随 Hitlag 暂停。扩散传播、燃烧 Tick 和草原核爆炸复用逐目标二维求交和阶段策略；结晶碎片与草原核在各自生成帧读取目标位置。玩家反应自伤只使用配置中的静态玩家位置/圆形碰撞体。跨目标命中确认产球仍聚合为一次。冻结的 `target-phase-v1` 只改变第一批 callback/Aura 衰减边界；`target-phase-v2` 也只声明 callback→该目标 `Reactable.Tick`。兼容预设和迁移输入继续选 deferred 反应交付，并原样保留 legacy/v1 目标任务模式。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
...
3. 使用 Zod 校验输入JSON并提供字段路径错误。
   当前还为 `SimulationRunManifest`、Burning/Quicken/Bloom 审计、核心 `reactionTaskLog` 及其到命中/Quicken/草原核/目标时间线的双向引用、冻结 v1 的 `targetTaskPhaseLog`、v2 的 `targetPhaseLog` 及各自目标顺序/目标时钟/Aura 链/关联日志、ReactionA/B 伤害组、草原核生命周期/接触/时间线、玩家伤害/HP/结晶盾引用、目标时钟/Hitlag 日志及回放守恒、`TargetStateTimeline` 等关键输出提供严格 Zod 校验，并以模拟器真实日志做回归。两种目标相位日志按模式互斥；Aura 结果在来源槽存在时还强制 actor 唯一、聚合 Gauge 等于最大槽、逐条 mutation 守恒；无来源槽的历史投影继续兼容。完整 `SimulationResult` 的顶层运行时 Zod Schema 尚未完成，不得把这些关键契约外推为“全部结果均已运行时校验”。
4. 加入 Schema 迁移系统，例如：
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.39 权威 Current State 为起点：先保持冻结的 1.37/1.38 Golden、两种目标相位日志、`runManifest`、核心 `reactionTaskLog` 和 1.39 反应交付引用的一致性与性能门，再以新的 `aura-v8` / 1.40 修复 Quicken→Bloom 后的 EC next-tick cleanup；随后补 Burning callback 内同步跨目标 Aura/反应命中、其他来袭元素的有序反应、完整多 Aura 排列和通用目标任务所有权。感电 Tick/Wane 与 Quicken→Bloom 等 core work 在另立版本契约前继续保持全局。之后才处理玩家 Aura/敌袭生存模型、角色机制映射、UID 配置编译和专用 UI。
