...
- 当前输入契约为 `schemaVersion: 1.46.0`、`engineVersion: 1.46.0-direct-damage-group-root`，当前 `runManifest.version` 为 `1.2.0`；反应核心、目标任务边界、伤害交付顺序、感电目标范围、经典反应公式选择和普通直伤 Damage Group 选择分别由 `reactionEngine.mode`、`targetTaskModel.mode`、`reactionDeliveryModel.mode`、`electroChargedPropagationModel.mode`、`reactionFormulaModel` 与 `directDamageGroupModel` 显式固定。1.45→1.46 迁移只更新运行身份并注入固定 Damage Group 选择，原样保留公式、Aura、传播、目标任务、目标时钟、玩家伤害与反应交付模式；它不会给尚未核验的技能自动分配 Tag/Group，也不改变默认伤害数值或自动启用其他 opt-in 机制。冻结的精确 V145/V144/V142 wire 和 Manifest 1.1.0/1.0.0 保持原语义；精确 1.44、1.45 与当前 1.46 均可在 `legal-frame-v1 + 60 FPS + aura-v7/v8/v9` 门下显式选择 v3。1.43 仍只是尚未发布的 energy wire 保留版本号。历史版本夹带 `directDamageGroupModel`、命中级 `directDamageGroup` 或其他未来字段会 fail-closed。
- `aura-v5` 已把正/反融化、正/反蒸发、超载、超导、感电、冻结/碎冰、火/水/冰/雷扩散、火/水/冰/雷结晶、原激化/超激化/蔓激化、燃烧，以及绽放/草原核/烈绽放/超绽放接入无 DOM 的核心。`aura-v6` 沿用这些主体语义，并对雷元素来袭按固定 gcsim 提交 `b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541` 以共享来袭 Gauge 依次检查 `超激化 → 超载 → 感电 → 冻结底超导 → 普通超导 → 原激化 → 绽放`。`aura-v7` 继承 v6，但把 Quicken 后的水草绽放跟进改为 core zero-delay task：任务继承触发事件的 frame/priority、取得更晚的全局 sequence，因而在已入队的同帧命中后按 FIFO 执行，并在执行点重读实时 Hydro/Quicken Aura。缺失任一状态时任务明确跳过，不使用触发时的陈旧快照伪造 Bloom；它不属于敌方 per-target queue，冻结的 1.37 v1 和 1.38 v2 都不会把它重新分类为 target-owned task。1.40 的 `aura-v8` 继承 v7，并在 follow-up 恰好耗尽同代最后 Hydro 时，把 EC cleanup 延迟到下一次有效目标 Tick。1.42 的 `aura-v9` 再继承 v8 cleanup，并把 F+10 首伤与 F+70 callback 独立排在全局帧：目标 Hitlag 只冻结 `Reactable.Tick` / Aura；F70 缺水雷共存写 `tick-skipped` 并进入 dormant，F70 前恢复可正常结算，F71 后恢复不重启 cadence；非零伤害才维持 `+6f` Wane listener，Wane 对每个 Hydro/Electro 来源槽各扣至多 `0.4U`；流在 cleanup 截止前已终止时以 `ended-before-deadline` 和 reciprocal target-phase / timeline / periodic 引用闭合，旧代 Wane 不得污染替换代次。精确 1.42 历史身份下 v9 只允许 `legal-frame-v1 + 60 FPS + target-phase-v2`；精确 1.44、1.45 与当前 1.46 可在同样的帧率/时间线门下与 v2 或显式 v3 组合。这些经典反应路径仍全部是 `fixed-gcsim-provisional`，不能视为官服真值或完整 gcsim 精度；v1–v8 的历史语义和 Golden 保持不变。
...
- 当前 1.46 完整性门继承并加固 1.45 的基础反应链输入根：`enemyTargets.initialAura` 必须按所选 Aura 耐久版本精确规范化为 simulation-start 状态（v1/v2 `1U -> 0.8U / 426f`，v3–v9 `1U -> 0.8U / 570f`），不能只让初始状态与时间线互相自证。所有转化反应 DamageEvent 的 `sourceActorId / scalingOwnerId / creditOwnerId` 及名称投影必须归属于同一个反应来源，禁止把感电等逐击面板或伤害归属协调转嫁给旁观角色；V146 继续由固定公式 profile 重算反应等级基准与倍率，并为 ordinary direct DamageEvent 追加固定 Damage Group 重放证明。
...
- 每次模拟返回 `runManifest`，固定配置哈希、Schema/引擎/数据版本、解析后的 crit/energy/兼容模式、随机种子、有序插件身份、完整 `reactionFormulaRoot` 与 `directDamageGroupRoot`；复现键使用 `gdl-v2-fnv1a32-*`。当前 configHash/repro key 的 32-bit FNV-1a 已知可碰撞，只是 drift detector，不能作为唯一运行身份或密码学签名；下一 Schema/Manifest 版本应迁移到 SHA-256。声明式插件效果由核心生成内容哈希；任意代码插件的 descriptor/contentHash 仍是插件作者提供的受信声明。
...
- Zod 已严格覆盖输入和当前精确 1.46 `SimulationResult`；当前 `simulationResultV146Schema` 与冻结 `simulationResultV145Schema`、`simulationResultV144Schema`、`simulationResultV142Schema` 分别保持精确身份。V146 继承 1.45 的公式证明、1.44 的 v3 callback/delivery/attempt 和全部既有跨日志完整性门，再新增配置 Damage Group 选择、Manifest root、状态重放、插件后固定序列、DamageEvent 乘区与 reciprocal 外键证明。外部或持久化的完整结果必须走完整 Zod；核心内部当前结果在公开返回前使用 `assertTrustedSimulationResultV146()` 做跨字段断言。冻结版本仍由各自精确 Schema/断言独立接纳；内部可信断言不等同于完整 Zod parse，也不能用于接纳外部或持久化 wire。
...
- 明确未完成：除 24 标签/16 类经典反应发布门和列明的 v6–v9 代表性来袭链之外的其他来袭元素全组合、特殊 Aura overlap 与所有特殊/多 Aura 可达排列、附近传播的官服半径/目标上限/层数/选择规则及副目标流接管/cadence/Wane、Lunar/月反应、完整角色特有 ICD、元素附着的数字 application sequence 与当前 boolean `icdAllowed`/key 对齐（包括纳西妲 `1.5` 等序列值）、把 Tag/Group 批量绑定到全角色技能、真实粒子/掉球与飞行、全角色动作/取消帧、从角色/武器/攻击类型自动推导 Hitlag、玩家自身 Hitlag/动作冻结、Burning 之外的通用敌方 callback 零延迟跨目标交付、通用敌方任务暂停/所有权与全目标 barrier、固定 gcsim 的 `DoNotLog` “Freeze Broken” 合成攻击、玩家 Aura/敌袭/治疗/死亡/复活、完整三维命中，以及全角色/武器/圣遗物/敌人的可执行机制数据库。1.46 的 58 组只覆盖普通直伤 reset/damage sequence，不是 58 组元素附着 ICD。普通 Aura 的多来源槽、元素级共享衰减和逐槽消费已按固定 gcsim 参考实现，但尚未把共享衰减率序列化为结果信任根。v3 只完成已建模 Burning Tick 的局部切片；可查询目录只表示 `provisional + metadata-only` 覆盖，UID 展示柜仍不能可靠编译任意账号为 `SimConfig`，“毕业站位”也仍是不可模拟占位。默认杜林预设的示例魔法数继续是 `provisional`。本项目仍未达到 gcsim 精度。
...
- Burning 的 `legacy-event-heap-v1` 有意保留 1.30 兼容相位：Fuel 于第 121 个目标帧清理，且同一目标帧普通命中先于 Tick；v7 的 refresh 计数修正不会反向改写它。1.37 v1 与 1.38 v2 也继续冻结，其实际 Burning 范围伤害仍在后续全局 `reactionDamage` 处理。精确 1.44、1.45 与当前 1.46 的显式 v3 可在 owner callback 内交付零延迟跨目标伤害/Aura；尚未运行当帧 Tick 的接收目标按 F-1/当前 Aura→F `Reactable.Tick` 结算，已运行 Tick 的目标在其后应用。启用目标时钟时陈旧唤醒会先重投影。所有这些顺序仅为 `fixed-gcsim-provisional`，不是官服真值、完整 gcsim target phase 或完整 gcsim 精度。
...
- 当前 Schema/引擎为 `1.46.0` / `1.46.0-direct-damage-group-root`，Manifest 为 1.2.0。既有具名目标、逐目标 Aura/ICD/抗性、二维几何、目标移动/阶段、目标时钟、EC cleanup 和传播边界保持不变；普通直伤 Damage Group 作为独立状态机与 root 追加。默认预设和迁移输入继续使用 `legacy-event-heap-v1`、`single-target-v1` 并保留既有 Aura/反应交付模式；精确 1.44、1.45 或 1.46 输入均可在版本门下 opt-in `target-phase-v3`。`AbilityBlueprint` 1.8 与通用编译器可选传递 `directDamageGroup`，同时继续保留来源、前置条件和未实现项闸门；既有 Blueprint 不会被自动赋值。
...
3. 使用 Zod 校验输入 JSON 并提供字段路径错误。当前精确 1.46 输出由 `simulationResultV146Schema` 覆盖全部已声明顶层字段和叶节点；冻结 `simulationResultV145Schema` / `simulationResultV144Schema` / `simulationResultV142Schema` 仍只接受各自精确历史身份。外部或持久化结果必须完整解析，核心内部当前结果则运行 `assertTrustedSimulationResultV146()`。v3 delivery/attempt、V145 公式根、V146 Damage Group root/log 与相关外键都必须 strict 校验；内部可信断言不等同于完整 Zod parse，不能作为外部 wire 的验证边界。
4. 加入 Schema 迁移系统，例如：
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.46 权威 Current State 为起点：保持全部历史 Golden、Manifest 1.2 双 root、核心 `reactionTaskLog`、1.39 反应交付引用、1.40 EC cleanup reciprocal 引用、1.41/1.42 传播与 cadence 向量、24 标签/16 类经典反应发布门，以及 1.44–1.46 v3 callback delivery/attempt 完整性契约。下一步优先把元素附着 ICD 的 key 与数字 application sequence 对齐（包括 `1.5` 等值），再补全角色技能的经核验 Tag/Group、其他来袭元素有序反应、特殊/多 Aura 排列和 Burning 之外的通用目标任务所有权；Lunar 反应另立版本实现。基础机制稳定后才处理玩家 Aura/敌袭生存模型、全角色机制映射、UID 配置编译和专用 UI。
