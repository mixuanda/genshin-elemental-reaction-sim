...
- Milestone 5 已建立首个逐技能部分机制闸门：Schema/引擎升级到 `1.3.0` / `1.3.0-icd-profiles`，支持声明式角色特有 ICD Profile；`AbilityBlueprint` 与通用编译器会从固定目录解析倍率、保留证据/前置条件/未实现项，并默认拒绝 `partial`。
- 杜林黑 E 已形成独立的 `provisional + partial` 审计向量：三段倍率、命中帧、DurinSkill ICD、33 固定回能和 4 火粒子均有 Vitest，并在网页逐段伤害、累计伤害曲线、Aura 曲线和能量曲线展示。它不是杜林整角或 120 秒正式队伍预设。
- UID `283733593` 已在 2026-07-26 做真实只读浏览器联调：返回 12 名公开角色，角色、武器和技能达到 0 项未匹配；旅行者元素变体由技能 ID 集合选择。Playwright 仍使用固定响应，避免网络波动进入 CI。
- 冻结的杜林预设仍为 `legacy-v0.1` Golden 兼容配置，手工反应和示例魔法数没有被包装为正式数据。新增黑 E 审计向量不能替代它；精质转变状态、逐后续动作取消、回能/产球 ICD、白 E、爆发、命座和装备尚未完成。
