...
- Milestone 5 已建立首个逐技能部分机制闸门：Schema/引擎升级到 `1.9.0` / `1.9.0-movement-commands`，支持声明式角色特有附着 ICD Profile、按角色/键共享的固定回能 ICD、逐击命中产球及其粒子 ICD、行动状态、显式冲刺/跳跃命令、分后续动作取消帧，以及能量失败后的完整副作用回滚；`AbilityBlueprint` 1.6 与通用编译器会从固定目录解析倍率、保留证据/前置条件/未实现项，并默认拒绝 `partial`。
...
- 冻结的杜林预设仍为 `legacy-v0.1` Golden 兼容配置，手工反应和示例魔法数没有被包装为正式数据。新增黑 E 审计向量不能替代它；当前已映射普攻/重击/战技/爆发/冲刺/跳跃/切人取消，但冲刺/跳跃仍不含耐力和移动物理。Miss/无敌/多目标逐目标命中确认、白 E、爆发、状态驱动的被动、命座和装备尚未完成。
