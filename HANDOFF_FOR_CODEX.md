...
### 2026-07-28 权威 Current State

以下状态优先于后面的历史里程碑描述：

- 当前输入契约为 `schemaVersion: 1.37.0`、`engineVersion: 1.37.0-target-task-phase`；反应核心仍须由 `reactionEngine.mode` 显式选择，目标任务边界另由 `targetTaskModel.mode` 显式选择。1.35→1.36 的 identity-only 迁移继续作为历史契约保留；1.36→1.37 为所有历史输入补入 `targetTaskModel: { mode: "legacy-event-heap-v1" }`，精确保留 `reactionEngine`、`playerDamageModel`、`targetClockModel`、逐元素敌方抗性和其余配置，不会自动启用 `target-phase-v1`，也不会静默切换 Aura 模式。玩家反应自伤、敌方目标本地时钟和新目标任务相位继续分别 opt-in；更早迁移继续遵守各自历史模式和字段的保留/禁用规则。
- `aura-v5` 已把正/反融化、正/反蒸发、超载、超导、感电、冻结/碎冰、扩散、结晶、原激化/超激化/蔓激化、燃烧，以及绽放/草原核/烈绽放/超绽放接入无 DOM 的核心。`aura-v6` 沿用这些主体语义，并对雷元素来袭按固定 gcsim 提交 `b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541` 以共享来袭 Gauge 依次检查 `超激化 → 超载 → 感电 → 冻结底超导 → 普通超导 → 原激化 → 绽放`。`aura-v7` 继承 v6，但把 Quicken 后的水草绽放跟进改为 core zero-delay task：任务继承触发事件的 frame/priority、取得更晚的全局 sequence，因而在已入队的同帧命中后按 FIFO 执行，并在执行点重读实时 Hydro/Quicken Aura。缺失任一状态时任务明确跳过，不使用触发时的陈旧快照伪造 Bloom；它不属于敌方 per-target queue，1.37 不会把它重新分类为 target-owned task。该切片仍是 `fixed-gcsim-provisional`，不能视为官服真值；v1–v6 的历史语义和 Golden 保持不变。
...
- 1.33 引入且 1.34–1.37 保留的目标时钟只在 `legal-frame-v1` 下启用。每个命中可显式声明 `targetHitlag: { haltFrames, factor }`；Schema 要求 `0 <= haltFrames <= 600` 且 `0 <= factor <= 1`。扩展帧采用 `ceil(ceil(haltFrames) × (1 - factor))`，命中所在目标帧先完成，暂停从下一全局帧开始。同目标可叠加、不同目标隔离；Miss 不应用，landed 但伤害免疫仍可应用，零扩展只记录审计。严格结果 Schema 会逐点重放目标时间线、禁止同一命中重复产生 Hitlag，并精确核对超导状态的累计延长。普通 Aura、Frozen、Quicken、Burning Fuel/Tick 和感电共存自然到期使用目标本地截止帧；已存在的超导减物抗状态会延长。`target-phase-v1` 中陈旧的 Burning 唤醒必须先按目标时钟重投影，不能在原全局帧提前衰减或执行。
- 感电的 `+10/+60` 伤害 Tick 与 `+6` Wane、附着 ICD、core reaction task/GCD、ReactionA/B、独立反应伤害、草原核/结晶实体、行动/Buff/能量/粒子、目标运动/阶段和玩家侧状态仍按全局帧运行。草原核日志明确标记 `global-frame-gadget-v1` / `not-affected-by-enemy-hitlag`；不能把 1.37 的第一批 target phase 描述为通用敌方任务暂停。
- 1.33–1.36 的兼容 Golden 身份与 SHA-256 继续作为历史证据保留；`legacy-default-120s-1.36.golden.json` 和 `quicken-bloom-task-order-1.36.golden.json` 均不被改写。`legacy-default-120s-1.37.golden.json` 冻结当前 1.37 兼容身份和原始浮点基线，1.37 的 Quicken→Bloom Fixture 冻结规范化配置身份而不改写 v7 核心任务语义；`reaction-matrix-1.35.golden.json` 的 17 个向量继续作为当前反应矩阵的语义 Golden 复用。历史 Fixture 不被重写；版本字段、配置哈希和复现键按各自身份合法更新。
...
- Zod 已严格覆盖输入、`SimulationRunManifest`、Burning/Quicken/Bloom 审计、核心 `reactionTaskLog`、`targetTaskPhaseLog`、ReactionA/B、草原核生命周期/接触/时间线和跨日志引用、玩家命中/自伤/HP/结晶盾吸收引用、目标时钟/Hitlag 日志及回放守恒、`TargetStateTimeline` 等关键输出。v7 的结果边界会把核心任务与触发命中、Quicken mutation、Bloom 审计、草原核预约和目标状态时间线做双向引用校验；1.37 的目标相位边界另核对目标注册顺序、目标本地时钟、任务前/后/衰减后 Aura 连续性及关联日志。1.35 引入的逐元素抗性结果校验继续保留。完整 `SimulationResult` 仍没有覆盖所有字段的单一顶层运行时 Zod Schema。
...
- 明确未完成：除列明的 v6/v7 代表性来袭链之外的其他来袭元素全组合、一般化 Aura overlap、所有特殊/多 Aura 可达排列、Lunar 反应、完整角色特有 ICD、真实粒子/掉球与飞行、全角色动作/取消帧、从角色/武器/攻击类型自动推导 Hitlag、玩家自身 Hitlag/动作冻结、通用敌方任务暂停与任务所有权、玩家 Aura/敌袭/治疗/死亡/复活、完整三维命中，以及全角色/武器/圣遗物/敌人的可执行机制数据库。1.37 的 `target-phase-v1` 目前只覆盖第一批 Burning target callback，不是完整 gcsim target phase。可查询目录只表示 provisional metadata 覆盖，绝不等于完整机制覆盖；UID 展示柜尚不能可靠编译任意账号，默认杜林预设的示例魔法数继续是 `provisional`。本项目仍未达到 gcsim 精度。
- 玩家八项抗性是用户显式输入和本项目的公式约定；固定 gcsim 提交用于列明路径的交叉参考，但没有提供可直接当作官服正式角色抗性数据库的真值。测试向量、草原核寿命和杜林示例数值都不得包装为正式官服数据或完整 gcsim 精度。
- Burning 的 `legacy-event-heap-v1` 有意保留 1.30 兼容相位：Fuel 于第 121 个目标帧清理，且同一目标帧普通命中先于 Tick；v7 的 refresh 计数修正不会反向改写它。1.37 的 `target-phase-v1` 必须显式 opt-in，目前只覆盖第一批 Burning target callback：callback 在该目标 Aura 衰减前检查 Marker/Fuel，按 `enemy.targets` 注册顺序执行，全部目标完成后才处理同帧 core/incoming work；启用目标时钟时陈旧唤醒会先重投影。该顺序仅为 `fixed-gcsim-provisional`，不是官服真值、完整 gcsim target phase 或完整 gcsim 精度。
- 当较早目标的 Frozen、Quicken 或 Burning Fuel 专用到期检查与较晚目标的 Burning callback 同帧时，数值状态与伤害保持确定且不会伪造 callback，但专用生命周期日志仍由全局事件堆处理，可能排在较晚目标 callback 之后；通用 target-owned task / special-Aura decay phase 尚未实现。
...
- 反应核心优先阶段已完成到 1.37：在既有 Bloom、草原核、烈绽放、超绽放、玩家反应自伤、目标本地 Hitlag、`aura-v6` 雷来袭链、逐元素抗性和 1.36 opt-in `aura-v7` 的 Quicken→Bloom core zero-delay task 之上，新增独立 `targetTaskModel`、显式 opt-in 的 `target-phase-v1`、第一批 Burning target callback/Aura 衰减边界、Hitlag 唤醒重投影和严格 `targetTaskPhaseLog`。下一顺序应继续补其他来袭元素、多 Aura 排列并从第一批切片扩展通用目标任务所有权；随后再考虑玩家 Aura、敌袭、治疗/死亡等生存机制，`OnBurning` 角色回调和纳西妲 C2 仍后置。
...
- 当前 Schema/引擎为 `1.37.0` / `1.37.0-target-task-phase`：除具名目标、逐目标 Aura/ICD/冻结抗性、共享/逐目标八项基础抗性、三层策略、阶段窗口和显式 fanout 外，命中可用二维圆形、可旋转矩形、有限线段胶囊或填充扇形范围自动求交全部圆形碰撞体目标；几何可使用世界坐标，或通过场景静态 `actorPoses` 将施放者局部坐标旋转/平移到世界。目标可沿 60 FPS 声明式线性分段移动，核心按命中帧插值并把坐标空间、施放者姿态、解析后形状、最近距离和阈值写入逐击日志，但目标移动/阶段窗口仍是全局帧，不随 Hitlag 暂停。扩散传播、燃烧 Tick 和草原核爆炸复用逐目标二维求交和阶段策略；结晶碎片与草原核在各自生成帧读取目标位置。玩家反应自伤只使用配置中的静态玩家位置/圆形碰撞体。跨目标命中确认产球仍聚合为一次。`target-phase-v1` 只改变明确列出的第一批 target-owned callback 边界，历史/预设继续默认 legacy。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
...
- 当前事件优先级：
  ...
  4. `hit`
  5. `reactionDamage`
- 1.37 的 `target-phase-v1` 在 `action` 后、同帧 buff/energy/hit/core reaction damage 前注册目标任务优先级，并按目标注册顺序处理第一批 Burning target callback 与该目标 Aura 衰减；`legacy-event-heap-v1` 保持既有优先级和 Golden。`aura-v7` 的 `quickenBloomFollowup` 仍是 core zero-delay task：继承触发事件的 frame/priority，并取得更晚的全局 sequence，使已入队的同帧同优先级事件按 FIFO 先执行；它不属于 target-owned queue。继续扩展通用目标任务阶段时仍须另升引擎版本，不能静默重排 v1–v7 或 legacy 配置。
...
技能基线保存在 `packages/test-vectors/fixtures/legacy-default-120s.golden.json`。1.30–1.36 的历史升级继续按原契约保留；1.36 新增显式 opt-in `aura-v7` 和核心反应任务日志，1.37 再新增独立 `targetTaskModel` 与目标相位日志。1.35→1.36 是 identity-only 迁移，必须保留 v6、玩家模型、目标时钟和逐元素抗性，绝不能自动启用 v7；1.36→1.37 必须为历史输入补入 `legacy-event-heap-v1`，绝不能自动启用 target phase。`legacy-default-120s-1.36.golden.json` 与 `quicken-bloom-task-order-1.36.golden.json` 继续锁定冻结的历史身份；对应 1.37 Fixture 锁定当前规范化身份和不变语义。`reaction-matrix-1.35.golden.json` 的 17 个向量继续作为当前反应矩阵的语义回归证据。Golden 相等只证明迁移兼容和当前结构化结果没有漂移，不证明测试抗性、杜林预设中的手工反应、装备系数或其他示例魔法数是正式游戏数据，它们必须继续标记 `provisional`。

1.36 继续保留独立的 `QUICKEN_BLOOM_TASK_*` 历史版本常量，专用 Golden 不再依赖未来会变化的 `CURRENT_*` 名称。基础反应发布门由 `aura-v7-order-release.test.ts` 和 `aura-v7-public-grid.test.ts` 补强：前者锁定七种来袭元素的代表性多 Aura 顺序、Frozen 火蒸发 guard、F150 ICD 与 F426 小元素量到期；后者覆盖全部 `6^5` 公开初始普通 Aura 赋值，并以 62,208 个 covering 向量、186,624 次独立执行检查有限性、非负性、来源槽与消费守恒、重放确定性和输入数组换序。1.37 另由 `target-task-phase.test.ts` 与 `target-task-phase-log.test.ts` 锁定低 Fuel 的 decay 边界、Hitlag 唤醒重投影、多目标注册顺序和结构化相位日志。两类门禁都不等于特殊 Aura 全排列、完整 target task system 或完整 gcsim parity。

当前验证命令：

```bash
npm run typecheck
npx vitest run packages/schemas/src/schema.test.ts packages/sim-core/src/__tests__/formulas.test.ts packages/sim-core/src/__tests__/amplifying.test.ts packages/sim-core/src/__tests__/reaction-a.test.ts packages/sim-core/src/__tests__/reaction-b.test.ts packages/sim-core/src/__tests__/bloom-gauge.test.ts packages/sim-core/src/__tests__/bloom-aura.test.ts packages/sim-core/src/__tests__/bloom-integration.test.ts packages/sim-core/src/__tests__/dendro-core.test.ts packages/sim-core/src/__tests__/aura-v6-electro.test.ts packages/sim-core/src/__tests__/aura-v6-simulator.test.ts packages/sim-core/src/__tests__/hydro-order.test.ts packages/sim-core/src/__tests__/quicken-bloom-task-order.test.ts packages/sim-core/src/__tests__/burning.test.ts packages/sim-core/src/__tests__/burning-v7-refresh.test.ts packages/sim-core/src/__tests__/target-task-phase.test.ts packages/sim-core/src/__tests__/target-task-phase-log.test.ts packages/sim-core/src/__tests__/enemy-elemental-resistance.test.ts packages/sim-core/src/__tests__/crystallize.test.ts packages/sim-core/src/__tests__/player-damage.test.ts packages/sim-core/src/__tests__/player-reaction-damage.test.ts packages/sim-core/src/__tests__/target-clock.test.ts packages/sim-core/src/__tests__/aura-target-clock.test.ts packages/sim-core/src/__tests__/target-clock-integration.test.ts packages/sim-core/src/__tests__/target-hitlag-status.test.ts packages/sim-core/src/__tests__/reaction-matrix-golden.test.ts packages/sim-core/src/__tests__/golden.test.ts packages/sim-core/src/__tests__/performance.test.ts
...
3. 使用 Zod 校验输入JSON并提供字段路径错误。
   当前还为 `SimulationRunManifest`、Burning/Quicken/Bloom 审计、核心 `reactionTaskLog` 及其到命中/Quicken/草原核/目标时间线的双向引用、`targetTaskPhaseLog` 及其目标顺序/目标时钟/Aura 链/关联日志、ReactionA/B 伤害组、草原核生命周期/接触/时间线、玩家伤害/HP/结晶盾引用、目标时钟/Hitlag 日志及回放守恒、`TargetStateTimeline` 等关键输出提供严格 Zod 校验，并以模拟器真实日志做回归。Aura 结果在来源槽存在时还强制 actor 唯一、聚合 Gauge 等于最大槽、逐条 mutation 守恒；无来源槽的历史投影继续兼容。完整 `SimulationResult` 的顶层运行时 Zod Schema 尚未完成，不得把这些关键契约外推为“全部结果均已运行时校验”。
4. 加入 Schema 迁移系统，例如：
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.37 权威 Current State 为起点：先保持全部历史/当前 Golden、`runManifest`、核心 `reactionTaskLog`、`targetTaskPhaseLog` 跨日志一致性和性能门，再扩展其他来袭元素的有序反应、完整多 Aura 排列与第一批目标任务相位之外的通用任务所有权，之后才处理玩家 Aura/敌袭生存模型、角色机制映射、UID 配置编译和专用 UI。
