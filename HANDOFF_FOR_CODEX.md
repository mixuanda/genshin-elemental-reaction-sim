...
### 2026-07-31 权威 Current State

以下状态优先于后面的历史里程碑描述：

- 当前输入契约为 `schemaVersion: 1.42.0`、`engineVersion: 1.42.0-ec-global-cadence-safety`；反应核心、目标任务边界、伤害交付顺序和感电目标范围仍分别由 `reactionEngine.mode`、`targetTaskModel.mode`、`reactionDeliveryModel.mode` 与 `electroChargedPropagationModel.mode` 显式选择。1.41 的传播模型继续保留；1.41→1.42 迁移只更新运行身份，原样保留 `aura-v8` 和 `{ mode: "single-target-v1" }`，绝不自动启用 `aura-v9` 或附近湿目标传播。只有精确 1.42、`legal-frame-v1 + 60 FPS + target-phase-v2` 输入才能显式选择 `aura-v9`。历史版本夹带未来字段或未来 Aura 模式会 fail-closed。
- `aura-v5` 已把正/反融化、正/反蒸发、超载、超导、感电、冻结/碎冰、火/水/冰/雷扩散、火/水/冰/雷结晶、原激化/超激化/蔓激化、燃烧，以及绽放/草原核/烈绽放/超绽放接入无 DOM 的核心。`aura-v6` 沿用这些主体语义，并对雷元素来袭按固定 gcsim 提交 `b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541` 以共享来袭 Gauge 依次检查 `超激化 → 超载 → 感电 → 冻结底超导 → 普通超导 → 原激化 → 绽放`。`aura-v7` 继承 v6，但把 Quicken 后的水草绽放跟进改为 core zero-delay task：任务继承触发事件的 frame/priority、取得更晚的全局 sequence，因而在已入队的同帧命中后按 FIFO 执行，并在执行点重读实时 Hydro/Quicken Aura。缺失任一状态时任务明确跳过，不使用触发时的陈旧快照伪造 Bloom；它不属于敌方 per-target queue，冻结的 1.37 v1 和 1.38 v2 都不会把它重新分类为 target-owned task。1.40 的 `aura-v8` 继承 v7，并在 follow-up 恰好耗尽同代最后 Hydro 时，把 EC cleanup 延迟到下一次有效目标 Tick。1.42 的 `aura-v9` 再继承 v8 cleanup，并把 F+10 首伤与 F+70 callback 独立排在全局帧：目标 Hitlag 只冻结 `Reactable.Tick` / Aura；F70 缺水雷共存写 `tick-skipped` 并进入 dormant，F70 前恢复可正常结算，F71 后恢复不重启 cadence；非零伤害才维持 `+6f` Wane listener，Wane 对每个 Hydro/Electro 来源槽各扣至多 `0.4U`；流在 cleanup 截止前已终止时以 `ended-before-deadline` 和 reciprocal target-phase / timeline / periodic 引用闭合，旧代 Wane 不得污染替换代次。精确 v9 只允许 `legal-frame-v1 + 60 FPS + target-phase-v2`。这些经典反应路径仍全部是 `fixed-gcsim-provisional`，不能视为官服真值或完整 gcsim 精度；v1–v8 的历史语义和 Golden 保持不变。
...
- Zod 已严格覆盖输入和精确 1.42 `SimulationResult` 的全部 65 个顶层字段；`simulationResultV142Schema` 进一步严格解析逐击、ReactionAudit、状态/实体生命周期、行动、能量/粒子、汇总、曲线、Aura 边界与合法时间线，并闭合 result/config/manifest 身份、别名、ID/排序、个位显示、伤害构成、总伤/DPS/各类汇总和主要反链。既有 `SimulationRunManifest`、Burning/Quicken/Bloom、ReactionA/B、草原核、玩家伤害、目标时钟/Hitlag、反应交付、EC cleanup/传播/cadence 与 `TargetStateTimeline` 深层 facet 继续保留。外部或持久化的完整结果必须走完整 Zod；核心内部结果在公开返回前使用零拷贝可信断言复用跨字段完整性规则，避免全量 parse 深拷贝持续反应日志。可信断言只适用于内部已构造对象，不等同于完整 Zod parse，也不能用于接纳外部或持久化 wire。v9 结果门仍必须按 EC 所有权过滤周期记录，既允许无 EC 的 Melt/Overload 等 1.42 结果，也严格验证纯 EC 和混合 EC + 非 EC 的反链。
...
- UI 继续只消费核心的敌方逐击、构成、时间线和曲线结构；本轮先保证基础反应核心，不新增附近传播、EC cleanup、玩家自伤/HP/盾破裂或草原核专用面板。传播产生的每条伤害应通过结构化 `DamageEvent` 自然进入现有全队、个人、技能、时间轴、逐段伤害和曲线；展示扩展延后，UI 不能自行补算候选、伤害或 Aura。
- 明确未完成：除 24 标签/16 类经典反应发布门和列明的 v6–v9 代表性来袭链之外的其他来袭元素全组合、一般化 Aura overlap、所有特殊/多 Aura 可达排列、附近传播的官服半径/目标上限/层数/选择规则及副目标流接管/cadence/Wane、Lunar/月反应、完整角色特有 ICD、真实粒子/掉球与飞行、全角色动作/取消帧、从角色/武器/攻击类型自动推导 Hitlag、玩家自身 Hitlag/动作冻结、通用敌方任务暂停与任务所有权、Burning callback 内同步跨目标 Aura/反应命中、固定 gcsim 的 `DoNotLog` “Freeze Broken” 合成攻击、玩家 Aura/敌袭/治疗/死亡/复活、完整三维命中，以及全角色/武器/圣遗物/敌人的可执行机制数据库。跨过 F70 callback 的传统感电长目标 Hitlag 已由 v9 覆盖，不再列为缺口；可查询目录只表示 `provisional + metadata-only` 覆盖，UID 展示柜仍不能可靠编译任意账号为 `SimConfig`，“毕业站位”也仍是不可模拟占位。默认杜林预设的示例魔法数继续是 `provisional`。本项目仍未达到 gcsim 精度。
...
- 反应核心优先阶段已推进到 1.42：1.40 的 `aura-v8` 已冻结 Quicken→Bloom 后 EC next-target-Tick cleanup；1.41 再以独立、默认关闭的 `nearby-wet-radius-v1` 实现附近湿目标逐目标伤害传播和候选审计；1.42 `aura-v9` 则冻结传统感电 F+10/F+70 全局 cadence、长目标 Hitlag、恢复边界、dormant、Wane listener/逐来源削减和提前终止 audit，并加入 24 标签/16 类经典反应发布门。当前顺序是继续补其他来袭元素与多 Aura 排列，再实现 Burning callback 内同步跨目标 Aura/反应命中和通用目标任务所有权；随后才考虑 Lunar、玩家 Aura、敌袭、治疗/死亡等生存机制和新增展示。
...
- 1.42 结果完整性继续采用兼容回放加固：配置行动、命中确认、粒子生成、逐角色结算、能量汇总和曲线必须形成完整且唯一的投影，公共 Zod 边界与 trusted 边界都要拒绝协同删改。冻结的 V142 输入 Schema 仍允许历史负数回能声明，但新执行会在运行入口 fail-closed；1.43 才会正式收窄输入域并加入 `particleEventId`、行动/曲线反链和显式同帧序号。
...
- 当前 Schema/引擎为 `1.42.0` / `1.42.0-ec-global-cadence-safety`。既有具名目标、逐目标 Aura/ICD/抗性、二维几何、目标移动/阶段、目标时钟、EC cleanup 和传播边界保持不变；传播半径只在 `nearby-wet-radius-v1` 的 Tick 交付点读取源/目标位置与圆形 hurtbox。默认预设和迁移输入继续使用 `single-target-v1` 并保留既有 `aura-v8`/目标任务/反应交付模式，只有精确 1.42 输入才能 opt-in `aura-v9`。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
...
- 当前事件优先级：
  ...
  4. `hit`
  5. `reactionDamage`
- 冻结的 1.37 `target-phase-v1` 在 `action` 后、同帧 buff/energy/hit/core reaction damage 前注册目标任务优先级，并按目标注册顺序处理第一批 Burning target callback 与该目标 Aura 衰减。1.38 `target-phase-v2` 只把每个目标 callback→同一目标 `Reactable.Tick` 固定在该边界，不建立全目标 barrier，也不把感电 `+10/+60` 伤害 Tick、`+6` Wane、ICD、ReactionA/B 或其他 core work 搬入目标相位。`legacy-event-heap-v1` 保持既有优先级和 Golden；`aura-v7/v8/v9` 的 `quickenBloomFollowup` 仍是继承触发 frame/priority、取得更晚全局 sequence 的 core zero-delay task，在 v1/v2 中都不属于 target-owned queue，v8/v9 只把 depletion cleanup 决议放到下一目标 Tick。v9 的 F+10/F+70 callback 与 Wane 始终属于全局队列，Hitlag 只延后目标 `Reactable.Tick` / Aura。继续扩展通用目标任务阶段时仍须另升引擎版本，不能静默重排 v1–v9 或历史目标任务模式。
...
技能基线保存在 `packages/test-vectors/fixtures/legacy-default-120s.golden.json`。1.30–1.41 的历史升级继续按原契约保留，任何后续工作都不得覆盖其 Fixture。1.40→1.41 必须保留 Aura、目标任务、目标时钟与伤害交付选择并补入 `single-target-v1`；1.41→1.42 再只更新身份并保留 `aura-v8 + single-target-v1`，两步都绝不能自动启用 nearby propagation 或 `aura-v9`。1.42 默认数值必须继续与 Vanilla 浮点基线一致，传播和 v9 cadence 使用各自独立 Fixture。Golden 相等只证明迁移兼容和对应结构化结果没有漂移，不证明测试抗性、传播半径、杜林预设中的手工反应、装备系数、固定 gcsim 提交或其他示例魔法数是官方真值；这些边界继续标记 `provisional`、`community-provisional` 或 `fixed-gcsim-provisional`。

1.36–1.41 的历史门继续按各自固定版本常量运行。1.41 的 `electro-charged-propagation.test.ts` 和独立传播 Golden 负责湿/干/范围外/位置未解析/免疫、hurtbox 边界、逐元素抗性、目标顺序、源快照复用、副目标流不接管/不重置、不递归/无副目标 Wane 和确定性。1.42 的 `aura-v9-ec-global-cadence.test.ts` 与独立 cadence Golden 负责 F+10/F+70 独立全局排程、长 Hitlag、F70 missing coexistence、F20/F69/F70/F71 恢复边界、Wane listener、每来源槽 `-0.4U`、旧代隔离和 `ended-before-deadline` reciprocal 审计；`reaction-matrix-golden.test.ts` 另锁定 24 标签/16 类经典反应。前者仍只验证 `fixed-gcsim-provisional`，1.41 传播门仍只验证 `community-provisional`；都不证明官服完整语义或 gcsim parity。未实际执行对应命令前不得写入通过结论、SHA 或测试计数。
...
3. 使用 Zod 校验输入 JSON 并提供字段路径错误。精确 1.42 输出由 `simulationResultV142Schema` 覆盖全部 65 个顶层字段和所有已声明叶节点；外部或持久化结果必须完整解析，核心内部则在已有深层 facet 之后运行零拷贝跨字段断言。内部可信断言不等同于完整 Zod parse，不能作为外部 wire 的验证边界。两种目标相位日志按模式互斥；Aura 结果在来源槽存在时还强制 actor 唯一、聚合 Gauge 等于最大槽、逐条 mutation 守恒；无来源槽的历史投影继续兼容。历史完整结果不会被当前 Schema 宽松迁移，只有精确 1.42 结果可通过。
4. 加入 Schema 迁移系统，例如：
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.42 权威 Current State 为起点：保持全部历史 Golden、两种目标相位日志、`runManifest`、核心 `reactionTaskLog`、1.39 反应交付引用、1.40 EC cleanup reciprocal 引用、两份 1.41 Fixture，以及 1.42 `aura-v9` cadence / Wane / ended-before-deadline 审计和 24 标签/16 类经典反应发布门。随后补其他来袭元素的有序反应、完整特殊/多 Aura 排列、Burning callback 内同步跨目标 Aura/反应命中和通用目标任务所有权；Lunar 反应另立版本实现。基础机制稳定后才处理玩家 Aura/敌袭生存模型、角色机制映射、UID 配置编译和专用 UI。
