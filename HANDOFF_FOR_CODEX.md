...
- 当前开发优先级是基础经典反应、Aura 生命周期和结果完整性，不是展示扩张。1.44 的 Burning 结果门现在反向要求 start/stop 审计，重放 generation、首 Tick、15f cadence、tickIndex、Fuel 自然到期、callback、反应伤害与时间线反链，并把 Marker/Fuel 标量和 Fuel-expire 的 Aura 消费扎回权威快照；普通 stop 必须有精确 audit、callback、直接 Aura 消费、目标相位衰减或显式机制截断所有者。仍未从输入配置完整重放每个 Burning Gauge、全部特殊 Aura 和所有可达反应链，因此不能把这些门称为完整 Aura 证明。
- v3 结果完整性不允许 motion/phase/delivery 日志互相自证：注册目标、名称、初始位置、圆形 hurtbox、`targetMotions`、`targetPhases` 和三层目标策略从 `SimConfig` 重放；callback 截止、Burning 圆心/半径/`1U` 火附着，以及每个 attempt 的 outcome、距离、阈值、reason、`damageAllowed / auraAllowed / hitConfirmAllowed`、伤害倍率和 mechanics status 都必须与配置根一致。`aura-v2/v3` 遇到 Pyro 先 Overload、剩余 Gauge 仍可对 Frozen Melt 的未实现链时，以 `legacy-multi-reaction-order` / `TARGET_MECHANICS_TRUNCATION` fail-closed；这不是该链已实现。
...
- 本轮全量 Vitest 已实际运行并通过 `82` 个测试文件、`1122/1122` 项测试；该记录不代表 build 或 Playwright 已完成，两者结果必须以后续现场命令为准。
...
- Zod 已严格覆盖输入和精确 1.44 `SimulationResult` 的全部 65 个顶层字段；当前 `simulationResultV144Schema` 与冻结 `simulationResultV142Schema` 分别保持精确身份。1.44 在既有逐击、ReactionAudit、状态/实体生命周期、行动、能量/粒子、汇总、曲线、Aura 边界和合法时间线闭合之上，新增 v3 callback owner、delivery 事件元组、注册顺序 attempts、`before/after-reactable-tick`、`reactionDamageLog`、命中、伤害与 `TargetStateTimeline` 的 reciprocal 完整性门。本轮继续把 Burning 生命周期、v3 几何/策略和 target-clock 截止扎回输入、事件或已审计时钟根；public 与 trusted 两条边界复用同一组跨字段不变量。外部或持久化的完整结果必须走完整 Zod；核心内部结果在公开返回前使用 `assertTrustedSimulationResultV144()` 做零拷贝跨字段断言。该可信断言只适用于内部已构造对象，不等同于完整 Zod parse，也不能用于接纳外部或持久化 wire。
...
- 明确未完成：除 24 标签/16 类经典反应发布门和列明的 v6–v9 代表性来袭链之外的其他来袭元素全组合、一般化 Aura overlap、所有特殊/多 Aura 可达排列、附近传播的官服半径/目标上限/层数/选择规则及副目标流接管/cadence/Wane、Lunar/月反应、完整角色特有 ICD、真实粒子/掉球与飞行、全角色动作/取消帧、从角色/武器/攻击类型自动推导 Hitlag、玩家自身 Hitlag/动作冻结、Burning 之外的通用敌方 callback 零延迟跨目标交付、通用敌方任务暂停/所有权与全目标 barrier、固定 gcsim 的 `DoNotLog` “Freeze Broken” 合成攻击、玩家 Aura/敌袭/治疗/死亡/复活、完整三维命中，以及全角色/武器/圣遗物/敌人的可执行机制数据库。v3 只完成已建模 Burning Tick 的局部切片；可查询目录只表示 `provisional + metadata-only` 覆盖，UID 展示柜仍不能可靠编译任意账号为 `SimConfig`，“毕业站位”也仍是不可模拟占位。默认杜林预设的示例魔法数继续是 `provisional`。本项目仍未达到 gcsim 精度。
- 玩家八项抗性是用户显式输入和本项目的公式约定；固定 gcsim 提交用于列明路径的交叉参考，但没有提供可直接当作官服正式角色抗性数据库的真值。测试向量、草原核寿命和杜林示例数值都不得包装为正式官服数据或完整 gcsim 精度。
- Burning 的 `legacy-event-heap-v1` 有意保留 1.30 兼容相位：Fuel 于第 121 个目标帧清理，且同一目标帧普通命中先于 Tick；v7 的 refresh 计数修正不会反向改写它。1.37 v1 与 1.38 v2 也继续冻结，其实际 Burning 范围伤害仍在后续全局 `reactionDamage` 处理。只有精确 1.44 的显式 v3 才在 owner callback 内交付零延迟跨目标伤害/Aura；尚未运行当帧 Tick 的接收目标按 F-1/当前 Aura→F `Reactable.Tick` 结算，已运行 Tick 的目标在其后应用。启用目标时钟时陈旧唤醒会先重投影。所有这些顺序仅为 `fixed-gcsim-provisional`，不是官服真值、完整 gcsim target phase 或完整 gcsim 精度。
...
- 当前 Schema/引擎为 `1.44.0` / `1.44.0-burning-callback-delivery`。既有具名目标、逐目标 Aura/ICD/抗性、二维几何、目标移动/阶段、目标时钟、EC cleanup 和传播边界保持不变。默认预设和迁移输入继续使用 `legacy-event-heap-v1`、`single-target-v1` 并保留既有 Aura/反应交付模式；只有精确 1.44 输入才能 opt-in `target-phase-v3`。`AbilityBlueprint` 1.7 与通用编译器继续保留来源、前置条件和未实现项闸门。
...
技能基线保存在 `packages/test-vectors/fixtures/legacy-default-120s.golden.json`。1.30–1.42 的历史升级继续按原契约保留，任何后续工作都不得覆盖其 Fixture。1.42→1.44 只更新身份并保留历史 Aura、`legacy-event-heap-v1`、`single-target-v1`、目标时钟与反应交付选择，绝不自动启用 `target-phase-v3`、nearby propagation 或 `aura-v9`。1.44 默认数值必须继续与 Vanilla/1.42 浮点基线一致；兼容 Fixture 的 SHA-256 为 `e0c2e1475ec97b35bd0ee7bb1bf6b3bc0e505588e1ea76001b8011216d475d05`，`configHash = fnv1a32:dad42c01`，`reproducibilityKey = gdl-v2-fnv1a32-03487d7e`。1.42 的默认/cadence Fixture SHA 继续是 `ccb4bd071cbd5643f4a59dc41273801dd6e76a778bc876ea3ed6ab23266425df` 与 `ed7a41b1bc67adb1908367172db2bcecd0e668dbdd9f214f14829adbb3375611`，不得回写。Golden 相等只证明迁移兼容和对应结构化结果没有漂移，不证明测试抗性、传播半径、杜林预设中的手工反应、装备系数、固定 gcsim 提交或其他示例魔法数是官方真值；这些边界继续标记 `provisional`、`community-provisional` 或 `fixed-gcsim-provisional`。
...
3. 使用 Zod 校验输入 JSON 并提供字段路径错误。精确 1.44 输出由 `simulationResultV144Schema` 覆盖全部 65 个顶层字段和所有已声明叶节点；冻结 `simulationResultV142Schema` 仍只接受精确 1.42 身份。外部或持久化结果必须完整解析，核心内部则运行 `assertTrustedSimulationResultV144()`。v3 的 delivery/attempt 判别联合、事件元组、注册顺序、应用相位和跨日志 reciprocal 外键都必须 strict 校验；内部可信断言不等同于完整 Zod parse，不能作为外部 wire 的验证边界。历史完整结果不会被当前 Schema 宽松迁移。
4. 加入 Schema 迁移系统，例如：
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.44 权威 Current State 为起点：保持全部历史 Golden、`runManifest`、核心 `reactionTaskLog`、1.39 反应交付引用、1.40 EC cleanup reciprocal 引用、1.41/1.42 传播与 cadence 向量、24 标签/16 类经典反应发布门，以及 1.44 v3 callback delivery/attempt 完整性契约。随后补其他来袭元素的有序反应、完整特殊/多 Aura 排列和 Burning 之外的通用目标任务所有权；Lunar 反应另立版本实现。基础机制稳定后才处理玩家 Aura/敌袭生存模型、角色机制映射、UID 配置编译和专用 UI。
