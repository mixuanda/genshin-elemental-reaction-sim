...
- Milestone 5 已建立首个逐技能部分机制闸门：Schema/引擎升级到 `1.4.0` / `1.4.0-action-states`，支持声明式角色特有 ICD Profile，以及行动状态的授予、前置、消耗、刷新和精确到期；`AbilityBlueprint` 1.1 与通用编译器会从固定目录解析倍率、保留证据/前置条件/未实现项，并默认拒绝 `partial`。
...
- 冻结的杜林预设仍为 `legacy-v0.1` Golden 兼容配置，手工反应和示例魔法数没有被包装为正式数据。新增黑 E 审计向量不能替代它；逐后续动作取消、回能/产球 ICD、白 E、爆发、状态驱动的被动、命座和装备尚未完成。
