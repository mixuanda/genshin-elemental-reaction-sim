...
- 当前输入契约为 `schemaVersion: 1.49.0`、`engineVersion: 1.49.0-reaction-owned-reset-boundary`，当前 `runManifest.version` 为 `1.5.0`；反应核心、目标任务、交付顺序、感电传播、经典反应公式、普通直伤组、数字元素施加和反应所有施加策略分别由显式版本字段固定。冻结 1.48/V1 策略 SHA-256 为 `sha256:50abcc04ad7bc55510e5786cbc3ace5105238e5fb2dea8e0944cd95708c80acc`；原生 1.49 默认预设选择 V2，SHA-256 为 `sha256:9b3b07731d49ebf8abb445708c3edb99b3ce8c3c7465ce5ca02b0a7c8092a660`。无版本、旧版本和精确 1.48 输入迁移到 1.49 时保留 V1，不静默升级到 V2；迁移不猜测 fixed group、不自动绑定未核验技能、不改变默认伤害，也不启用策略外的派生施加。精确 V148/V147/V146/V145/V144/V142 wire、Manifest 和 Fixture 保持冻结；精确 1.44–1.49 均可在既有版本门下显式选择 v3。历史版本夹带未来 model/selector 会 fail-closed。
- `aura-v5` 已把正/反融化、正/反蒸发、超载、超导、感电、冻结/碎冰、火/水/冰/雷扩散、火/水/冰/雷结晶、原激化/超激化/蔓激化、燃烧，以及绽放/草原核/烈绽放/超绽放接入无 DOM 的核心。`aura-v6`–`aura-v9` 继续冻结共享 Gauge 有序链、Quicken→Bloom core task、EC cleanup 与全局 cadence/Hitlag/Wane 所有权。精确 1.42 历史身份下 v9 只允许 `legal-frame-v1 + 60 FPS + target-phase-v2`；精确 1.44–1.49 可在同样的帧率/时间线门下与 v2 或显式 v3 组合。这些经典反应路径仍全部是 `fixed-gcsim-provisional`，不能视为官服真值或完整 gcsim 精度；v1–v8 的历史语义和 Golden 保持不变。
...
- 当前 V149 完整性门继承 V148 的基础反应、公式、普通直伤组、配置直接 application 与两种反应所有 application 证明，再按配置选择独立重放 V1/V2 policy root、selector、reset schedule 和 decision，并拒绝独立反应 parent 复用 `eventSequence`。V148 及更早历史证明保持原样；V149 新增层不等于从完整 scheduler trace、Ability/Aura、动作快照或角色数据库重放全链，也不证明其他反应应该施加元素。
...
- 每次模拟返回 Manifest 1.5.0，固定配置哈希、Schema/引擎/数据版本、解析后的运行选项、随机种子、有序插件身份、完整 `reactionFormulaRoot`、`directDamageGroupRoot`、`elementalApplicationIcdRoot` 与已选 V1/V2 `reactionOwnedElementalApplicationRoot`；复现键使用 `gdl-v2-fnv1a32-*`。冻结 V148 Manifest 仍是 1.4.0，V147 仍是 1.3.0。当前 32-bit FNV-1a 已知可碰撞，只是 drift detector，不能作为密码学身份或签名。
...
- 当前现场结果：完整 unit 为 121 个文件、1574 项测试；data check 确认 120 个角色、125 套天赋、762 个技能/被动、237 把武器；typecheck、build 与 Playwright Chromium 36/36 通过。性能探针 3/3 通过，伤害/事件/hash 均未漂移；最新一次持续 Burning 中位数 `57.605ms`、p95 `65.909ms`、最大值 `69.428ms`。`npm run check` 已全绿，但本来不包含 E2E，浏览器门仍须单独运行。墙钟阈值不是跨设备 SLA，仍可能受主机负载影响。本轮没有扩展 UI，浏览器只验证既有指标、角色/技能构成与伤害曲线。build 仍只有主 JS chunk 超过 500 kB 的非阻断警告。
...
- Zod 已严格覆盖当前 V149 输入与结果；当前 `simulationResultV149Schema` 和 `assertTrustedSimulationResultV149()` 在 V148 全部证明上增加 V1/V2 配置/root/selector/reset-policy/decision 重放和反应 parent sequence 完整性。冻结 `simulationResultV148Schema`、`simulationResultV147Schema`、`simulationResultV146Schema`、`simulationResultV145Schema`、`simulationResultV144Schema`、`simulationResultV142Schema` 仍保持精确身份并由各自断言接纳。外部或持久化完整结果必须走完整 Zod；trusted assertion 不是外部 wire 验证边界。
...
- Burning 的 `legacy-event-heap-v1` 有意保留 1.30 兼容相位；1.37 v1 与 1.38 v2 也继续冻结。精确 1.44–1.49 的显式 v3 可在 owner callback 内交付列明的零延迟跨目标伤害/Aura；启用目标时钟时陈旧唤醒会先重投影。1.48 只将实际 Burning Tick 的 target-global observable projection 纳入统一日志，1.49 只给该 channel 与 Swirl 增加显式 V2 重置边界，不改写 1.47/1.48 wire。所有这些顺序仅为 `fixed-gcsim-provisional`，不是官服真值、完整 target phase 或完整 gcsim 精度。
...
- 当前 Schema/引擎为 `1.49.0` / `1.49.0-reaction-owned-reset-boundary`，Manifest 为 1.5.0。既有具名目标、Aura/反应、几何、时钟、EC cleanup/传播、普通直伤组和 V147 配置直接 application 边界保持不变；V149 只给已有 Burning Tick 与 Swirl AoE 传播增加显式 V1/V2 边界策略。原生 1.49 默认预设使用 V2，历史迁移保留 V1；两者继续保留既有 Aura/目标任务/反应交付模式。精确 1.44–1.49 输入均可在版本门下 opt-in `target-phase-v3`。`AbilityBlueprint` 当前为 1.9，1.0–1.8 可迁移；编译器可显式传递两类 descriptor/application selector，但不会自动猜测 fixed group 或反应所有施加。
...
3. 使用 Zod 校验输入 JSON 并提供字段路径错误。当前精确 1.49 输出由 `simulationResultV149Schema` 覆盖全部已声明顶层字段和叶节点；冻结 `simulationResultV148Schema` / `simulationResultV147Schema` / `simulationResultV146Schema` / `simulationResultV145Schema` / `simulationResultV144Schema` / `simulationResultV142Schema` 仍只接受各自精确历史身份。外部或持久化结果必须完整解析，核心内部当前结果运行 `assertTrustedSimulationResultV149()`。V145 公式根、V146 Damage Group root/log、V147 配置直接 application root/log、V148 反应所有策略 root/log，以及 V149 的 V1/V2 root/selector/reset-policy/decision 与 parent sequence 所有权都必须 strict 校验；trusted assertion 不能作为外部 wire 的验证边界。
4. 加入 Schema 迁移系统，例如：
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.49 权威 Current State 为起点：保持全部历史 Golden、Manifest 1.5 四 root、两份 1.49 Fixture、冻结 V148/V147 wire 与 Fixture、核心 `reactionTaskLog`、反应交付/EC cleanup 引用、1.41/1.42 传播与 cadence 向量、24 标签/16 类经典反应发布门，以及 1.44–1.49 v3 callback delivery/attempt 完整性契约。下一步先补 Burning restart/cascade、双 Anemo、混合同帧 Swirl 与版本化 scheduler provenance，再逐技能绑定经过来源核验的两类 Tag/Group；任何新的反应所有施加都必须先核对固定来源并升版策略根。Lunar 反应另立版本实现。基础机制稳定后才处理玩家 Aura/敌袭生存模型、全角色机制映射、UID 配置编译和专用 UI。
