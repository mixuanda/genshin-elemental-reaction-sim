...
- 当前输入契约为 `schemaVersion: 1.52.0`、`engineVersion: 1.52.0-freeze-broken-attack`，当前 `runManifest.version` 为 `1.8.0`；前六份信任根与 V1.51 wire/Manifest/Fixture 完整冻结，新增第七根 `freezeBrokenAttackRoot`。所有 `1.51.0` 及更早输入迁移到 1.52 时为 `freezeBrokenAttackModel` 选择 V1，不静默升级到 V2；只有原生 current 配置可显式选择 V2。V1 root 为 `legacy-no-freeze-broken-attack-callback` / `legacy-no-freeze-broken-attack-callback-v1` / `sha256:2831fac7a15189b772db58c245ffd8091b1128b5fd5ea516885f03a99961c838`，V2 root 为 `fixed-gcsim-freeze-broken-attack-normalized-v2` / `gcsim-b4ae769-freeze-broken-attack-normalized-provisional-v2` / `sha256:71646812a4061c9ef2d4ae8ca7cef1abaa79d718c8831ffaf5e3f27832955e14`。迁移不猜测 fixed group、不自动绑定未核验技能、不改变默认伤害，也不物化参考合成攻击。精确 V151/V150/V149/V148/V147/V146/V145/V144/V142 wire、Manifest 和 Fixture 保持冻结；精确 1.44–1.52 均可在既有版本门下显式选择 v3。历史版本夹带未来 model/selector 会 fail-closed。
- `aura-v5` 已把正/反融化、正/反蒸发、超载、超导、感电、冻结/碎冰、火/水/冰/雷扩散、火/水/冰/雷结晶、原激化/超激化/蔓激化、燃烧，以及绽放/草原核/烈绽放/超绽放接入无 DOM 的核心。`aura-v6`–`aura-v9` 继续冻结共享 Gauge 有序链、Quicken→Bloom core task、EC cleanup 与全局 cadence/Hitlag/Wane 所有权。精确 1.42 历史身份下 v9 只允许 `legal-frame-v1 + 60 FPS + target-phase-v2`；精确 1.44–1.52 可在同样的帧率/时间线门下与 v2 或显式 v3 组合。这些经典反应路径仍全部是 `fixed-gcsim-provisional`，不能视为官服真值或完整 gcsim 精度；v1–v8 的历史语义和 Golden 保持不变。
...
- V2 固定参考 `genshinsim/gcsim@b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541`，但执行状态严格为 `reference-audit-only-not-dispatched`。行内保留 `sourceIsSim: true`、`doNotLog: true` provenance，`damageEventId` / `hitResolutionLogId` 固定为 null；本地不消费 RNG，不物化 DamageEvent/HitResolution，不增加命中、伤害曲线、总伤或 DPS。callback bus、Mona bubble/impulse、全局/订阅者副作用和参考 crit RNG draw 均未实现；固定参考中的 `doNotLog` 在该路径上是未读取字段。不得把这份 audit root 写成已执行合成攻击、官服真值或完整 gcsim parity。
...
- 当前 V152 完整性门继承冻结 V151 的全部证明，再按配置重放 Freeze Broken V1/V2 root、Frozen 耗尽来源和 `freezeBrokenAttackLog`。V1 必须零行；V2 的五类正触发须恰好一行，Melt/Superconduct/部分消耗须零行，且任何行都不得链接 DamageEvent/HitResolution。V151 及更早历史证明保持原样；V152 新增层不等于执行 callback bus、参考 RNG、Mona bubble/impulse 或从完整 scheduler trace、Ability/Aura、动作快照、角色数据库重放全链。
...
- 每次 current 模拟返回 Manifest 1.8.0，固定配置哈希、Schema/引擎/数据版本、解析后的运行选项、随机种子、有序插件身份、前六份冻结 root 与已选 `freezeBrokenAttackRoot`；复现键使用 `gdl-v2-fnv1a32-*`。冻结 V151 Manifest 仍是 1.7.0，V150 仍是 1.6.0，V149 仍是 1.5.0。当前 32-bit FNV-1a 已知可碰撞，只是 drift detector，不能作为密码学身份或签名。
...
- Zod 已严格覆盖当前 V152 输入与结果；`simulationResultV152Schema` 和 `assertTrustedSimulationResultV152()` 在冻结 V151 全部证明上增加第七根、Frozen 耗尽审计与 audit-only 边界。`simulationResultV151Schema` 和 `assertTrustedSimulationResultV151()` 以及所有更早精确 Schema/断言仍保持冻结身份。外部或持久化完整结果必须走完整 Zod；trusted assertion 不是外部 wire 验证边界。
...
- 当前 Schema/引擎为 `1.52.0` / `1.52.0-freeze-broken-attack`，Manifest 为 1.8.0。既有具名目标、Aura/反应、几何、时钟、EC cleanup/传播、普通直伤组、配置直接 application、Burning/Swirl 反应所有施加、ReactionA/B 和基础调度边界保持不变；V152 只新增 Freeze Broken V1/V2 root、audit-only 行和结果完整性证明。所有历史迁移固定使用 V1；原生 current 配置才可显式选择 V2。精确 1.44–1.52 输入均可在版本门下 opt-in `target-phase-v3`。`AbilityBlueprint` 当前为 1.9，1.0–1.8 可迁移；编译器可显式传递两类 descriptor/application selector，但不会自动猜测 fixed group 或反应所有施加。
...
3. 使用 Zod 校验输入 JSON 并提供字段路径错误。当前精确 1.52 输出由 `simulationResultV152Schema` 覆盖全部已声明顶层字段和叶节点；冻结 `simulationResultV151Schema` 及更早精确 Schema 仍只接受各自历史身份。外部或持久化结果必须完整解析，核心内部当前结果运行 `assertTrustedSimulationResultV152()`。V145–V151 的公式、Damage Group、application、ReactionA/B 与调度 root/log 保持 strict；V152 另严格校验 `freezeBrokenAttackModel`、Manifest root、Frozen 耗尽来源、audit-only 行、正负触发基数和空 DamageEvent/HitResolution 引用。trusted assertion 不能作为外部 wire 的验证边界。
4. 加入 Schema 迁移系统，例如：
...
Milestone 0、Milestone 1 和 Milestone 2 的第一轮要求已经完成。以下清单保留为历史验收契约，不能因后续功能扩展而退化。当前续作以本文件顶部的 1.52 权威 Current State 为起点：保持全部历史 Golden、Manifest 1.8 七 root、独立的 1.52 Freeze Broken audit Golden、冻结 V151 Manifest 1.7/基础调度 Golden、V150 Manifest 1.6/ReactionA/B Golden 与全部更早 wire/Fixture、核心 `reactionTaskLog` / `reactionDamageGroupResetLog` / `basicReactionSchedulerLog` / `freezeBrokenAttackLog`、反应交付/EC cleanup 引用、1.41/1.42 传播与 cadence 向量、24 标签/16 类经典反应发布门，以及 1.44–1.52 v3 callback delivery/attempt 完整性契约。下一步先建立版本化 callback bus，再补 Freeze Broken 的 Mona bubble/impulse、全局/订阅者副作用与明确 RNG 语义；不得把 audit row 直接当作已执行攻击。随后继续 Burning restart/cascade、双 Anemo、混合同帧 Swirl 与更完整 scheduler provenance，再逐技能绑定来源核验后的 Tag/Group。Lunar 反应另立版本实现；基础机制稳定后才处理玩家 Aura/敌袭生存模型、全角色机制映射、UID 配置编译和专用 UI。
