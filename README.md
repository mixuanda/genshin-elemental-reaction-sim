...
一个以“逐段可审计、配置可迁移、结果可复现”为目标的原神队伍 DPS 模拟器。当前完成了 Vanilla v0.1 基线冻结、TypeScript 模拟核心、合法帧时间线、火/冰/水/雷普通 Aura、自动增幅反应与超载独立伤害最小闭环、含命中触发与内部冷却审计的粒子/能量闭环、版本化数据目录与 UID 映射基础，以及带来源闸门的杜林黑/白 E 部分机制审计向量。
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.21.0 配置迁移；当前 Schema 为 `1.22.0`、引擎为 `1.22.0-overload-reaction`。
- `aura-v1` 支持火/冰/水普通 Aura、可扩展元素量、衰减、默认三击/2.5秒 ICD、No ICD、独立的角色/Tag/Group 流和显式声明的角色特有 ICD Profile。
- `aura-v2` 在保持上述语义的基础上加入雷普通 Aura，以及火打雷、雷打火两种超载触发方向。超载本体与独立反应伤害分开记录；独立伤害延迟 1 帧、同一触发目标具有 6 帧伤害 GCD，并以触发目标命中时的位置为圆心对半径 3 内的注册目标逐一求交。
...
- 超载触发命中与每个实际范围伤害都是独立 `DamageEvent`；触发事件通过 `transformativeReaction` 记录排队/GCD，伤害事件通过 `parentDamageEventId` 反链触发命中，并带独立的等级基准、精通、反应增伤和抗性因子。
- 角色/技能伤害构成。
...
- 感电、超导、冻结、扩散、结晶、绽放、燃烧、激化等其余转化/加算反应及其伤害事件。当前只有超载进入这套独立伤害管线。
- 超载尚未模拟击退、韧性、爆炸高度/三维碰撞、物件/召唤物/玩家自身受击或真实敌方移动；半径 3 求交依赖场景显式提供的二维目标坐标与圆形碰撞半径。等级基准和反应常数来自固定 gcsim 提交的交叉校验，不代表整个 Aura/反应系统已达到 gcsim 精度。
