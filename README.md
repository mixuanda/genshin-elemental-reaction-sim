...
一个以“逐段可审计、配置可迁移、结果可复现”为目标的原神队伍 DPS 模拟器。当前完成了 Vanilla v0.1 基线冻结、TypeScript 模拟核心、合法帧时间线、火/冰/水/雷普通 Aura、自动增幅反应、超载/超导独立伤害与超导物抗状态最小闭环、含命中触发与内部冷却审计的粒子/能量闭环、版本化数据目录与 UID 映射基础，以及带来源闸门的杜林黑/白 E 部分机制审计向量。
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.22.0 配置迁移；当前 Schema 为 `1.23.0`、引擎为 `1.23.0-superconduct-reaction`。
...
- 超载/超导触发命中与每个实际范围伤害都是独立 `DamageEvent`；触发事件通过 `transformativeReaction` 记录排队/GCD/目标状态定义，伤害事件通过 `parentDamageEventId` 反链触发命中，并带独立的等级基准、精通、反应增伤和抗性因子。
- 角色/技能伤害构成。
...
- 感电、冻结、扩散、结晶、绽放、燃烧、激化等其余转化/加算反应及其伤害事件。当前只有超载和超导进入这套独立伤害管线。
