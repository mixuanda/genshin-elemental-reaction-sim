...
只验证当前 1.48 四信任根、直接/反应所有元素施加 ICD、普通直伤 Damage Group、24 标签/16 类经典反应发布门、感电单目标兼容/附近湿目标传播、`aura-v9` 全局 cadence safety、逐元素敌方抗性、Quicken→Bloom 核心任务、`aura-v8` cleanup、冻结的目标相位、v3 Burning callback 零延迟跨目标交付、递归碎冰交付、可靠 Schema 与兼容 Golden 时可运行：
...
本轮最终现场门：`npm run test:unit` 为 117 个文件、1550 项测试全部通过；`npm run data:check` 确认 120 个角色、125 套天赋、762 个技能/被动和 237 把武器；`npm run typecheck`、`npm run build` 与 Chromium Playwright 36/36 通过。性能探针的伤害、Tick、应用行和冻结投影哈希均未漂移，但最终一次墙钟门在高系统负载下为 2/3：持续 Burning 中位数 `144.942ms`，超过未放宽的 `<100ms` 桌面门；最终代码仍需在低负载主机上重跑 3/3。既有页面继续渲染默认指标、角色/技能构成和伤害曲线，本轮没有新增 UI；构建仍有单个主 JS chunk 超过 500 kB 的非阻断警告。
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.47.0 配置迁移；当前 Schema 为 `1.48.0`、引擎为 `1.48.0-reaction-owned-application-root`、Manifest 为 `1.4.0`。1.46→1.47 的 `elementalApplicationIcdModel` 和显式 selector 迁移保持冻结；1.47→1.48 注入固定 `reactionOwnedElementalApplicationModel`，并原样保留公式、Damage Group、Aura、传播、目标任务、目标时钟、玩家模型和反应交付模式。它不改变默认数值，不会将保留组暴露给配置的直接命中，也不自动启用未列明的反应派生施加。精确 V147/V146/V145/V144/V142 输入、结果与 Manifest 保持冻结 legacy 身份；历史输入夹带未来字段会 fail-closed。
...
- 每次运行返回 Manifest 1.4.0，固定输入 Schema/引擎/数据版本、配置哈希、解析后的运行选项、有序插件身份、`reactionFormulaRoot`、`directDamageGroupRoot`、`elementalApplicationIcdRoot`、`reactionOwnedElementalApplicationRoot` 和 `gdl-v2-fnv1a32-*` 复现键；冻结 V147/V146/V145/V144/V142 Manifest 保持各自精确版本。当前 32-bit FNV-1a 只用于漂移检测，可碰撞，不能充当密码学签名。
- 输出侧当前使用精确 `simulationResultV148Schema`，同时保留身份严格的冻结 `simulationResultV147Schema`、`simulationResultV146Schema`、`simulationResultV145Schema`、`simulationResultV144Schema` 与 `simulationResultV142Schema`。V148 在 V147 直接命中重放之上，再从第四策略根重放 Burning Tick 与 Swirl 传播的 source kind、selector、独立状态窗口、数字倍率、名义/有效 Gauge 和 reciprocal 外键；删除、重复、换序或协调篡改会被 public Zod 与 `assertTrustedSimulationResultV148()` 拒绝。这四份 root 仍不证明完整 Aura、动作快照输入来源、角色数据库、官服真值或完整 gcsim 精度。
...
- 1.32–1.47 的版本化模型和历史 Fixture 均保持冻结；1.48 只追加反应所有元素施加策略 root、统一日志及独立 Golden。所有 Fixture 都是回归证据，不是官方数值认证或完整 gcsim parity 报告；测试抗性、草原核寿命、固定参考路径、附近传播半径和默认杜林预设的示例魔法数仍是 `provisional`、`community-provisional` 或 `fixed-gcsim-provisional`。

## 当前精度边界

本版本不是 gcsim 精度实现。`legacy-v0.1` 继续保持 Golden 兼容；`aura-v1`–`aura-v9`、三种目标相位、碎冰递归交付、感电 cleanup/传播/cadence、1.45 经典反应公式根、1.46 普通直伤 Damage Group、1.47 数字元素施加 ICD、1.48 Burning/Swirl 反应所有施加和粒子引擎只覆盖当前可审计机制闭环。固定 gcsim 交叉参考路径保持 `fixed-gcsim-provisional`；1.41 附近传播单独标记为 `community-provisional`，不能称为官方/官服真值或完整 gcsim 精度。尚未实现或未完整实现：

- 1.47 已固定数字 elemental-application sequence 并保留纳西妲 Skill 的 `1.5` 数值槽；1.48 只将 Burning Tick 和 Swirl AoE 传播纳入统一 `elementalApplicationIcdLog`。Swirl 源目标的伤害段不携带附着；EC/Wane、Overload、Superconduct、Shatter、Bloom/Burgeon/Hyperbloom、Quicken 后续、core 伤害和 Crystallize 都不生成反应所有施加行。58 组来源表也不等于 58 组已批量绑定到全角色技能：公共直接命中只能选 55 组，且当前只有合成向量和少量来源核验 Ability 使用固定 selector。同帧 reset/order 仍取决于当前任务插入顺序，是 provisional 合同。

- `aura-v4`–`aura-v9` 已实现上述 Burning Marker、Fuel、周期伤害、Tick 火附着、归属/快照刷新和核心输出。`legacy-event-heap-v1`、v1 和 v2 继续按冻结历史语义将实际范围伤害排入后续全局 `reactionDamage`。精确 1.44–1.48 都可显式选择 `target-phase-v3`，在 owner callback 内完成列明的零延迟跨目标交付。`target-local-hitlag-v1` 造成的陈旧唤醒会先重投影到正确全局帧；该切片仍没有实现通用敌方任务所有权、完整 gcsim target phase 或官服全部边界，正延迟子反应也仍由全局 heap 处理。
...
- 感电结果完整性由 public Zod 与当前 `assertTrustedSimulationResultV148()` 复用同一组可变异证明，冻结版本继续使用各自精确历史断言。它能闭合已建模 generation、生命周期、Wane、cleanup 和时间线引用，但尚未从完整 Ability/应用/Aura 历史唯一重放全部输入；官方传播半径/目标选择、多 Aura 全排列和 Lunar 反应仍未完成，现有门不能当作官服真值、完整 EC 生命周期或 gcsim parity。
