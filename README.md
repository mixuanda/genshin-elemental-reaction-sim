...
- Zod 严格校验、字段路径错误，以及 v0.1 / 1.0.0 / 1.1.0 / 1.2.0 / 1.3.0 / 1.4.0 配置迁移；当前 Schema 为 `1.5.0`、引擎为 `1.5.0-followup-cancels`。
...
- `AbilityBlueprint` 1.2 把技能倍率引用、命中帧、分后续动作取消帧、附着、ICD、固定回能、产球、行动状态、前置条件、未实现机制和逐项证据放入严格 Schema；1.0 / 1.1 Blueprint 可迁移，通用编译器默认拒绝 `partial`。
...
合法时间线当前是确定性命令编译器。能量不足仍由伤害模拟阶段记录；尚未实现“施放失败后重新编排后续命令和返还预占冷却”的动态队列回滚。为避免预先错误改变状态，Schema 暂时禁止带能量消耗的行动授予或消耗行动状态。Aura 引擎当前为单目标；没有 AoE 各目标独立 Aura、Hitlag 和完整角色动作/ICD 数据库。
