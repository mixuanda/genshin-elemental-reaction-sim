...
- Zod 严格校验、字段路径错误，以及 v0.1 / 1.0.0 / 1.1.0 / 1.2.0 / 1.3.0 / 1.4.0 / 1.5.0 / 1.6.0 / 1.7.0 / 1.8.0 / 1.9.0 / 1.10.0 / 1.11.0 / 1.12.0 / 1.13.0 配置迁移；当前 Schema 为 `1.14.0`、引擎为 `1.14.0-multi-target-registry`。
...
- `enemy.targets` 可注册最多 32 个具名敌方目标，并分别覆盖等级、抗性、减防和初始附着；未声明时核心物化兼容目标 `enemy-0`。每个目标拥有独立 Aura 与附着 ICD 状态，结果、逐击表和 Aura 曲线均保留目标身份并可筛选。
...
- 命中产球已支持逐击触发、已注册目标的 `landed / miss`、三层目标效果策略、按帧阶段窗口和精确内部冷却边界；但 Miss 与阶段仍是场景输入，不是由命中范围、目标位置、Boss AI 或碰撞自动求得。同一攻击对多个目标的 AoE 扇出、一次产球语义和逐目标回调聚合仍未建模。杜林黑/白爆发、命座和专属装备效果尚未完成；黑/白状态目前只完成互斥与合法性审计，尚未驱动全部被动效果。
- Hitlag、单击 AoE 扇出、索敌、移动、真实 Boss 阶段状态机、目标死亡、护盾和特殊易伤窗口。
...
合法时间线对每个有能量消耗的命令按顺序执行确定性前缀探测，再用已确认的失败集合重编译最终时间线；因此失败行动不会推进游标、预占冷却或改变状态，后续命令会按新帧重新执行。它仍不是 gcsim 式单遍动态行动执行器：命中确认分支、目标死亡、几何条件和玩家条件语句尚未进入命令语言。Aura/ICD 已按注册目标隔离，但没有由一次 AoE 自动扇出的逐目标命中、Hitlag 和完整角色动作/ICD 数据库。
