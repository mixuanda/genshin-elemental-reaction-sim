...
只验证当前 1.46 双信任根、普通直伤 Damage Group、24 标签/16 类经典反应发布门、感电单目标兼容/附近湿目标传播、`aura-v9` 全局 cadence safety、逐元素敌方抗性、Quicken→Bloom 核心任务、`aura-v8` cleanup、冻结的 1.37 v1 目标相位、1.38 v2 callback→`Reactable.Tick` 边界、1.44–1.46 v3 Burning callback 零延迟跨目标交付、递归碎冰交付、增幅覆盖闸门、玩家自伤、目标本地时钟、可靠 Schema 与兼容 Golden 时可运行：
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.45.0 配置迁移；当前 Schema 为 `1.46.0`、引擎为 `1.46.0-direct-damage-group-root`。1.35→1.45 的历史迁移契约继续冻结；1.45→1.46 只更新运行身份并注入固定 `directDamageGroupModel`，原样保留 `reactionFormulaModel`、`reactionEngine`、`electroChargedPropagationModel`、目标任务、目标时钟、玩家模型与伤害交付模式，不会自动分配未核验的技能 Tag/Group，也不会改变数值或自动启用其他 opt-in 机制。精确 V145/V144/V142 输入、结果与各自 Manifest 保持冻结 legacy 身份；当前 V146 Manifest 为 1.2.0。历史输入不得夹带 `directDamageGroupModel`、命中级 `directDamageGroup` 或其他未来字段；1.43 仍只是未发布 energy wire 的保留版本号。逐元素抗性仍从 1.35 起才是合法 wire contract，旧版本夹带该字段会 fail-closed。
...
- `targetTaskModel` 与目标时钟、Aura 版本分别建模，并提供四种模式：`legacy-event-heap-v1` 是历史迁移和当前兼容预设的默认值；`target-phase-v1` 是冻结的 1.37 第一批 Burning callback→Aura 衰减契约；`target-phase-v2` 是 1.38 的独立显式 opt-in；`target-phase-v3` 允许精确 1.44、1.45 或当前精确 1.46 在 `legal-frame-v1 + 60 FPS + aura-v7/v8/v9` 门下显式选择。v3 保留 v2 的 QueueEnemyTask→同目标 `Reactable.Tick` 边界，但把实际 Burning Tick 的跨目标范围命中作为 callback 所有的 `burning-callback-zero-delay-v1` 微事件同帧交付。接收目标若尚未运行当帧 Tick，附着结算使用 F-1/当前已物化 Aura，再进入该目标 F `Reactable.Tick`；注册顺序更早、已运行 Tick 的目标则记为 `after-reactable-tick`。v3 完整性校验不允许结果日志互相自证：目标注册顺序、名称、初始位置、圆形 hurtbox、运动分段、阶段窗口和三层目标策略均从输入配置重放；callback 截止、Burning 圆心/半径/`1U` 火附着，以及每个 attempt 的距离、阈值、原因、`damageAllowed / auraAllowed / hitConfirmAllowed`、伤害策略和机制状态都必须与配置根一致。v3 `delivery.attempts` 必须按所有敌人注册顺序完整记录 `landed / miss / unresolved`，并闭合 `reactionDamageLog`、命中、伤害和 `targetStateTimeline` 外键。正延迟子反应仍进全局 heap；`aura-v9` F+10/F+70 callback 和 Wane 也仍使用全局帧。冻结的 v1 使用 `targetTaskPhaseLog`；v2/v3 使用带严格模式判别的、相互排斥的 `targetPhaseLog`。
...
- 每次运行返回 `runManifest`：当前版本 1.2.0 固定输入 Schema/引擎/数据版本、版本化配置哈希、解析后的运行选项、按顺序排列且带内容哈希的插件身份、完整 `reactionFormulaRoot`、完整 `directDamageGroupRoot` 和 `gdl-v2-fnv1a32-*` 复现键；冻结的 V145 Manifest 继续使用精确 1.1.0，V142/V144 继续使用精确 1.0.0。当前 configHash/repro key 的 32-bit FNV-1a 已知可碰撞，只能用于确定性漂移检测，不能作为唯一运行身份、密码学完整性或签名；下一 Schema/Manifest 版本应迁移到 SHA-256。声明式效果由核心规范化后计算内容哈希，任意代码插件的 descriptor/contentHash 仍属于插件作者提供的受信声明，不能把它当作代码真实性证明。
- 输出侧当前使用精确 `1.46` 的 `simulationResultV146Schema`，同时保留身份严格的冻结 `simulationResultV145Schema`、`simulationResultV144Schema` 与 `simulationResultV142Schema`。所有顶层字段和 DamageEvent、ReactionAudit、普通直伤 Damage Group、反应/状态生命周期、行动、能量/粒子、汇总、曲线、Aura 边界及合法时间线叶节点都使用 strict Zod 结构；未知字段、非有限数、与当前已建模判别联合不匹配的分支及历史结果身份会 fail-closed。统一完整性层继续闭合 result/config/manifest 身份与别名、逐击 ID/队列顺序/个位显示/伤害构成、命中与已注册机制的主要反应反链、总伤/DPS/反应命中、角色/目标/技能/逐秒汇总和伤害曲线。V146 在 V145 公式证明之上，从配置和固定 Damage Group profile 独立重放目标/来源角色/Tag 窗口、reset、计数器、序列、插件前后倍率、DamageEvent 因子与 reciprocal 外键，拒绝删除、重复、换序或协同伪造日志。外部 JSON、持久化完整结果及完整 `SimulationResult` 测试向量必须走完整 Schema；`sim-core` 当前返回边界运行 `assertTrustedSimulationResultV146()`，冻结版本只由各自精确断言接纳。可信断言不等同于完整 Zod parse；两份固定 root 也不代替完整元素附着 ICD、Aura、动作快照输入来源、角色数据或 gcsim 精度验证。
...
- 1.32 增加显式 `playerDamageModel`，1.33 增加显式 `targetClockModel`，1.34 增加显式 opt-in `aura-v6`，1.35 增加可选的完整敌方八项抗性表，1.36 增加显式 opt-in `aura-v7` 与核心 `reactionTaskLog`，1.37 再增加显式 `targetTaskModel` 与 `targetTaskPhaseLog`，1.38 增加 `target-phase-v2` 与独立 `targetPhaseLog`，1.39 增加显式 `reactionDeliveryModel`，1.40 增加显式 opt-in `aura-v8` cleanup，1.41 增加显式 `electroChargedPropagationModel`，1.42 增加显式 opt-in `aura-v9` 全局 cadence safety，1.44 增加显式 `target-phase-v3` 与 callback-owned delivery audit，1.45 固定经典反应公式根，1.46 固定普通直伤 Damage Group root 与结果重放日志。1.45→1.46 只更新身份、Manifest 并增加未绑定技能的固定选择，不自动改变旧模式或伤害数值；1.43 保留未发布。所有历史 Fixture 均不得回写；每一版本只能新增独立 Fixture。所有 Fixture 都是回归证据，不是官方数值认证或完整 gcsim parity 报告；其中测试抗性、草原核寿命、固定 gcsim 路径、附近传播半径和默认杜林预设的示例魔法数都仍是 `provisional`、`community-provisional` 或 `fixed-gcsim-provisional`。

## 当前精度边界

本版本不是 gcsim 精度实现。`legacy-v0.1` 继续保持 Golden 兼容；`aura-v1`–`aura-v9`、冻结的 `target-phase-v1`、受限的 `target-phase-v2`、1.44 显式 opt-in 的 `target-phase-v3`、1.39 的碎冰递归交付切片、1.40 的 EC next-target-Tick cleanup、1.41 默认关闭的附近湿目标传播、1.42 的 EC 全局 cadence safety、1.45 经典反应公式根、1.46 普通直伤 Damage Group 和粒子引擎只覆盖当前可审计机制闭环。v3 也只声明已建模 Burning Tick 的 callback-owned 零延迟跨目标交付、F-1/当前 Aura 读取与接收目标 `Reactable.Tick` 的局部顺序；它不是通用全目标 barrier、全部反应同步语义或完整 gcsim target phase。固定 gcsim 交叉参考路径保持 `fixed-gcsim-provisional`；1.41 附近传播单独标记为 `community-provisional`，不能称为官方/官服真值或完整 gcsim 精度。尚未实现或未完整实现：

- 1.46 只固定普通直接伤害的 Damage Group reset/damage sequence。元素附着仍由既有 Aura 引擎中的 boolean `icdAllowed` 与旧 Profile 处理，其状态 key 和数字序列尚未与本次固定 Damage Group root 统一；固定参考中的 elemental-application sequence、例如纳西妲 `1.5` 数值槽，并未进入当前 1.46 wire。不得把 58 组 Damage Group 覆盖写成 58 组元素附着 ICD 已实现。

- `aura-v4`–`aura-v9` 已实现上述 Burning Marker、Fuel、周期伤害、Tick 火附着、归属/快照刷新和核心输出。`legacy-event-heap-v1`、v1 和 v2 继续按冻结历史语义将实际范围伤害排入后续全局 `reactionDamage`。精确 1.44、1.45 与当前精确 1.46 都可显式选择 `target-phase-v3`，在 owner callback 内完成零延迟跨目标交付，并按注册顺序标记接收目标为 `before-reactable-tick` 或 `after-reactable-tick`。`target-local-hitlag-v1` 造成的陈旧唤醒会先重投影到正确全局帧；该切片仍没有实现通用敌方任务所有权、完整 gcsim target phase 或官服全部边界，正延迟子反应也仍由全局 heap 处理。
...
- 感电结果完整性现在由 public Zod 与当前 `assertTrustedSimulationResultV146()` 复用同一组可变异证明；冻结 V145/V144 继续使用各自精确历史断言。每个目标的 `start` generation 必须从 1 连续递增，start/refresh audit 与生命周期行双向唯一闭合；命中移除型 ordinary stop 必须与唯一 `COEXISTING_AURA_REMOVED_BY_HIT` 行精确对应，legacy missing-stop 也必须继承上一排程的 frame/source/trigger，机制截断则明确禁止伪造终止；Wane 必须反链原 Tick 的 `waneFrame/tickIndex`，并由回调前水雷共存和来源目标实际子伤害推导 `stop / wane-skipped / wane`。实际 Wane 按 Hydro→Electro 顺序，从已闭合的前态对每个来源槽分别扣除至多 `0.4U`，再核对 reason、下一 Tick、共存截止、上一目标时间线 deadline、reciprocal timeline point 及 `aura-v9` cadence/listener。精确 1.44 wire 会省略清理后消费量 `<=1e-10U` 的元素；校验器只在该元素确实耗尽的序列化 epsilon 边界接受这一冻结例外，物质消费不得漏项。Wane 后期限只证明与舍入前后 Gauge、`ceil(gauge / decay - 1e-9)` 和重放后的目标时钟存在同一可行衰减率，尚未从 Ability/ICD/完整 Aura 历史重放唯一 `decayPerFrame`。pre-v8 跨 generation Wane 仍作为冻结兼容例外；官方传播半径/目标选择、多 Aura 全排列和 Lunar 反应仍未完成，现有 1.42 Golden 与这些完整性门都不能当作官服真值、完整 EC 生命周期或 gcsim parity。
...
- 尚未把普通直伤 Damage Group 或元素附着 ICD 的 Tag/Group 批量绑定到全角色技能。1.46 固定 58 组可查询的直接伤害序列，但现有正式预设不因名称相似而自动赋值；元素附着侧仍只有少量声明式 Profile 与 DurinSkill 的 18 帧 / `[允许, 阻止, 阻止]` 审计映射。
...
packages/icd-profiles    固定普通直伤 Damage Group 数据叶与信任根
