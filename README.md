...
只验证 1.37 反应核心、逐元素敌方抗性、Quicken→Bloom 核心任务、第一批目标任务相位、增幅覆盖闸门、玩家自伤、目标本地时钟、可靠 Schema 与兼容 Golden 时可运行：
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.36.0 配置迁移；当前 Schema 为 `1.37.0`、引擎为 `1.37.0-target-task-phase`。1.35→1.36 的 identity-only 迁移仍按历史契约保留；1.36→1.37 为所有历史输入补入 `targetTaskModel: { mode: "legacy-event-heap-v1" }`，精确保留 `reactionEngine`、`playerDamageModel`、`targetClockModel`、逐元素敌方抗性和其余配置，不会自动启用 `target-phase-v1`，也不会把既有 Aura 模式静默切换为 `aura-v7`。1.35 及更早配置继续遵守各自历史模式的保留/禁用规则；逐元素抗性仍从 1.35 起才是合法 wire contract，旧版本夹带该字段会 fail-closed。
...
- 启用目标时钟后，核心返回严格校验的 `targetClockAudit`、可重放的压缩 `targetClockLog` 和逐次 `targetHitlagLog`。Schema 会逐点重放并核对 `TargetStateTimeline.targetFrame`，拒绝同一命中被重复消费为多次 Hitlag，并按 reciprocal 日志精确累计超导状态的延长帧数。普通 Aura、Frozen、Quicken、Burning Fuel/Tick 与感电共存自然到期使用稳定的目标本地截止帧；感电伤害 Tick/Wane、ReactionA/B、扩散/结晶/碎冰、独立反应伤害和草原核生成/到期/烈绽放/超绽放仍按全局帧运行。已存在且结束帧晚于命中帧的超导减物抗状态会按实际 Hitlag 延长；同帧稍后才创建的状态不会被追溯延长。
- `targetTaskModel` 与目标时钟、Aura 版本分别建模：`legacy-event-heap-v1` 是历史迁移和当前兼容预设的默认值；`target-phase-v1` 必须显式 opt-in，并要求运行时使用 `legal-frame-v1`，配置了反应引擎时只允许 `aura-v7`。当前第一批只覆盖 Burning target callback：同一全局帧按 `enemy.targets` 注册顺序处理每个目标的 callback，再推进该目标 Aura 衰减，全部目标完成后才处理同帧 core/incoming work；Hitlag 造成的陈旧唤醒会先按目标本地时钟重投影。核心返回严格的 `targetTaskPhaseLog`，记录任务前、任务后、衰减后的 Aura 及相关 Burning、命中和反应任务日志 ID。
...
- 输出侧已为 `SimulationRunManifest`、Burning/Quicken/Bloom 审计、`reactionTaskLog`、`targetTaskPhaseLog`、ReactionA/B 伤害组、草原核生命周期/接触/时间线、玩家命中/自伤/HP 时间线/汇总、目标时钟/Hitlag 及其跨日志引用、`TargetStateTimeline` 等关键投影提供严格 Zod Schema，并用真实模拟结果做运行时解析回归。Quicken→Bloom 任务会与触发命中、Quicken mutation、Bloom 审计、草原核预约和目标状态时间线做双向引用校验；目标任务相位会核对目标顺序、目标时钟、Aura 连续性和关联日志。整个 `SimulationResult` 仍只有 TypeScript 顶层契约，尚未具备覆盖全部结果字段的单一运行时 Zod Schema。
...
- `legacy-default-120s.golden.json` 继续冻结 Vanilla 的原始浮点结果；1.33–1.36 的版本化 Fixture 保留各自历史投影，`legacy-default-120s-1.36.golden.json` 不被改写；`legacy-default-120s-1.37.golden.json` 冻结当前 1.37 兼容身份与不变的数值基线。
- `burning-aura-v4-1.30.golden.json` 冻结 1.30 Burning 兼容切片；1.34–1.36 身份继续作为历史证据保留，当前 1.37 身份使用新的兼容 Fixture。
...
- 1.32 增加显式 `playerDamageModel`，1.33 增加显式 `targetClockModel`，1.34 增加显式 opt-in `aura-v6`，1.35 增加可选的完整敌方八项抗性表，1.36 增加显式 opt-in `aura-v7` 与核心 `reactionTaskLog`，1.37 再增加显式 `targetTaskModel` 与 `targetTaskPhaseLog`。1.35→1.36 只更新版本身份，不自动启用 v7；1.36→1.37 把历史输入固定迁移到 `legacy-event-heap-v1`，不会自动启用 target phase。`legacy-default-120s-1.36.golden.json` 和 `quicken-bloom-task-order-1.36.golden.json` 继续冻结历史身份；对应的 1.37 Fixture 冻结当前规范化身份和未漂移语义。1.35 的 17 向量反应矩阵继续作为语义 Golden 复用，历史 v1–v7 Fixture 均不重写。所有 Fixture 都是回归证据，不是官方数值认证或完整 gcsim parity 报告；其中测试抗性、草原核寿命和默认杜林预设的示例魔法数都仍是 provisional。

## 当前精度边界

本版本不是 gcsim 精度实现。`legacy-v0.1` 继续保持 Golden 兼容；`aura-v1`–`aura-v7`、第一批 `target-phase-v1` 和粒子引擎只覆盖当前可审计机制闭环。所有相关顺序都只标记为 `fixed-gcsim-provisional`：固定 gcsim 提交自身也保留反应顺序、草原核持续时间和燃烧测试 TODO，因此只能称为“固定代码路径兼容语义”，不能称为官方/官服真值或完整 gcsim 精度。尚未实现或未完整实现：

- `aura-v4/v5/v6/v7` 已实现上述 Burning Marker、Fuel、周期伤害、Tick 火附着、归属/快照刷新和可视化；v7 只修正 refresh 的反应计数投影。`legacy-event-heap-v1` 有意继承冻结的 1.30 相位：Fuel 在本项目兼容语义的第 121 个目标帧清理，且同一目标帧的普通命中先于 Burning Tick。1.37 的 `target-phase-v1` 必须显式 opt-in，目前只覆盖第一批 Burning target callback：callback 在该目标 Aura 衰减前检查 Marker/Fuel，按目标注册顺序执行，全部目标完成后才处理同帧 core/incoming work；`target-local-hitlag-v1` 造成的陈旧唤醒会先重投影到正确全局帧。该切片没有实现通用敌方任务所有权、完整 gcsim target phase 或官服全部边界。
...
- `aura-v5/v6/v7` 已实现上述绽放、草原核、烈绽放和超绽放纵向切片，并可在 `reaction-self-v1` 中结算其玩家自伤；v7 的 Quicken→Bloom 仍是列明的 core zero-delay task，不属于 1.37 的 target-owned queue。草原核 `300f` 寿命、简化二维位置/范围和最近目标选择仍是 provisional，丰穰之核、卡维强制迸发、角色专属核心修正和完整三维碰撞仍未实现。旧 `aura-v3/v4` 对绽放系继续保持历史 fail-closed。
...
- 火、水、冰、雷、草、风、岩七种来袭元素的代表性高信息量链已有 `aura-v7` 精确顺序测试；公开可配置的五种普通初始 Aura 又通过 62,208 个 mixed-gauge covering 向量检查 finite、非负、来源槽/消费守恒、确定性与输入数组换序。该门仍不覆盖 Frozen、Quicken、Burning/Fuel 等特殊 Aura 的全部可达排列，也不执行后续 Tick、Core、ReactionA/B 或目标任务相位；1.37 另以低 Fuel 边界、Hitlag 重投影、多目标顺序和 `targetTaskPhaseLog` 测试第一批 target phase。未覆盖分支仍应 fail-closed 或作为新引擎版本实现。所有固定顺序只标记为 `fixed-gcsim-provisional`，参考源码自身的顺序 TODO 也禁止把它外推为官服真值。
...
项目借鉴 gcsim 的“角色/技能伤害构成、逐帧 Sample、每个事件可展开计算、显式能量问题与版本化配置”思路。杜林黑/白 E 的动作帧、黑 E ICD、白 E 无附着口径、回能和产球行为，以及超载/超导/感电/冻结/碎冰/扩散/结晶/激化/燃烧、草原核、玩家反应自伤/结晶盾吸收和敌方 Hitlag 目标 Tick 顺序的列明代码路径使用固定提交 `b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541` 交叉校验；1–100 级反应基准数值表直接保留自该提交并附带 MIT 许可。Hitlag 的 `haltFrames/factor` 仍由场景显式输入，尚无角色/武器/攻击类型数据库自动推导。玩家八元素抗性和 1.35 的敌方八元素基础抗性也仍是用户显式输入和本项目的公式约定，不是从该提交取得的官服正式角色/敌人数据库真值。固定提交本身仍含草原核持续时间注释和燃烧测试 TODO，因此这里表示“对固定代码路径的可复现交叉校验”，不是官方数值或官服实测证明。TypeScript 实现和 Schema 为独立编写，没有复制其 Go 角色实现。完整倍率目录来自单独固定的 `genshin-db` MIT 数据包；Enka 只用于公开展示柜和数字 ID 互操作。详见 [THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md)。
