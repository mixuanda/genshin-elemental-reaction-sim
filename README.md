...
一个以“逐段可审计、配置可迁移、结果可复现”为目标的原神队伍 DPS 模拟器。当前完成了 Vanilla v0.1 基线冻结、TypeScript 模拟核心、合法帧时间线、火/冰/水/雷普通 Aura、自动增幅反应、超载/超导/感电/碎冰独立伤害、火/水/冰/雷扩散与范围传播、超导物抗状态以及冻元素耐久/冻结底反应最小闭环、含命中触发与内部冷却审计的粒子/能量闭环、版本化数据目录与 UID 映射基础，以及带来源闸门的杜林黑/白 E 部分机制审计向量。
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.26.0 配置迁移；当前 Schema 为 `1.27.0`、引擎为 `1.27.0-swirl-propagation`。
...
- 结晶、绽放、燃烧、激化等其余转化/加算反应及其伤害事件。当前超载、超导、感电、碎冰和四元素扩散已进入独立伤害/传播管线；冻结本身是无独立伤害的状态反应。
