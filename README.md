...
一个以“逐段可审计、配置可迁移、结果可复现”为目标的原神队伍 DPS 模拟器。当前完成了 Vanilla v0.1 基线冻结、纯 TypeScript 模拟核心、合法帧时间线、基础元素反应矩阵、`aura-v5` 有序 Aura 链、ReactionA/B 伤害组、燃烧 Marker/Fuel/Tick，以及绽放、草原核、烈绽放和超绽放的确定性纵向切片。1.32–1.42 先后建立玩家反应自伤、目标本地时钟、`aura-v6/v7/v8/v9`、逐元素敌方抗性、两种冻结目标相位、递归碎冰、感电 cleanup/传播/cadence 和 24 标签/16 类经典反应发布门；Lunar 反应族仍未实现。1.44 新增显式 opt-in 的 `target-phase-v3` Burning callback delivery，1.45 固定经典反应公式根，1.46 加入仅面向普通直接伤害的 Damage Group，1.47 将固定参考的数字元素施加序列接入独立 root，1.48–1.49 给 Burning Tick 和 Swirl AoE 传播施加建立反应所有策略根与版本化边界；1.50 则把转化反应的 ReactionA/B 伤害组重置独立为纯 TypeScript、root-bound 的任务顺序核心。所有这些参考路径仍标记为 `fixed-gcsim-provisional` 或 `community-provisional`，不是官服真值或完整 gcsim parity。敌方每次实际伤害继续由核心返回结构化构成并进入既有全队、个人、技能、时间轴、逐段日志和累计/构成曲线；本轮无 UI 扩展，展示层仍只读核心结构化结果。

当前运行身份为 `schemaVersion: 1.50.0`、`engineVersion: 1.50.0-reaction-damage-reset-boundary`，`runManifest.version` 为 `1.6.0`。Manifest 同时绑定五份无 DOM 的固定根：经典反应公式根 `gcsim-b4ae769-classic-provisional-v1` / `sha256:7ae4ee955e0c7986c47931cff596694c8cd4754b48df90e0ad1cf092738ccafd`，普通直伤 Damage Group 根 `gcsim-b4ae769-damage-groups-provisional-v1` / `sha256:7e6d16a2a90ac7d9bb84daa80c43f09d28fb65e45319c62f67d14c50bb5e9c70`，数字元素施加根 `gcsim-b4ae769-elemental-application-provisional-v1` / `sha256:df461cf8aefee33ec57b8a8f83e2ec26497f17be8bc3ee1e6d667bf91d4015c1`，配置选定的 Burning/Swirl 反应所有施加策略根，以及配置选定的 ReactionA/B 伤害组策略根。1.49 的施加策略 V1/V2 继续保持冻结身份；1.50 新增的伤害组 V1 为 `gcsim-b4ae769-reaction-damage-group-legacy-window-provisional-v1` / `sha256:db377845d06edaac61e92de5a9478117f2fdc79e55e920ad445a102cf9b9a3bd`，原生 1.50 默认 V2 为 `gcsim-b4ae769-reaction-damage-group-task-order-provisional-v2` / `sha256:026b9728156ddd124a2d85793b80d71a1d3f3baacec8376c8ac120bf68c17346`。固定参考根都来自 `genshinsim/gcsim@b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541`，且明确携带 `officialServerTruth: false` 与 `completeGcsimParity: false`。无版本与所有 1.49 及更早输入迁移到 1.50 时，新增 `reactionDamageGroupModel` 固定选择 V1，不会静默升级为 V2；只有原生 1.50 预设显式选择 V2。该迁移不改变默认伤害、不猜测技能 Tag/Group，也不自动启用策略外的反应派生施加。精确 V149/V148/V147/V146/V145/V144/V142 wire、Manifest 和 Fixture 继续按各自身份冻结。
...
只验证当前 1.50 五信任根、ReactionA/B 伤害组 V1/V2 边界、F+29 同帧 FIFO、Burning/Swirl 反应所有重置边界、直接/反应所有元素施加 ICD、普通直伤 Damage Group、24 标签/16 类经典反应发布门、感电单目标兼容/附近湿目标传播、`aura-v9` 全局 cadence safety、逐元素敌方抗性、Quicken→Bloom 核心任务、`aura-v8` cleanup、冻结的目标相位、v3 Burning callback 零延迟跨目标交付、递归碎冰交付、可靠 Schema 与兼容 Golden 时可运行：
...
第一组现场通过 13 个文件、206 项测试，第二组现场通过 4 个文件、69 项测试；合计 17 个文件、275 项测试全部通过。它们不是完整 `npm test` 或 `npm run check` 的替代。`npm run check` 包含数据校验、类型检查、单元/性能测试和构建，但不包含 Playwright，浏览器门必须另行运行 `npm run test:e2e`。

1.50 的发布计数与性能数字必须以当次 `npm run test:unit`、`npm run test:performance`、`npm run typecheck`、`npm run build` 和 `npm run test:e2e` 的现场输出为准，不沿用 1.49 的历史计数或墙钟数字。既有页面继续只读核心返回的默认指标、角色/技能构成和伤害曲线，本轮没有新增 UI。
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.49.0 配置迁移；当前 Schema 为 `1.50.0`、引擎为 `1.50.0-reaction-damage-reset-boundary`、Manifest 为 `1.6.0`。1.46→1.49 的数字施加与 Burning/Swirl 施加策略保持冻结；1.49→1.50 只新增 V1 `reactionDamageGroupModel` 并推进身份。无版本和所有旧版输入都迁移为 1.50 + ReactionA/B V1，不会被静默升级到 V2；原生 1.50 默认预设才显式使用 ReactionA/B V2。迁移原样保留公式、普通直伤 Damage Group、Aura、传播、目标任务、目标时钟、玩家模型和反应交付模式，不改默认伤害、不猜测 fixed group，也不自动启用未列明的派生施加。精确 V149/V148/V147/V146/V145/V144/V142 输入、结果与 Manifest 保持冻结 legacy 身份；历史输入夹带未来字段会 fail-closed。
...
- 每次运行返回 Manifest 1.6.0，固定输入 Schema/引擎/数据版本、配置哈希、解析后的运行选项、有序插件身份、`reactionFormulaRoot`、`directDamageGroupRoot`、`elementalApplicationIcdRoot`、已选 V1/V2 `reactionOwnedElementalApplicationRoot`、已选 V1/V2 `reactionDamageGroupRoot` 和 `gdl-v2-fnv1a32-*` 复现键；冻结 V149 Manifest 仍是 1.5.0，V148 仍是 1.4.0，更早版本保持各自精确身份。当前 32-bit FNV-1a 只用于漂移检测，可碰撞，不能充当密码学签名。
- 输出侧当前使用精确 `simulationResultV150Schema`，同时保留身份严格的冻结 `simulationResultV149Schema`、`simulationResultV148Schema`、`simulationResultV147Schema`、`simulationResultV146Schema`、`simulationResultV145Schema`、`simulationResultV144Schema` 与 `simulationResultV142Schema`。V150 在 V149 全部证明之上，闭合 `reactionDamageGroupModel`、Manifest root、ReactionA/B decision、F+29 reset schedule 和 `reactionDamageGroupResetLog` 反链；删除、重复、换序或篡改可重放字段会被 public Zod 与 `assertTrustedSimulationResultV150()` 拒绝。五份 root 仍不证明完整 Aura、ICD、动作快照输入来源、角色数据库、官服真值或完整 gcsim 精度。结果仍不是完整 scheduler trace，所以不能由这些局部日志证明通用任务调度完全 parity。
...
- 1.32–1.49 的版本化模型和历史 Fixture 均保持冻结；1.50 只新增 ReactionA/B 伤害组 V1/V2 root、F+29 任务顺序边界、对应 Schema/完整性门和独立 Golden。所有 Fixture 都是回归证据，不是官方数值认证或完整 gcsim parity 报告；测试抗性、草原核寿命、固定参考路径、附近传播半径和默认杜林预设的示例魔法数仍是 `provisional`、`community-provisional` 或 `fixed-gcsim-provisional`。

## 当前精度边界

本版本不是 gcsim 精度实现。`legacy-v0.1` 继续保持 Golden 兼容；`aura-v1`–`aura-v9`、三种目标相位、碎冰递归交付、感电 cleanup/传播/cadence、1.45 经典反应公式根、1.46 普通直伤 Damage Group、1.47 数字元素施加 ICD、1.48 Burning/Swirl 反应所有施加、1.49 施加重置边界、1.50 ReactionA/B 伤害组重置以及粒子引擎只覆盖当前可审计机制闭环。固定 gcsim 交叉参考路径保持 `fixed-gcsim-provisional`；1.41 附近传播单独标记为 `community-provisional`，不能称为官方/官服真值或完整 gcsim 精度。尚未实现或未完整实现：
...
- `aura-v4`–`aura-v9` 已实现上述 Burning Marker、Fuel、周期伤害、Tick 火附着、归属/快照刷新和核心输出。`legacy-event-heap-v1`、v1 和 v2 继续按冻结历史语义将实际范围伤害排入后续全局 `reactionDamage`。精确 1.44–1.50 都可显式选择 `target-phase-v3`，在 owner callback 内完成列明的零延迟跨目标交付。`target-local-hitlag-v1` 造成的陈旧唤醒会先重投影到正确全局帧；该切片仍没有实现通用敌方任务所有权、完整 gcsim target phase 或官服全部边界，正延迟子反应也仍由全局 heap 处理。
...
- 感电结果完整性由 public Zod 与当前 `assertTrustedSimulationResultV150()` 复用同一组可变异证明，冻结版本继续使用各自精确历史断言。它能闭合已建模 generation、生命周期、Wane、cleanup 和时间线引用，但尚未从完整 Ability/应用/Aura 历史唯一重放全部输入；官方传播半径/目标选择、多 Aura 全排列和 Lunar 反应仍未完成，现有门不能当作官服真值、完整 EC 生命周期或 gcsim parity。
