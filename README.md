...
- Zod 严格校验、字段路径错误，以及 v0.1–1.14.0 配置迁移；当前 Schema 为 `1.15.0`、引擎为 `1.15.0-aoe-fanout`。
...
- 命中产球已支持逐击触发、AoE 逐目标 `landed / miss`、三层目标策略、按帧阶段和“至少一个目标允许回调时只产一次球”的聚合；但 Miss 与扇出目标仍由场景输入，不是由范围、位置、Boss AI 或碰撞自动求得。
- Hitlag、几何 AoE 求交、索敌、移动、真实 Boss 阶段状态机、目标死亡、护盾和特殊易伤窗口。
