...
一个以“逐段可审计、配置可迁移、结果可复现”为目标的原神队伍 DPS 模拟器。公开仓库为 [mixuanda/genshin-elemental-reaction-sim](https://github.com/mixuanda/genshin-elemental-reaction-sim)。当前完成了 Vanilla v0.1 基线冻结、纯 TypeScript 模拟核心、合法帧时间线、基础元素反应矩阵、`aura-v5` 有序 Aura 链、ReactionA/B 伤害组、燃烧 Marker/Fuel/Tick，以及绽放、草原核、烈绽放和超绽放的确定性纵向切片。1.32–1.42 先后建立玩家反应自伤、目标本地时钟、`aura-v6/v7/v8/v9`、逐元素敌方抗性、两种冻结目标相位、递归碎冰、感电 cleanup/传播/cadence 和 24 标签/16 类经典反应发布门；Lunar 反应族仍未实现。1.44 新增显式 opt-in 的 `target-phase-v3` Burning callback delivery，1.45 固定经典反应公式根，1.46 加入仅面向普通直接伤害的 Damage Group，1.47 将固定参考的数字元素施加序列接入独立 root，1.48–1.49 给 Burning Tick 和 Swirl AoE 传播施加建立反应所有策略根与版本化边界，1.50 把转化反应的 ReactionA/B 伤害组重置独立为纯 TypeScript、root-bound 的任务顺序核心，1.51 新增基础反应调度根；1.52 再为 Frozen 从正值耗尽到零的本地规范化 `Freeze Broken` 审计建立第七信任根。所有这些参考路径仍标记为 `fixed-gcsim-provisional` 或 `community-provisional`，不是官服真值或完整 gcsim parity。敌方每次实际伤害继续由核心返回结构化构成并进入既有全队、个人、技能、时间轴、逐段日志和累计/构成曲线；当前优先稳定基础反应，不扩展 UI，展示层仍只读核心结构化结果。

当前运行身份为 `schemaVersion: 1.52.0`、`engineVersion: 1.52.0-freeze-broken-attack`，`runManifest.version` 为 `1.8.0`。Manifest 同时绑定七份无 DOM 的固定根：经典反应公式根、普通直伤 Damage Group 根、数字元素施加根、配置选定的 Burning/Swirl 反应所有施加策略根、配置选定的 ReactionA/B 伤害组策略根、`basicReactionSchedulerRoot`，以及新增的 `freezeBrokenAttackRoot`。前六份 root 与 V1.51 wire/Manifest/Fixture 保持冻结。Freeze Broken V1 为 `legacy-no-freeze-broken-attack-callback` / `legacy-no-freeze-broken-attack-callback-v1` / `sha256:2831fac7a15189b772db58c245ffd8091b1128b5fd5ea516885f03a99961c838`；V2 为 `fixed-gcsim-freeze-broken-attack-normalized-v2` / `gcsim-b4ae769-freeze-broken-attack-normalized-provisional-v2` / `sha256:71646812a4061c9ef2d4ae8ca7cef1abaa79d718c8831ffaf5e3f27832955e14`。所有 `1.51.0` 及更早输入迁移到 1.52 时注入 V1，继续不产生 Freeze Broken callback/log；只有原生 current 配置可显式选择 V2。两份 root 都声明 `officialServerTruth: false` 与 `completeGcsimParity: false`。
...
上述 `13 + 4` 个文件、`275` 项定向测试是 V1.50 冻结阶段的历史记录，不是 V1.52 发布计数。它们不是完整 `npm test` 或 `npm run check` 的替代。`npm run check` 包含数据校验、类型检查、单元/性能测试和构建，但不包含 Playwright，浏览器门必须另行运行 `npm run test:e2e`。

1.51 发布前现场已通过：`npm run test:reactions` 为 22 个文件、300/300；完整 unit 为 139 个文件、1679/1679；性能 3/3；数据、typecheck、build 与 Chromium Playwright 36/36 均通过。完整 `npm run check` 顺序中的三项中位数为 29.370ms、5.195ms、93.471ms；这是冻结的历史记录。V1.52 本轮现场已通过：`npm run test:reactions` 29 个文件、372/372；完整 unit 146 个文件、1752/1752；数据目录 120 个角色、125 套天赋、762 个技能与被动、237 把武器；typecheck、性能 3/3、production build 与 Chromium Playwright 36/36。最终 `npm run check` 中三项性能中位数为 25.975ms、4.971ms、79.773ms；build 仍有主 JS chunk 超过 500kB 的非阻断警告。既有页面继续只读核心返回的默认指标、角色/技能构成和伤害曲线，当前没有新增 UI。
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.51.0 配置迁移；当前 Schema 为 `1.52.0`、引擎为 `1.52.0-freeze-broken-attack`、Manifest 为 `1.8.0`。V151 及更早契约保持冻结；1.51→1.52 只新增 V1 `freezeBrokenAttackModel` 并推进身份。无版本和所有旧版输入都迁移为 1.52 + Freeze Broken V1，不会被静默升级到 V2；原生 current 配置才可显式选择 V2。迁移原样保留公式、普通直伤 Damage Group、Aura、传播、基础调度、目标任务、目标时钟、玩家模型和反应交付模式，不改默认伤害、不猜测 fixed group，也不自动物化参考合成攻击。精确 V151/V150/V149/V148/V147/V146/V145/V144/V142 输入、结果与 Manifest 保持冻结 legacy 身份；历史输入夹带未来字段会 fail-closed。
...
- 每次 current 运行返回 Manifest 1.8.0，固定输入 Schema/引擎/数据版本、配置哈希、解析后的运行选项、有序插件身份、前六份冻结 root、新增 `freezeBrokenAttackRoot` 和 `gdl-v2-fnv1a32-*` 复现键；冻结 V151 Manifest 仍是 1.7.0，V150 仍是 1.6.0，更早版本保持各自精确身份。当前 32-bit FNV-1a 只用于漂移检测，可碰撞，不能充当密码学签名。
- 输出侧当前使用精确 `simulationResultV152Schema`，同时保留身份严格的冻结 `simulationResultV151Schema` 及更早版本。V152 在 V151 全部证明之上，闭合 `freezeBrokenAttackModel`、Manifest root、Frozen 耗尽来源与 audit row；删除、重复、换序、伪造正触发或给负例添加行会被 public Zod 与 `assertTrustedSimulationResultV152()` 拒绝。触发行明确保持 audit-only、`damageEventId: null`、`hitResolutionLogId: null`，所以它不是伤害或命中记录。七份 root 仍不证明完整 Aura、ICD、callback bus、Mona bubble/impulse、参考 RNG、动作快照输入来源、角色数据库、官服真值或完整 gcsim 精度。
...
- 1.32–1.51 的版本化模型和历史 Fixture 均保持冻结；1.52 只新增 Freeze Broken V1/V2 root、规范化 audit-only 行、对应 Schema/完整性门和独立 Golden。所有 Fixture 都是回归证据，不是官方数值认证或完整 gcsim parity 报告；测试抗性、草原核寿命、固定参考路径、附近传播半径和默认杜林预设的示例魔法数仍是 `provisional`、`community-provisional` 或 `fixed-gcsim-provisional`。

## 当前精度边界

本版本不是 gcsim 精度实现。`legacy-v0.1` 继续保持 Golden 兼容；`aura-v1`–`aura-v9`、三种目标相位、碎冰递归交付、感电 cleanup/传播/cadence、1.45 经典反应公式根、1.46 普通直伤 Damage Group、1.47 数字元素施加 ICD、1.48 Burning/Swirl 反应所有施加、1.49 施加重置边界、1.50 ReactionA/B 伤害组重置、1.51 基础反应调度根、1.52 Freeze Broken audit root 以及粒子引擎只覆盖当前可审计机制闭环。1.51 调度根当前窄覆盖 Burning/Swirl，不是通用 barrier；1.52 只记录正值到耗尽的 Frozen provenance，不执行固定参考 callback。Burning 的 monotonic generation guard 是本项目避免陈旧 callback 复活的安全偏差，不声称与固定 gcsim 的 frame-token 重启行为完全相同。固定 gcsim 交叉参考路径保持 `fixed-gcsim-provisional`；1.41 附近传播单独标记为 `community-provisional`，不能称为官方/官服真值或完整 gcsim 精度。尚未实现或未完整实现：
...
- `aura-v4`–`aura-v9` 已实现上述 Burning Marker、Fuel、周期伤害、Tick 火附着、归属/快照刷新和核心输出。`legacy-event-heap-v1`、v1 和 v2 继续按冻结历史语义将实际范围伤害排入后续全局 `reactionDamage`。精确 1.44–1.52 都可显式选择 `target-phase-v3`，在 owner callback 内完成列明的零延迟跨目标交付。`target-local-hitlag-v1` 造成的陈旧唤醒会先重投影到正确全局帧；该切片仍没有实现通用敌方任务所有权、完整 gcsim target phase 或官服全部边界，正延迟子反应也仍由全局 heap 处理。
...
- 感电结果完整性由 public Zod 与当前 `assertTrustedSimulationResultV152()` 复用同一组可变异证明，冻结 V151 及更早版本继续使用各自精确历史断言。它能闭合已建模 generation、生命周期、Wane、cleanup 和时间线引用，但尚未从完整 Ability/应用/Aura 历史唯一重放全部输入；官方传播半径/目标选择、多 Aura 全排列和 Lunar 反应仍未完成，现有门不能当作官服真值、完整 EC 生命周期或 gcsim parity。
...
项目借鉴 gcsim 的逐帧、逐事件审计与版本化配置思路。经典反应公式、1.46 普通直伤 Damage Group 与 1.47 数字元素施加 sequence 分别作为独立 root 固定到提交 `b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541`；两种 ICD 数据即使拥有相似 Group 名称也不能混用。元素施加 root 保留 58 组来源数据，但公共直接命中排除引擎拥有的 `reaction-a`、`reaction-b`、`burning`，且反应派生应用仍未统一进入 1.47 日志。固定提交的经典感电路径使用单目标命中，所以 1.41 `nearby-wet-radius-v1` 仍是默认关闭的 `community-provisional` 扩展。Hitlag、抗性和大量场景数据仍由用户显式输入；固定源码自身也保留 TODO。因此固定路径只能称 `fixed-gcsim-provisional`，附近传播只能称 `community-provisional`，两者都不是官方数值、官服实测证明或完整 gcsim 精度。详见 [THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md)。
