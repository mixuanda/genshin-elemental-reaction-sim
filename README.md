...
只验证 1.42 反应核心、24 标签/16 类经典反应发布门、感电单目标兼容/附近湿目标传播、`aura-v9` 全局 cadence safety、逐元素敌方抗性、Quicken→Bloom 核心任务、`aura-v8` cleanup、冻结的 1.37 v1 目标相位、1.38 v2 callback→`Reactable.Tick` 边界、递归碎冰交付、增幅覆盖闸门、玩家自伤、目标本地时钟、可靠 Schema 与兼容 Golden 时可运行：
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.41.0 配置迁移；当前 Schema 为 `1.42.0`、引擎为 `1.42.0-ec-global-cadence-safety`。1.35→1.41 的历史迁移契约继续冻结；1.41→1.42 只更新运行身份，原样保留 `reactionEngine`（包括 `aura-v8`）、`electroChargedPropagationModel`（默认仍为 `single-target-v1`）、目标任务、目标时钟、玩家模型与伤害交付模式，不会自动启用 `aura-v9` 或 `nearby-wet-radius-v1`。历史输入不得夹带未来字段，迁移也绝不自动启用递归碎冰、`aura-v8/v9` 或附近湿目标传播。逐元素抗性仍从 1.35 起才是合法 wire contract，旧版本夹带该字段会 fail-closed。
...
- `targetTaskModel` 与目标时钟、Aura 版本分别建模，并提供三种模式：`legacy-event-heap-v1` 是历史迁移和当前兼容预设的默认值；`target-phase-v1` 是冻结的 1.37 第一批 Burning callback→Aura 衰减契约；`target-phase-v2` 是 1.38 的独立显式 opt-in。v1/v2 都要求运行时使用 `legal-frame-v1`；历史 v1/v2 继续使用 `aura-v7`，精确 1.40–1.42 的 `aura-v8` 以及精确 1.42 的 `aura-v9` 只允许 60 FPS 的 v2。v2 只保证同一目标边界内 QueueEnemyTask / 目标 callback 先于该目标 `Reactable.Tick`，不声称建立全目标 barrier，也不会把随后 core work 搬入目标相位。Hitlag 造成的陈旧唤醒会先按目标本地时钟重投影；`aura-v9` 的 F+10/F+70 感电 callback 和后续 Wane 仍使用全局帧。冻结的 v1 使用 `targetTaskPhaseLog`；v2 使用相互排斥的 `targetPhaseLog`。
...
- 输出侧现有精确 `1.42` 的 `simulationResultV142Schema`：65 个顶层字段和 DamageEvent、ReactionAudit、反应/状态生命周期、行动、能量/粒子、汇总、曲线、Aura 边界及合法时间线叶节点全部使用 strict Zod 结构；未知字段、非有限数、与当前已建模判别联合不匹配的分支及历史结果身份会 fail-closed。统一完整性层还会闭合 result/config/manifest 身份与别名、逐击 ID/队列顺序/个位显示/伤害构成、命中与已注册机制的主要反应反链、总伤/DPS/反应命中、角色/目标/技能/逐秒汇总和伤害曲线。外部 JSON、持久化的完整结果及未来新增的完整 `SimulationResult` 测试向量必须走完整 Schema；现有仅含冻结摘要的 Golden 不是完整结果 wire。`sim-core` 内部结果已经过 TypeScript 构造及 Burning/Quicken/Bloom、ReactionA/B、草原核、玩家伤害、目标时钟/Hitlag、反应交付和 EC 生命周期等深层 facet，因此在公开返回边界复用跨字段完整性规则做零拷贝可信断言，避免每次模拟深拷贝数 MB 日志；该内部断言不等同于完整 Zod parse，不能用于接纳外部或持久化 wire。Schema 证明输出结构和已注册的不变量，不代替公式、Aura 语义、角色数据或 gcsim 精度验证。
...
- 1.32 增加显式 `playerDamageModel`，1.33 增加显式 `targetClockModel`，1.34 增加显式 opt-in `aura-v6`，1.35 增加可选的完整敌方八项抗性表，1.36 增加显式 opt-in `aura-v7` 与核心 `reactionTaskLog`，1.37 再增加显式 `targetTaskModel` 与 `targetTaskPhaseLog`，1.38 增加 `target-phase-v2` 与独立 `targetPhaseLog`，1.39 增加显式 `reactionDeliveryModel`，1.40 增加显式 opt-in `aura-v8` cleanup，1.41 增加显式 `electroChargedPropagationModel`，1.42 增加显式 opt-in `aura-v9` 全局 cadence safety。1.35→1.41 继续按冻结历史契约迁移；1.40→1.41 注入 `single-target-v1`，1.41→1.42 保留 `aura-v8` 与 `single-target-v1`，两步都不自动启用附近传播或 `aura-v9`。所有历史 Fixture 均不得回写；每一版本只能新增独立 Fixture。所有 Fixture 都是回归证据，不是官方数值认证或完整 gcsim parity 报告；其中测试抗性、草原核寿命、固定 gcsim 路径、附近传播半径和默认杜林预设的示例魔法数都仍是 `provisional`、`community-provisional` 或 `fixed-gcsim-provisional`。

## 当前精度边界

本版本不是 gcsim 精度实现。`legacy-v0.1` 继续保持 Golden 兼容；`aura-v1`–`aura-v9`、冻结的 `target-phase-v1`、受限的 `target-phase-v2`、1.39 的碎冰递归交付切片、1.40 的 EC next-target-Tick cleanup、1.41 默认关闭的附近湿目标传播、1.42 的 EC 全局 cadence safety 和粒子引擎只覆盖当前可审计机制闭环。v2 只声明目标 callback→该目标 `Reactable.Tick`，递归交付也只声明零延迟碎冰父子链；1.40 cleanup 只处理 Quicken→Bloom 耗尽同代水 Aura 的明确边界；1.41 附近传播只提供显式半径下的逐目标伤害与候选审计；1.42 只冻结传统感电的 F+10/F+70 全局排程、目标 Hitlag、恢复、Wane 与 cleanup 所有权。它们都不能外推为完整目标相位、通用任务所有权、全部反应同步语义或完整感电传导。固定 gcsim 交叉参考路径保持 `fixed-gcsim-provisional`；1.41 附近传播单独标记为 `community-provisional`，不能称为官方/官服真值或完整 gcsim 精度。尚未实现或未完整实现：
...
- 1.42 会从配置、行动、命中确认、粒子事件和固定回能逐点重放 `energyLog`、`energyStats` 与整条 `energyCurve`，但仍保留其冻结的“同帧行动先于粒子、行动开始立即扣能”语义。固定 gcsim 提交 `ef41805d855a60b9e1035293584b85c085dc69e7` 则先处理已排定的全局粒子任务，并按角色定义的延迟帧扣爆发能量；下一版必须用显式 `energyTaskModel`、调度序号和新 Golden 实现，不能回写 1.42 或宣称当前已经同 gcsim 对齐。
