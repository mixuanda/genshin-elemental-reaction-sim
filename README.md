...
只验证 1.39 反应核心、逐元素敌方抗性、Quicken→Bloom 核心任务、冻结的 1.37 v1 目标相位、1.38 v2 callback→`Reactable.Tick` 边界、递归碎冰交付、增幅覆盖闸门、玩家自伤、目标本地时钟、可靠 Schema 与兼容 Golden 时可运行：
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.38.0 配置迁移；当前 Schema 为 `1.39.0`、引擎为 `1.39.0-shatter-recursive-delivery`。1.35→1.36 的 identity-only 迁移仍按历史契约保留；1.36→1.37 为此前输入补入 `targetTaskModel: { mode: "legacy-event-heap-v1" }`；1.37→1.38 精确保留 `legacy-event-heap-v1` 或 `target-phase-v1`，绝不自动升级到 `target-phase-v2`；1.38→1.39 原样保留目标任务模式，并补入 `reactionDeliveryModel: { mode: "deferred-event-heap-v1" }`。历史输入不得夹带 1.39 字段，迁移也绝不自动启用递归碎冰。迁移继续保留 `reactionEngine`、`playerDamageModel`、`targetClockModel`、逐元素敌方抗性和其余配置；1.35 及更早配置继续遵守各自历史模式的保留/禁用规则，逐元素抗性仍从 1.35 起才是合法 wire contract，旧版本夹带该字段会 fail-closed。
...
- `reactionDeliveryModel` 独立控制反应伤害的交付顺序。`deferred-event-heap-v1` 是所有现有预设和迁移配置的兼容模式；`shatter-recursive-zero-delay-v1` 只允许精确的 1.39 身份、`legal-frame-v1`、60 FPS 和 `aura-v7` 显式 opt-in。递归模式在同一帧、目标和来源上下文中先交付碎冰子伤害，再交付其直接或嵌套反应父伤害；`parentDamageEventId` 因而可以合法前向引用。严格结果 Schema 要求连续 ID、无环父链，以及每个递归碎冰子段与唯一 `reactionDamageLog` 的 reciprocal 引用。该模式只覆盖零延迟碎冰，不会把其他反应或目标任务自动改为同步递归。
...
- Quicken→Bloom 跟进若消耗掉水雷共存中的水 Aura，当前还没有在下一次有效目标 `Reactable.Tick` 完成感电流清理；该边界计划放入新的 `aura-v8` / 1.40 契约。已经排队的首次感电伤害如何保留、同帧补水如何维持同一代次、Hitlag 如何重投影和活动来源如何清理，都必须先由新测试冻结，不能静默修改 `aura-v7` 或 1.38 Golden。
