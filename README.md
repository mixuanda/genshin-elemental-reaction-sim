...
- Zod 严格校验、字段路径错误，以及 v0.1 / 1.0.0 / 1.1.0 / 1.2.0 / 1.3.0 配置迁移；当前 Schema 为 `1.4.0`、引擎为 `1.4.0-action-states`。
...
- `AbilityBlueprint` 1.1 把技能倍率引用、命中帧、附着、ICD、固定回能、产球、行动状态、前置条件、未实现机制和逐项证据放入严格 Schema；1.0 Blueprint 可迁移，通用编译器默认拒绝 `partial`。
- 内置“杜林黑 E · 部分机制审计向量”：精质转变 6 秒窗口会被核心授予/强制前置/消耗，随后进入 30 秒黑度之否；天赋 10 级三段倍率 `1.30032 / 0.9576 / 1.16352`、命中帧 `48 / 53 / 58`、DurinSkill 自定义 ICD、33 固定回能和 4 火粒子均由核心输出，并在网页逐段、行动状态表、伤害曲线、Aura 曲线和能量曲线中展示。
