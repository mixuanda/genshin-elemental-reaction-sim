...
- Zod 严格校验、字段路径错误，以及 v0.1 / 1.0.0 / 1.1.0 / 1.2.0 / 1.3.0 / 1.4.0 / 1.5.0 / 1.6.0 / 1.7.0 / 1.8.0 / 1.9.0 配置迁移；当前 Schema 为 `1.10.0`、引擎为 `1.10.0-timeline-state-clears`。
...
- 命中产球已支持逐击触发和精确内部冷却边界，但当前单目标模型把进入伤害日志视为命中可受击敌人；Miss、无敌目标、多目标各自回调和逐目标产球仍未建模。杜林黑/白爆发、命座和专属装备效果尚未完成；黑/白状态目前只完成互斥与合法性审计，尚未驱动全部被动效果。
