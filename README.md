...
- Zod 严格校验、字段路径错误，以及 v0.1 / 1.0.0 / 1.1.0 / 1.2.0 / 1.3.0 / 1.4.0 / 1.5.0 / 1.6.0 / 1.7.0 / 1.8.0 配置迁移；当前 Schema 为 `1.9.0`、引擎为 `1.9.0-movement-commands`。
...
- `AbilityBlueprint` 1.6 把技能倍率引用、命中帧、分后续动作取消帧、附着、ICD、固定回能及其内部冷却、命中产球及其内部冷却、行动状态、前置条件、未实现机制和逐项证据放入严格 Schema；1.0 / 1.1 / 1.2 / 1.3 / 1.4 / 1.5 Blueprint 可迁移，通用编译器默认拒绝 `partial`。
...
- 尚未给全角色填入经过实测核验的命中帧、逐后续动作取消帧、动画结束帧和冷却数据；杜林只映射了指定 E/黑分支。冲刺/跳跃目前要求配置显式给出占用帧，不模拟耐力、位移、无敌帧、碰撞、落地或真实动作帧。
