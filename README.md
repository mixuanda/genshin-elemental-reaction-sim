...
只验证 1.34 反应核心、玩家自伤、目标本地时钟、可靠 Schema 与兼容 Golden 时可运行：
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.33.0 配置迁移；当前 Schema 为 `1.34.0`、引擎为 `1.34.0-general-reaction-order`。1.33 配置迁移只更新 Schema/引擎身份，精确保留原 `reactionEngine`、`playerDamageModel` 与 `targetClockModel`；1.32 配置保留原玩家模型并补入禁用的目标时钟，1.31 及更早配置继续保留各自反应模式并得到禁用的玩家模型与目标时钟。迁移不会静默切换到 `aura-v6` 或改变历史时间线。
...
- `aura-v6` 是独立 opt-in，继承 v5 的 Aura 耐久、Burning、Bloom 与草原核语义，并为雷元素来袭按固定 gcsim 提交 `b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541` 结算 `超激化 → 超载 → 感电 → 冻结底超导 → 普通超导 → 原激化 → 绽放`。各消费分支共享同一来袭 Gauge，不会为同一命中重复创建元素量；同击产生多个转化反应时，完整有序结果写入 `transformativeReactions`，旧的单值 `transformativeReaction` 只保留第一项作为兼容投影。该顺序标记为 `fixed-gcsim-provisional`；参考源码本身对反应顺序保留 TODO，因此它是固定代码路径的可复现契约，不是官服真值。v1–v5 输出与历史 Golden 不变。
...
- `legacy-default-120s.golden.json` 继续冻结 Vanilla 的原始浮点结果；`legacy-default-120s-1.33.golden.json` 保留 1.33 历史投影，`legacy-default-120s-1.34.golden.json` 冻结当前 1.34 兼容投影。
- `burning-aura-v4-1.30.golden.json` 冻结 1.30 Burning 兼容切片；1.33 及更早身份继续作为历史证据保留，1.34 当前身份以新 Fixture 为准。
- `reaction-matrix-1.31.golden.json`、`reaction-matrix-1.32.golden.json` 与 `reaction-matrix-1.33.golden.json` 保留历史 14 个 `legal-frame-v1 + aura-v5` 向量；`reaction-matrix-1.34.golden.json` 冻结当前 15 个反应向量和 Schema/引擎身份，并继续让兼容向量保持 `targetClockModel: disabled`。Hitlag 与 `aura-v6` 雷多反应链分别由独立单元/集成向量冻结。
- 1.32 增加显式 `playerDamageModel`，1.33 增加显式 `targetClockModel`，1.34 只新增显式 opt-in `aura-v6`；1.33 迁移不会更改上述模型或 Aura 模式。兼容模式的敌方逐击、总伤、DPS、角色/技能聚合、事件顺序和旧 Golden 均不得变化。所有 Fixture 都是回归证据，不是官方数值认证或完整 gcsim parity 报告；其中 10% 玩家抗性只是测试输入。
...
- 敌方基础抗性目前仍是一个标量，并由目标默认规则投影到各伤害元素；尚未建立逐元素敌方基础抗性输入。减抗/减防 Debuff 当前也作用于所有已注册目标，尚不能声明只影响单个敌人。
