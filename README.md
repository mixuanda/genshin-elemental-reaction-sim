...
一个以“逐段可审计、配置可迁移、结果可复现”为目标的原神队伍 DPS 模拟器。当前完成了 Vanilla v0.1 基线冻结、纯 TypeScript 模拟核心、合法帧时间线、基础元素反应矩阵、`aura-v5` 有序 Aura 链、ReactionA/B 伤害组、燃烧 Marker/Fuel/Tick，以及绽放、草原核、烈绽放和超绽放的确定性纵向切片。1.32–1.42 先后建立玩家反应自伤、目标本地时钟、`aura-v6/v7/v8/v9`、逐元素敌方抗性、两种冻结目标相位、递归碎冰、感电 cleanup/传播/cadence 和 24 标签/16 类经典反应发布门；Lunar 反应族仍未实现。1.44 新增显式 opt-in 的 `target-phase-v3` Burning callback delivery，1.45 固定经典反应公式根，1.46 加入仅面向普通直接伤害的 Damage Group，1.47 再把固定参考的数字元素施加序列接入独立 root、显式 selector、目标级状态机和结果完整性门。所有这些参考路径仍标记为 `fixed-gcsim-provisional` 或 `community-provisional`，不是官服真值或完整 gcsim parity。敌方每次实际伤害继续由核心返回结构化构成并进入既有全队、个人、技能、时间轴、逐段日志和累计/构成曲线；本轮无 UI 扩展，展示层仍只读核心结构化结果。

当前运行身份为 `schemaVersion: 1.47.0`、`engineVersion: 1.47.0-elemental-application-icd-root`，`runManifest.version` 为 `1.3.0`。Manifest 同时绑定三份无 DOM 的固定根：经典反应公式根 `gcsim-b4ae769-classic-provisional-v1` / `sha256:7ae4ee955e0c7986c47931cff596694c8cd4754b48df90e0ad1cf092738ccafd`，普通直伤 Damage Group 根 `gcsim-b4ae769-damage-groups-provisional-v1` / `sha256:7e6d16a2a90ac7d9bb84daa80c43f09d28fb65e45319c62f67d14c50bb5e9c70`，以及数字元素施加根 `gcsim-b4ae769-elemental-application-provisional-v1` / `sha256:df461cf8aefee33ec57b8a8f83e2ec26497f17be8bc3ee1e6d667bf91d4015c1`。三者都固定来源 `genshinsim/gcsim@b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541`，且明确携带 `officialServerTruth: false` 与 `completeGcsimParity: false`。1.46→1.47 迁移只更新身份、注入 `elementalApplicationIcdModel`，并把旧应用显式投影为 No ICD 或 legacy boolean selector；它不会静默切换到固定数字组、给技能猜测 Tag/Group、改变默认伤害，或自动启用其他机制。精确 V146/V145/V144/V142 wire、Manifest 和 Fixture 继续按各自身份冻结。
...
只验证当前 1.47 三信任根、数字元素施加 ICD、普通直伤 Damage Group、24 标签/16 类经典反应发布门、感电单目标兼容/附近湿目标传播、`aura-v9` 全局 cadence safety、逐元素敌方抗性、Quicken→Bloom 核心任务、`aura-v8` cleanup、冻结的目标相位、v3 Burning callback 零延迟跨目标交付、递归碎冰交付、可靠 Schema 与兼容 Golden 时可运行：
...
本轮最终现场门：`npm run test:unit` 为 106 个文件、1446 项测试全部通过；`npm run test:performance` 为 3/3 通过；`npm run data:check`、`npm run typecheck` 和 `npm run build` 通过。`npm run test:e2e` 在 Chromium 中 36/36 通过；独立浏览器检查同时确认页面 HTTP 200、无 console error/错误覆盖层，并正常渲染默认预设的指标、角色/技能构成和两条伤害 Canvas 曲线。构建仍有单个主 JS chunk 超过 500 kB 的非阻断警告。
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.46.0 配置迁移；当前 Schema 为 `1.47.0`、引擎为 `1.47.0-elemental-application-icd-root`、Manifest 为 `1.3.0`。1.46→1.47 会注入固定 `elementalApplicationIcdModel`，把旧 `no-icd` 显式迁移为 `no-icd-v1`，把其余旧组迁移为 `legacy-boolean-profile-v1`，并原样保留公式、Damage Group、Aura、传播、目标任务、目标时钟、玩家模型和反应交付模式。它不会把旧组静默解释为固定数字组，也不会改变默认数值或自动启用未核验技能映射。精确 V146/V145/V144/V142 输入、结果与 Manifest 保持冻结 legacy 身份；历史输入夹带未来字段会 fail-closed。
...
- 1.47 的 `packages/sim-core/src/elemental-application-icd.ts` 为每个目标运行独立状态机。当前应用形状为 `{ gaugeUnits, icd }`，selector 必须是 No ICD、显式 legacy boolean profile 或固定 gcsim numeric group；带应用的 physical 命中（包括继承缩放角色 physical 元素的命中）在 Schema 阶段拒绝。`elementalApplicationIcdLog` 为每个配置了应用的直接命中目标尝试写一行，包含 selector、名义/有效元素量、窗口和 reciprocal 命中/伤害外键；Miss、Aura 被阻断、无 Aura 引擎和先前机制截断分别写明确 skip reason，且不会推进窗口。反应派生附着尚未纳入这个统一日志。
...
- `targetTaskModel` 与目标时钟、Aura 版本分别建模，并提供四种模式：`legacy-event-heap-v1` 是历史迁移和当前兼容预设的默认值；`target-phase-v1`、`target-phase-v2` 保持冻结历史语义；`target-phase-v3` 允许精确 1.44、1.45、1.46 或当前 1.47 在 `legal-frame-v1 + 60 FPS + aura-v7/v8/v9` 门下显式选择。v3 只把已建模 Burning Tick 的跨目标范围命中作为 callback 所有的零延迟微事件交付，正延迟子反应、感电 callback/Wane 和其他 core work 仍留在各自队列；它不是通用全目标 barrier。
...
- 每次运行返回 Manifest 1.3.0，固定输入 Schema/引擎/数据版本、配置哈希、解析后的运行选项、有序插件身份、`reactionFormulaRoot`、`directDamageGroupRoot`、`elementalApplicationIcdRoot` 和 `gdl-v2-fnv1a32-*` 复现键；冻结 V146/V145/V144/V142 Manifest 保持各自精确版本。当前 32-bit FNV-1a 只用于漂移检测，可碰撞，不能充当密码学签名。
- 输出侧当前使用精确 `simulationResultV147Schema`，同时保留身份严格的冻结 `simulationResultV146Schema`、`simulationResultV145Schema`、`simulationResultV144Schema` 与 `simulationResultV142Schema`。V147 在既有公式、Damage Group、Aura/反应及跨日志证明之上，从配置和固定 application profile 独立重放每个直接命中目标尝试、skip/evaluated 决策、状态窗口、数字倍率、名义/有效 Gauge 与 reciprocal 外键；删除、重复、换序或协调篡改会被 public Zod 与 `assertTrustedSimulationResultV147()` 拒绝。反应派生应用仍不在这份 1.47 重放范围内，三份固定 root 也不证明完整 Aura、动作快照输入来源、角色数据库或 gcsim 精度。
...
- 1.32–1.46 的版本化模型和历史 Fixture 均保持冻结；1.47 只追加数字元素施加 root、显式 selector、运行日志和独立 Golden。所有 Fixture 都是回归证据，不是官方数值认证或完整 gcsim parity 报告；测试抗性、草原核寿命、固定参考路径、附近传播半径和默认杜林预设的示例魔法数仍是 `provisional`、`community-provisional` 或 `fixed-gcsim-provisional`。

## 当前精度边界

本版本不是 gcsim 精度实现。`legacy-v0.1` 继续保持 Golden 兼容；`aura-v1`–`aura-v9`、三种目标相位、碎冰递归交付、感电 cleanup/传播/cadence、1.45 经典反应公式根、1.46 普通直伤 Damage Group、1.47 数字元素施加 ICD 和粒子引擎只覆盖当前可审计机制闭环。固定 gcsim 交叉参考路径保持 `fixed-gcsim-provisional`；1.41 附近传播单独标记为 `community-provisional`，不能称为官方/官服真值或完整 gcsim 精度。尚未实现或未完整实现：

- 1.47 已固定数字 elemental-application sequence，并保留纳西妲 Skill 的 `1.5` 数值槽；但这份 root 只对配置的普通直接命中目标尝试提供统一日志。Burning Tick、扩散传播等反应拥有或派生的元素施加仍沿用各自冻结管线，尚未统一进入 `elementalApplicationIcdLog`。58 组来源表也不等于 58 组已批量绑定到全角色技能：公共直接命中只能选 55 组，且当前只有合成向量和少量来源核验 Ability 使用固定 selector。

- `aura-v4`–`aura-v9` 已实现上述 Burning Marker、Fuel、周期伤害、Tick 火附着、归属/快照刷新和核心输出。`legacy-event-heap-v1`、v1 和 v2 继续按冻结历史语义将实际范围伤害排入后续全局 `reactionDamage`。精确 1.44、1.45、1.46 与当前 1.47 都可显式选择 `target-phase-v3`，在 owner callback 内完成列明的零延迟跨目标交付。`target-local-hitlag-v1` 造成的陈旧唤醒会先重投影到正确全局帧；该切片仍没有实现通用敌方任务所有权、完整 gcsim target phase 或官服全部边界，正延迟子反应也仍由全局 heap 处理。
...
- 感电结果完整性由 public Zod 与当前 `assertTrustedSimulationResultV147()` 复用同一组可变异证明，冻结版本继续使用各自精确历史断言。它能闭合已建模 generation、生命周期、Wane、cleanup 和时间线引用，但尚未从完整 Ability/应用/Aura 历史唯一重放全部输入；官方传播半径/目标选择、多 Aura 全排列和 Lunar 反应仍未完成，现有门不能当作官服真值、完整 EC 生命周期或 gcsim parity。
