...
- Zod 严格校验、字段路径错误，以及 v0.1–1.16.0 配置迁移；当前 Schema 为 `1.17.0`、引擎为 `1.17.0-target-motion`。
...
- 单个命中也可用 `geometry: { kind: "circle", origin, radius }` 对全部具备位置的注册目标自动求交；核心先按命中帧解析目标移动位置，再按 `目标中心距离 <= 攻击半径 + 目标碰撞半径` 判定，并把命中时位置、圆心、半径、距离、阈值和范围外原因写入逐击日志。`targeting` 与 `geometry` 互斥。
...
- 命中产球已支持逐击触发、显式 fanout 或圆形几何得到的逐目标 `landed / miss`、三层目标策略、按帧阶段和“至少一个目标允许回调时只产一次球”的聚合；目标初始坐标、线性移动分段、攻击圆心和半径仍由场景声明。
- 当前几何只支持二维圆与圆形碰撞体，目标移动只支持预先声明的线性分段；尚无扇形、矩形、线段、高度、角色移动、追踪弹道、AI 驱动敌人移动、索敌、障碍物、Hitlag、真实 Boss 阶段状态机、目标死亡、护盾和特殊易伤窗口。
...
合法时间线对每个有能量消耗的命令按顺序执行确定性前缀探测，再用已确认的失败集合重编译最终时间线；因此失败行动不会推进游标、预占冷却或改变状态，后续命令会按新帧重新执行。它仍不是 gcsim 式单遍动态行动执行器：命中确认分支、目标死亡、AI/命令驱动移动、运行时空间分支和玩家条件语句尚未进入命令语言。Aura/ICD 已按注册目标隔离，圆形 AoE 与声明式目标线性移动已能共同求交，但仍没有 Hitlag 和完整角色动作/ICD 数据库。
