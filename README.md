...
当前运行身份为 `schemaVersion: 1.45.0`、`engineVersion: 1.45.0-reaction-formula-root`，`runManifest.version` 为 `1.1.0`。1.45 将经典反应算术固定到 `gcsim-b4ae769-classic-provisional-v1`：来源提交 `b4ae769d7c1c1bce68fce5faf0b460c5b5b7f541`，规范化公式内容 SHA-256 为 `sha256:7ae4ee955e0c7986c47931cff596694c8cd4754b48df90e0ad1cf092738ccafd`。配置中的 `reactionFormulaModel`、Manifest 中的完整 `reactionFormulaRoot`、配置哈希和复现键必须互相闭合；1.44→1.45 迁移只增加当前身份与固定公式选择，不改变任何伤害数值或历史机制模式。精确 1.44 结果仍由 V144 Schema 和 Manifest 1.0.0 作为冻结 legacy wire 接受，旧 Fixture 不被回写。这个公式根只证明已注册经典反应的等级基准、倍率和公式投影绑定到该固定参考，不是官服真值，也不是完整 gcsim parity；它尚未证明完整 Aura/ICD、动作快照中 EM/反应增伤的来源、全角色技能命中数据库或 Lunar 反应。

当前开发顺序以经典基础反应、Aura 生命周期与可变异完整性门为先；这些基础机制稳定前，不新增专用展示面板或曲线样式。现有网页继续展示核心已经返回的全队、个人、技能、时间轴、逐段伤害、构成与曲线，但不在浏览器中二次计算伤害或反应。
...
只验证当前 1.45 公式信任根、24 标签/16 类经典反应发布门、感电单目标兼容/附近湿目标传播、`aura-v9` 全局 cadence safety、逐元素敌方抗性、Quicken→Bloom 核心任务、`aura-v8` cleanup、冻结的 1.37 v1 目标相位、1.38 v2 callback→`Reactable.Tick` 边界、1.44/1.45 v3 Burning callback 零延迟跨目标交付、递归碎冰交付、增幅覆盖闸门、玩家自伤、目标本地时钟、可靠 Schema 与兼容 Golden 时可运行：

```bash
npx vitest run packages/reaction-formulas/src/profile.test.ts packages/sim-core/src/__tests__/reaction-formula-profile.test.ts packages/sim-core/src/__tests__/reaction-formula-manifest.test.ts packages/sim-core/src/__tests__/reaction-formula-integrity.test.ts packages/sim-core/src/__tests__/target-phase-v3-formula-root.test.ts
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.44.0 配置迁移；当前 Schema 为 `1.45.0`、引擎为 `1.45.0-reaction-formula-root`。1.35→1.44 的历史迁移契约继续冻结；1.44→1.45 只更新运行身份并注入固定的 `reactionFormulaModel`，原样保留 `reactionEngine`、`electroChargedPropagationModel`、目标任务、目标时钟、玩家模型与伤害交付模式，不会改变数值或自动启用 `target-phase-v3`、`aura-v9`、递归碎冰或 `nearby-wet-radius-v1`。精确 V144 输入/结果与 Manifest 1.0.0 保持冻结 legacy 身份；当前 V145 Manifest 为 1.1.0。历史输入不得夹带 `reactionFormulaModel` 等未来字段，1.43 仍只是未发布 energy wire 的保留版本号。逐元素抗性仍从 1.35 起才是合法 wire contract，旧版本夹带该字段会 fail-closed。
...
- 每次运行返回 `runManifest`：当前版本 1.1.0 固定输入 Schema/引擎/数据版本、版本化配置哈希、解析后的运行选项、按顺序排列且带内容哈希的插件身份、完整 `reactionFormulaRoot` 和 `gdl-v2-fnv1a32-*` 复现键；冻结的 V142/V144 Manifest 继续使用精确 1.0.0 结构。FNV-1a 只用于确定性漂移检测，不是密码学完整性或签名；声明式效果由核心规范化后计算内容哈希，任意代码插件的 descriptor/contentHash 则仍属于插件作者提供的受信声明，不能把它当作代码真实性证明。
- 输出侧当前使用精确 `1.45` 的 `simulationResultV145Schema`，同时保留身份严格的冻结 `simulationResultV144Schema` 与 `simulationResultV142Schema`。65 个顶层字段和 DamageEvent、ReactionAudit、反应/状态生命周期、行动、能量/粒子、汇总、曲线、Aura 边界及合法时间线叶节点全部使用 strict Zod 结构；未知字段、非有限数、与当前已建模判别联合不匹配的分支及历史结果身份会 fail-closed。统一完整性层继续闭合 result/config/manifest 身份与别名、逐击 ID/队列顺序/个位显示/伤害构成、命中与已注册机制的主要反应反链、总伤/DPS/反应命中、角色/目标/技能/逐秒汇总和伤害曲线；冻结 V144 的 v3 callback 完整性门保持原样。V145 另要求配置选择、Manifest 公式根、来源角色等级和反应交付算术与固定 profile 一致，并拒绝通过改写增幅底数、转化等级基准/倍率、扩散 self/propagation 通道或激化加算倍率来协同伪造结果。外部 JSON、持久化的完整结果及未来新增的完整 `SimulationResult` 测试向量必须走完整 Schema；现有仅含冻结摘要的 Golden 不是完整结果 wire。`sim-core` 内部结果在公开返回边界运行 `assertTrustedSimulationResultV145()`（当前别名也指向 V145）复用跨字段完整性规则；冻结 V144 断言只服务其历史身份。该零拷贝内部断言不等同于完整 Zod parse，不能用于接纳外部或持久化 wire。公式根和 Schema 证明已注册投影及算术绑定，不代替完整 Aura/ICD、动作快照输入来源、角色数据或 gcsim 精度验证。
...
- 本轮全量 Vitest 已实际运行并通过 `82` 个测试文件、`1153/1153` 项测试；该数量只记录本次核心/Schema/Golden 验证，不预写 build 或 Playwright 结果。
...
- `aura-v4`–`aura-v9` 已实现上述 Burning Marker、Fuel、周期伤害、Tick 火附着、归属/快照刷新和核心输出。`legacy-event-heap-v1`、v1 和 v2 继续按冻结历史语义将实际范围伤害排入后续全局 `reactionDamage`。精确 1.44 与当前精确 1.45 都可显式选择 `target-phase-v3`，在 owner callback 内完成零延迟跨目标交付，并按注册顺序标记接收目标为 `before-reactable-tick` 或 `after-reactable-tick`。`target-local-hitlag-v1` 造成的陈旧唤醒会先重投影到正确全局帧；该切片仍没有实现通用敌方任务所有权、完整 gcsim target phase 或官服全部边界，正延迟子反应也仍由全局 heap 处理。
