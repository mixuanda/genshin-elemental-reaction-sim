...
只验证 1.41 反应核心、感电单目标兼容/附近湿目标传播、逐元素敌方抗性、Quicken→Bloom 核心任务、`aura-v8` 感电 cleanup、冻结的 1.37 v1 目标相位、1.38 v2 callback→`Reactable.Tick` 边界、递归碎冰交付、增幅覆盖闸门、玩家自伤、目标本地时钟、可靠 Schema 与兼容 Golden 时可运行：
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.40.0 配置迁移；当前 Schema 为 `1.41.0`、引擎为 `1.41.0-ec-secondary-wet-propagation`。1.35→1.40 的历史迁移契约继续冻结；1.40→1.41 只更新运行身份并注入 `electroChargedPropagationModel: { mode: "single-target-v1" }`，原样保留 Aura、目标任务、目标时钟、玩家模型与伤害交付模式，不会自动启用 `nearby-wet-radius-v1`。历史输入不得夹带未来字段，迁移也绝不自动启用递归碎冰、`aura-v8` cleanup 或附近湿目标传播。逐元素抗性仍从 1.35 起才是合法 wire contract，旧版本夹带该字段会 fail-closed。
...
- `targetTaskModel` 与目标时钟、Aura 版本分别建模，并提供三种模式：`legacy-event-heap-v1` 是历史迁移和当前兼容预设的默认值；`target-phase-v1` 是冻结的 1.37 第一批 Burning callback→Aura 衰减契约；`target-phase-v2` 是 1.38 的独立显式 opt-in。v1/v2 都要求运行时使用 `legal-frame-v1`；历史 v1/v2 继续使用 `aura-v7`，精确 1.40/1.41 的 `aura-v8` 只允许 60 FPS 的 v2。v2 只保证同一目标边界内 QueueEnemyTask / 目标 callback 先于该目标 `Reactable.Tick`，不声称建立全目标 barrier，也不会把随后 core work 搬入目标相位。Hitlag 造成的陈旧唤醒会先按目标本地时钟重投影。冻结的 v1 使用 `targetTaskPhaseLog`；v2 使用相互排斥的 `targetPhaseLog`。
- `reactionDeliveryModel` 独立控制反应伤害的交付顺序。`deferred-event-heap-v1` 是现有兼容预设和历史迁移的默认模式；`shatter-recursive-zero-delay-v1` 从精确 1.39 身份开始可显式选择，1.39→1.41 迁移会原样保留该选择，并继续要求 `legal-frame-v1` 与 60 FPS。递归模式在同一帧、目标和来源上下文中先交付碎冰子伤害，再交付其直接或嵌套反应父伤害；`parentDamageEventId` 因而可以合法前向引用。严格结果 Schema 要求连续 ID、无环父链，以及每个递归碎冰子段与唯一 `reactionDamageLog` 的 reciprocal 引用。该模式只覆盖零延迟碎冰，不会把其他反应或目标任务自动改为同步递归。
...
- 输出侧已为 `SimulationRunManifest`、Burning/Quicken/Bloom 审计、`reactionTaskLog`、冻结 v1 的 `targetTaskPhaseLog`、v2 的 `targetPhaseLog`、`reactionDeliveryModel` 的伤害父链/碎冰日志引用、1.40 `electroChargedCleanup` 的 reciprocal 引用、1.41 `electroChargedPropagationAudit` 的候选判定与逐目标伤害反链、ReactionA/B 伤害组、草原核生命周期/接触/时间线、玩家命中/自伤/HP 时间线/汇总、目标时钟/Hitlag 及其跨日志引用、`TargetStateTimeline` 等关键投影提供严格 Zod Schema。附近传播审计会记录显式半径、源位置、目标位置/距离/阈值、Hydro Gauge、选中结果、拒绝原因以及对应 `hitResolutionLogId` / `damageEventId`；UI 不得依据坐标或 Aura 自行补算候选。整个 `SimulationResult` 仍只有 TypeScript 顶层契约，尚未具备覆盖全部结果字段的单一运行时 Zod Schema。
...
- 1.32 增加显式 `playerDamageModel`，1.33 增加显式 `targetClockModel`，1.34 增加显式 opt-in `aura-v6`，1.35 增加可选的完整敌方八项抗性表，1.36 增加显式 opt-in `aura-v7` 与核心 `reactionTaskLog`，1.37 再增加显式 `targetTaskModel` 与 `targetTaskPhaseLog`，1.38 增加 `target-phase-v2` 与独立 `targetPhaseLog`，1.39 增加显式 `reactionDeliveryModel`，1.40 增加显式 opt-in `aura-v8` cleanup，1.41 增加显式 `electroChargedPropagationModel`。1.35→1.40 继续按冻结历史契约迁移；1.40→1.41 注入 `single-target-v1`，不自动启用附近传播。所有历史 Fixture 均不得回写；1.41 只能新增独立 Fixture。所有 Fixture 都是回归证据，不是官方数值认证或完整 gcsim parity 报告；其中测试抗性、草原核寿命、固定 gcsim 路径、附近传播半径和默认杜林预设的示例魔法数都仍是 `provisional` 或 `community-provisional`。

## 当前精度边界

本版本不是 gcsim 精度实现。`legacy-v0.1` 继续保持 Golden 兼容；`aura-v1`–`aura-v8`、冻结的 `target-phase-v1`、受限的 `target-phase-v2`、1.39 的碎冰递归交付切片、1.40 的 EC next-target-Tick cleanup、1.41 默认关闭的附近湿目标传播和粒子引擎只覆盖当前可审计机制闭环。v2 只声明目标 callback→该目标 `Reactable.Tick`，递归交付也只声明零延迟碎冰父子链；1.40 cleanup 只处理 Quicken→Bloom 耗尽同代水 Aura 的明确边界；1.41 附近传播只提供显式半径下的逐目标伤害与候选审计。它们都不能外推为完整目标相位、通用任务所有权、全部反应同步语义或完整感电传导。固定 gcsim 交叉参考路径保持 `fixed-gcsim-provisional`；1.41 附近传播单独标记为 `community-provisional`，不能称为官方/官服真值或完整 gcsim 精度。尚未实现或未完整实现：
...
- `aura-v8` 已冻结 Quicken→Bloom 耗尽同代水 Aura 后的 EC cleanup，包括 F1、Hitlag5→F6、同代恢复、代次替换、自然到期碰撞、模拟末端 pending 和 F10 首次伤害保留。1.41 的 `nearby-wet-radius-v1` 只增加源 Tick 对显式半径内湿目标的逐目标伤害分支：不附着 Aura、不递归、不接管或刷新副目标流、不重置 cadence、不产生副目标 Wane。它没有核验官服选择半径、目标上限、传播层数或完整多代次所有权，也未补齐跨过 F70 后续 callback 的长 Hitlag；其他多 Aura 来源 overlap 与感电/冻结/燃烧等特殊状态的全部排列仍未完成。
