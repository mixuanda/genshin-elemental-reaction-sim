...
- `aura-v5` 在不改写 v1–v4 回放语义的前提下加入基础反应矩阵和冰来袭有序链：`超导 → 融化 → 冻结`。水雷共存时可得到 `超导 → 冻结`；水雷火共存且冰量足够时可得到 `超导 → 反向融化 → 冻结`。这是固定参考的兼容顺序；任意 Aura 来源重叠和所有多反应排列仍未完成。更早的 `aura-v2/v3` 也没有完整的同击多反应排序：当火来袭先触发 Overload、剩余来袭 Gauge 仍足以对 Frozen 触发 Melt 时，核心会以 `legacy-multi-reaction-order` 进入 `TARGET_MECHANICS_TRUNCATION`，而不是静默跳过仍可达的 Melt；这是 fail-closed 边界，不表示 v2/v3 已实现 Overload→Melt 链。
...
- 本轮全量 Vitest 已实际运行并通过 `82` 个测试文件、`1122/1122` 项测试；该数量只记录本次核心/Schema/Golden 验证，不预写 build 或 Playwright 结果。
