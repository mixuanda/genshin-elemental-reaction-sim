...
- Zod 严格校验、字段路径错误，以及 v0.1 / 1.0.0 / 1.1.0 / 1.2.0 / 1.3.0 / 1.4.0 / 1.5.0 / 1.6.0 / 1.7.0 / 1.8.0 / 1.9.0 / 1.10.0 / 1.11.0 / 1.12.0 配置迁移；当前 Schema 为 `1.13.0`、引擎为 `1.13.0-target-phase-timeline`。
...
- 命中产球已支持逐击触发、显式单目标 `landed / miss`、三层目标效果策略、按帧阶段窗口和精确内部冷却边界；但 Miss 与阶段仍是场景输入，不是由命中范围、目标位置、Boss AI 或碰撞自动求得。多目标各自回调和逐目标产球仍未建模。杜林黑/白爆发、命座和专属装备效果尚未完成；黑/白状态目前只完成互斥与合法性审计，尚未驱动全部被动效果。
- Hitlag、多目标、AoE、索敌、移动、真实 Boss 阶段状态机、目标死亡、护盾和特殊易伤窗口。
