...
- Zod 严格校验、字段路径错误，以及 v0.1 / 1.0.0 / 1.1.0 / 1.2.0 / 1.3.0 / 1.4.0 / 1.5.0 / 1.6.0 配置迁移；当前 Schema 为 `1.7.0`、引擎为 `1.7.0-fixed-energy-icd`。
...
- `AbilityBlueprint` 1.4 把技能倍率引用、命中帧、分后续动作取消帧、附着、ICD、固定回能及其内部冷却、产球、行动状态、前置条件、未实现机制和逐项证据放入严格 Schema；1.0 / 1.1 / 1.2 / 1.3 Blueprint 可迁移，通用编译器默认拒绝 `partial`。
- 内置“杜林黑 E · 部分机制审计向量”：精质转变 6 秒窗口会被核心授予/强制前置/消耗，随后进入 30 秒黑度之否；已按 gcsim 固定提交映射当前命令模型可表达的普攻/战技/爆发/切人取消帧，天赋 10 级三段倍率 `1.30032 / 0.9576 / 1.16352`、命中帧 `48 / 53 / 58`、DurinSkill 自定义 ICD、带 6 秒共享内部冷却的 33 固定回能和 4 火粒子均由核心输出，并在网页逐段、行动状态表、伤害曲线、Aura 曲线和能量曲线中展示。
...
- 杜林的首次成功命中才产球、0.3 秒产球 ICD、白分支、黑/白爆发、命座和专属装备效果尚未完成；精质转变与黑状态目前只影响行动合法性，尚未驱动全部被动效果。
