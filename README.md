...
一个以“逐段可审计、配置可迁移、结果可复现”为目标的原神队伍 DPS 模拟器。当前完成了 Vanilla v0.1 基线冻结、TypeScript 模拟核心、合法帧时间线、火/冰/水 Aura 与自动增幅反应最小闭环、含命中触发与内部冷却审计的粒子/能量闭环、版本化数据目录与 UID 映射基础，以及首个带来源闸门的杜林黑 E 部分机制审计向量。
...
- Zod 严格校验、字段路径错误，以及 v0.1 / 1.0.0 / 1.1.0 / 1.2.0 / 1.3.0 / 1.4.0 / 1.5.0 / 1.6.0 / 1.7.0 配置迁移；当前 Schema 为 `1.8.0`、引擎为 `1.8.0-hit-particle-triggers`。
...
- `AbilityBlueprint` 1.5 把技能倍率引用、命中帧、分后续动作取消帧、附着、ICD、固定回能及其内部冷却、命中产球及其内部冷却、行动状态、前置条件、未实现机制和逐项证据放入严格 Schema；1.0 / 1.1 / 1.2 / 1.3 / 1.4 Blueprint 可迁移，通用编译器默认拒绝 `partial`。
- 内置“杜林黑 E · 部分机制审计向量”：精质转变 6 秒窗口会被核心授予/强制前置/消耗，随后进入 30 秒黑度之否；已按 gcsim 固定提交映射当前命令模型可表达的普攻/战技/爆发/切人取消帧，天赋 10 级三段倍率 `1.30032 / 0.9576 / 1.16352`、命中帧 `48 / 53 / 58`、DurinSkill 自定义 ICD、带 6 秒共享内部冷却的 33 固定回能，以及由首段已处理命中触发并受 18 帧共享粒子 ICD 约束的 4 火粒子，均由核心输出，并在网页逐段、行动状态表、伤害曲线、Aura 曲线和能量曲线中展示。
...
- 命中产球已支持逐击触发和精确内部冷却边界，但当前单目标模型把进入伤害日志视为命中可受击敌人；Miss、无敌目标、多目标各自回调和逐目标产球仍未建模。杜林白分支、黑/白爆发、命座和专属装备效果尚未完成；精质转变与黑状态目前只影响行动合法性，尚未驱动全部被动效果。
