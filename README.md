...
- Zod 严格校验、字段路径错误，以及 v0.1–1.18.0 配置迁移；当前 Schema 为 `1.19.0`、引擎为 `1.19.0-capsule-geometry`。
...
- 单个命中也可用 `geometry` 对全部具备位置的注册目标自动求交：`circle` 比较中心距离，`rectangle` 比较中心到旋转矩形的最近距离，`capsule` 比较中心到有限线段的最近距离与“扫掠半径 + 目标碰撞半径”。核心先解析命中帧位置，再记录形状、位置、端点、尺寸、旋转、距离、阈值和范围外原因；`targeting` 与 `geometry` 互斥。
...
- 粒子目前按一次定义聚合为同一到达事件；其飞行不读取命中几何，也没有角色位置、拾取碰撞或逐个粒子的独立飞行轨迹。
- 尚未给全角色填入经过实测核验的命中帧、逐后续动作取消帧、动画结束帧和冷却数据；杜林只映射了指定黑/白 E 分支。冲刺/跳跃目前要求配置显式给出占用帧，不模拟耐力、位移、无敌帧、碰撞、落地或真实动作帧。
- 命中产球已支持逐击触发、显式 fanout、圆形/旋转矩形/胶囊几何得到的逐目标 `landed / miss`、三层目标策略、按帧阶段和“至少一个目标允许回调时只产一次球”的聚合；目标初始坐标、线性移动分段和攻击形状参数仍由场景声明。
- 当前几何只支持二维圆、可旋转矩形和有限线段胶囊，目标碰撞体仍只有圆形，移动只支持预先声明的线性分段；尚无扇形、高度、角色移动、追踪弹道、AI 驱动敌人移动、索敌、障碍物、Hitlag、真实 Boss 阶段状态机、目标死亡、护盾和特殊易伤窗口。
...
合法时间线对每个有能量消耗的命令按顺序执行确定性前缀探测，再用已确认的失败集合重编译最终时间线；因此失败行动不会推进游标、预占冷却或改变状态，后续命令会按新帧重新执行。它仍不是 gcsim 式单遍动态行动执行器：命中确认分支、目标死亡、AI/命令驱动移动、运行时空间分支和玩家条件语句尚未进入命令语言。Aura/ICD 已按注册目标隔离，圆形/旋转矩形/胶囊 AoE 与声明式目标线性移动已能共同求交，但仍没有 Hitlag 和完整角色动作/ICD 数据库。
