...
- Zod 严格校验、字段路径错误，以及 v0.1 / 1.0.0 / 1.1.0 / 1.2.0 / 1.3.0 / 1.4.0 / 1.5.0 / 1.6.0 / 1.7.0 / 1.8.0 / 1.9.0 / 1.10.0 配置迁移；当前 Schema 为 `1.11.0`、引擎为 `1.11.0-target-hit-resolution`。
...
- 每个逐击先进入结构化 `hitResolutionLog`；默认单目标判定为 `landed`，场景可在命中定义的 `targeting` 中显式指定带原因的 `miss`。Miss 会显示为 0，但不会进入伤害聚合、Aura / 反应或命中确认产球，也不会错误启动粒子 ICD。
...
- 命中产球已支持逐击触发、显式单目标 `landed / miss` 和精确内部冷却边界；但 Miss 仍是场景脚本结果，不是由命中范围、目标位置或碰撞自动求得。无敌目标的附着/回调策略、多目标各自回调和逐目标产球仍未建模。杜林黑/白爆发、命座和专属装备效果尚未完成；黑/白状态目前只完成互斥与合法性审计，尚未驱动全部被动效果。
