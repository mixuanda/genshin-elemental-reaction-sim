...
一个以“逐段可审计、配置可迁移、结果可复现”为目标的原神队伍 DPS 模拟器。当前完成了 Vanilla v0.1 基线冻结、TypeScript 模拟核心、合法帧时间线、火/冰/水/雷普通 Aura、自动增幅反应、超载/超导独立伤害、超导物抗状态以及水雷共存/感电周期流最小闭环、含命中触发与内部冷却审计的粒子/能量闭环、版本化数据目录与 UID 映射基础，以及带来源闸门的杜林黑/白 E 部分机制审计向量。
...
- Zod 严格校验、字段路径错误，以及 v0.1–1.23.0 配置迁移；当前 Schema 为 `1.24.0`、引擎为 `1.24.0-electro-charged-reaction`。
...
- 草 Aura、火雷水共存、冻结/冻元素、燃烧和其他复合附着状态。
- 冻结、扩散、结晶、绽放、燃烧、激化等其余转化/加算反应及其伤害事件。当前超载、超导和感电已进入独立伤害管线。
- 超载/超导尚未模拟击退、韧性、爆炸高度/三维碰撞、物件/召唤物/玩家自身受击或真实敌方移动；半径 3 求交依赖场景显式提供的二维目标坐标与圆形碰撞半径。超导状态不受 Hitlag 延长，因为当前核心尚无 Hitlag。等级基准和反应常数来自固定 gcsim 提交的交叉校验，不代表整个 Aura/反应系统已达到 gcsim 精度。
- 感电当前严格跟随固定 gcsim 提交的单目标 Tick 语义；每个敌人独立维护共存 Aura 和周期流，不会凭距离自动向附近潮湿目标连锁。单次来袭元素同时触发多个反应、按来源 Aura overlap 数组、冻结共存和真实游戏是否存在额外目标传导仍未实现或未验证。
