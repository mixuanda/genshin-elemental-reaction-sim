...
function requireDeferredAuraCommit(
  result: SimulationResult,
  event: SimulationResult["damageEvents"][number],
) {
  const resolution = result.hitResolutionLog[event.targetResolutionId];
  if (resolution?.reactionDamageLogId === null || resolution === undefined) {
    throw new Error("Swirl propagation requires a reaction-damage row.");
  }
  const attack = result.basicReactionSchedulerLog.find(
    (row) =>
      row.kind === "swirl-attack-resolution" &&
      row.reactionDamageLogId === resolution.reactionDamageLogId &&
      row.hitResolutionLogId === resolution.id &&
      row.targetId === event.targetId,
  );
  if (attack?.disposition !== "deferred") {
    throw new Error("Swirl propagation requires a deferred scheduler attack.");
  }
  const commit = result.basicReactionSchedulerLog.find(
    (row) =>
      row.kind === "deferred-aura-attachment" && row.id === attack.pairedLogId,
  );
  if (commit === undefined) {
    throw new Error("Swirl propagation requires its paired Aura commit.");
  }
  return commit;
}
...
describe("AuraEngine Swirl durability and scheduling", () => {
  it("converts fixed gcsim durability exactly for a typical 1U Anemo hit", () => {
    ...
    expect(audit).toMatchObject({
      triggered: true,
      reaction: "swirlPyro",
      auraBefore: [{ element: "pyro", gaugeUnits: 0.8, expiresAtFrame: 426 }],
      auraApplied: [{ element: "anemo", gaugeUnits: 1 }],
      auraConsumed: [{ element: "pyro", gaugeUnits: 0.5 }],
      auraAfter: [{ element: "pyro", gaugeUnits: 0.3, expiresAtFrame: 160 }],
      swirlReactions: [
        {
          reaction: "swirlPyro",
          swirledElement: "pyro",
...
function makeSwirlSimulationConfig(
  secondaryAura: "pyro" | "hydro" | "cryo" | "electro" | null = "hydro",
      ...
      targets: [
        {
          ...
          name: "扩散源目标",
          position: { x: 0, y: 0 },
          initialAura: [{ element: swirledAura, gaugeUnits: 1 }],
        },
        {
          id: "enemy-1",
          name: "传播目标",
          ...
          ...(secondaryAura === null
            ? {}
            : {
                initialAura: [{ element: secondaryAura, gaugeUnits: 1 }],
              }),
        },
        {
          id: "enemy-2",
...
function expectSwirlMutationRejected(
  ...
  mutate(publicWire);
  expect(simulationResultV142Schema.safeParse(publicWire).success).toBe(false);

  const trustedResult = structuredClone(result);
  mutate(trustedResult);
  expect(() => assertTrustedSimulationResultV142(trustedResult)).toThrow(
...
describe("Swirl simulation integration", () => {
  it("rejects Swirl source-delivery and ReactionA projection drift at both result boundaries", () => {
    ...
    expectSwirlMutationRejected(result, (mutation) => {
      ...
      if (
        propagation?.applicationGaugeUnits === null ||
        propagation === undefined
      ) {
        throw new Error("Swirl vector must expose a propagation application.");
      }
    ...
    expectSwirlMutationRejected(result, (mutation) => {
      mutation.damageEvents[0]!.reactionAudit.swirlReactions[0]!.selfDamageFrame += 1;
    });
    expectSwirlMutationRejected(result, (mutation) => {
      mutation.damageEvents[0]!.reactionAudit.swirlReactions[0]!.scheduled = false;
    });
    expectSwirlMutationRejected(result, (mutation) => {
      ...
      if (child === undefined) {
        throw new Error("Swirl vector must expose a reaction-damage child.");
      }
    ...
    expectSwirlMutationRejected(result, (mutation) => {
      ...
      if (
        child?.reactionAudit.swirlDamageGroup === null ||
        child === undefined
      ) {
        throw new Error("Swirl vector must expose a child damage-group audit.");
      }
      child.reactionAudit.swirlDamageGroup.reaction = "swirlHydro";
    });
    expectSwirlMutationRejected(result, (mutation) => {
      const child = mutation.damageEvents.find(
        (event) =>
    ...
    expectSwirlMutationRejected(result, (mutation) => {
      ...
      if (decision === undefined) {
        throw new Error("Swirl vector must expose a ReactionA decision.");
      }
  ...
  it("links a propagated Overload as a child of the Swirl propagation hit", () => {
    const result = simulate(makeSwirlSimulationConfig("electro"), {
      critMode: "noCrit",
    });
    const propagation = result.damageEvents.find(
      (event) =>
  ...
  it("schedules every aura-v6 transformative reaction caused by an Electro propagation", () => {
    ...
    targets[0]!.initialAura = [{ element: "electro", gaugeUnits: 1 }];
    ...
    expect(
      result.reactionDamageLog
        .filter((entry) => entry.triggerDamageEventId === propagation.id)
  ...
  it("retains Hydro propagation as an auditable zero-damage application event", () => {
    const result = simulate(makeSwirlSimulationConfig(null, "hydro"), {
      critMode: "noCrit",
    });
    const propagation = result.damageEvents.find(
      (event) =>
    ...
    const commit = requireDeferredAuraCommit(result, propagation);
    expect(commit.auraApplied).toEqual([{ element: "hydro", gaugeUnits: 2.2 }]);
    expect(commit.auraAfter[0]?.gaugeUnits).toBeCloseTo(1.76, 10);
  });
  ...
  it.each(
    NESTED_SWIRL_AMPLIFYING_CASES.flatMap((vector) =>
      (["active-f3-f8", "expired-f0-f3"] as const).map((buffWindow) => ({
        ...vector,
        buffWindow,
      })),
    ),
    ...
    (vector) => {
      const config = makeNestedSwirlAmplifyingConfig(vector, vector.buffWindow);
      ...
      const buffActiveAtTrigger = vector.buffWindow === "expired-f0-f3";
      const elementalMastery = buffActiveAtTrigger ? 500 : 100;
      const transformativeReactionBonus =
        (buffActiveAtTrigger ? 0.5 : 0.2) + 0.05;
      const amplifyingReactionBonus = (buffActiveAtTrigger ? 0.2 : 0.5) + 0.05;
  ...
  it("applies ReactionA damage only to the first two target-local hits in 30 frames", () => {
      ...
      enemy: {
        ...
        targets: [
          ...sourceTargets.map((id, index) => ({
            ...
            initialAura: [{ element: "pyro" as const, gaugeUnits: 3 }],
          })),
          {
            id: "shared-target",
            name: "共同传播目标",
    ...
    const sharedPropagation = result.damageEvents.filter(
      (event) =>
        event.reaction === "swirlPyro" && event.targetId === "shared-target",
    );
    ...
    if (blockedEvent === undefined) {
      throw new Error("Expected the third shared-target Swirl event.");
    }
    expect(blockedEvent.reactionAudit.auraApplied).toEqual([]);
    const blockedCommit = requireDeferredAuraCommit(result, blockedEvent);
    expect(blockedCommit.auraApplied).toEqual([
      { element: "pyro", gaugeUnits: 2.2 },
    ]);
    expect(
      blockedCommit.auraAfter.some(
        (aura) => aura.element === "pyro" && aura.gaugeUnits > 0,
      ),
    ).toBe(true);
    expect(
    ...
    const blockedLog = result.reactionDamageLog.find(
      (entry) =>
        entry.scheduleKind === "swirl-propagation" && entry.damageFrame === 17,
    );
    ...
    expect(sharedDecision).toEqual({
      ...
      scopeKey: '["shared-target","anemo","ICDTagSwirlPyro"]',
      frame: 17,
      damageGroupTaskSequence: 21,
      windowGeneration: 0,
      windowStartFrame: 5,
      resetAtFrame: 34,
      resetTaskLogId: 3,
      resetTaskSequence: 11,
      hitIndex: 2,
      sequenceIndex: 2,
    ...
    expect(
      blockedLog?.damageGroupDecisions.map((decision) => decision.targetId),
    ).toEqual(blockedLog?.hitTargetIds);
    expect(blockedLog?.damageEventIds).toContain(blockedEvent?.id);
    expect(
      result.damageEvents.find((event) => event.id === blockedEvent?.id),
    ).toMatchObject({
      finalDamage: 0,
      damageComposition: {
        direct: 0,
