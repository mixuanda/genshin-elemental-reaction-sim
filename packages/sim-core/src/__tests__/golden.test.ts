...
const BURNING_V142_REPRODUCIBILITY_KEY = "gdl-v2-fnv1a32-c728aa15";
const BURNING_V142_CONFIG_HASH = "fnv1a32:ac41771e";
const BURNING_V146_CONFIG_HASH = "fnv1a32:ad3d3708";
const BURNING_V146_REPRODUCIBILITY_KEY = "gdl-v2-fnv1a32-c3075366";
...
const UPDATE_LEGACY_DEFAULT_V145_GOLDEN = "UPDATE_LEGACY_DEFAULT_V145_GOLDEN";
...
const LEGACY_DEFAULT_V144_REPRODUCIBILITY_KEY = "gdl-v2-fnv1a32-03487d7e";
const LEGACY_DEFAULT_V145_CONFIG_HASH = "fnv1a32:e53f9200";
const LEGACY_DEFAULT_V145_REPRODUCIBILITY_KEY = "gdl-v2-fnv1a32-b696a75d";
    ...
    return Object.fromEntries(
      Object.keys(record)
        ...
        .filter((key) => {
          ...
          return !(
            NULL_COMPATIBILITY_REFERENCE_FIELDS.has(key) && field === null
          );
        })
...
function projectApplicationsToFrozenV146Wire(
  ...
  if (Array.isArray(value)) {
    ...
    return value.map((entry, index) =>
      projectApplicationsToFrozenV146Wire(entry, referenceEntries[index]),
    );
  }
  if (value === null || typeof value !== "object") {
    return value;
  ...
  if (
    ...
    if (selector.mode === "no-icd-v1") {
      const reference =
        frozenReference !== null && typeof frozenReference === "object"
  ...
  return Object.fromEntries(
    Object.entries(record).map(([key, entry]) => [
      key,
      projectApplicationsToFrozenV146Wire(entry, frozenRecord?.[key]),
    ]),
  );
}
...
function projectCurrentManifestToFrozenV146(
  ...
  } satisfies Omit<SimulationRunManifestV146, "reproducibilityKey">;
...
function projectCurrentDamageEventsToFrozenV146(
  ...
  const configuredHits = [
    ...
    ...(frozenConfig.timeline?.abilities.flatMap((ability) => ability.hits) ??
      []),
  ];
  ...
  for (const row of result.elementalApplicationIcdLog) {
    ...
    const matches = configuredHits.filter(
      (hit) => hit?.id === row.hitId && hit.application !== undefined,
    );
    ...
    const selectorMatchesFrozenWire =
      (selector.mode === "no-icd-v1" && application.icdGroup === "no-icd") ||
      (selector.mode === "legacy-boolean-profile-v1" &&
    ...
    if (
      ...
      event.reactionAudit.applicationGaugeUnits !== row.effectiveGaugeUnits ||
      event.reactionAudit.icdAllowed !== row.decision.allowed
    ) {
      throw new Error(
...
function atomicCreateFixtureBytes(outputUrl: URL, bytes: string): void {
...
function projectLegacyDefaultV145ToFrozenV144(
  ...
  return legacyDefault120sGoldenFixtureV144Schema.parse({
    ...
    reproducibilityKey: LEGACY_DEFAULT_V144_REPRODUCIBILITY_KEY,
  });
}
...
const legacyDefault120sGoldenFixtureV145Schema = z
  .object({
    ...
    provenance: z
      ...
      .strict(),
    schemaVersion: z.literal(REACTION_FORMULA_ROOT_SCHEMA_VERSION),
    engineVersion: z.literal(REACTION_FORMULA_ROOT_ENGINE_VERSION),
    configHash: z.literal(LEGACY_DEFAULT_V145_CONFIG_HASH),
    reproducibilityKey: z.literal(LEGACY_DEFAULT_V145_REPRODUCIBILITY_KEY),
    options: z.any(),
    ...
    byCharacter: z.any(),
    bySkill: z.any(),
    legacyDamageEventsSha256: z.string().regex(/^[0-9a-f]{64}$/),
  ...
  .superRefine((fixture, context) => {
    ...
    if (
      ...
      fixture.runManifest.version !== REACTION_FORMULA_RUN_MANIFEST_VERSION ||
...
function loadFrozenLegacyDefaultV141Fixture(): LegacyDefaultV141Fixture {
  if (process.env.UPDATE_LEGACY_DEFAULT_V141_GOLDEN === "1") {
    throw new Error(
      "legacy-default-120s-1.41.golden.json is frozen; create only the versioned 1.42 fixture.",
    );
  }
  const sourceBytes = readFileSync(LEGACY_DEFAULT_V141_FIXTURE_URL);
  const sourceSha256 = createHash("sha256").update(sourceBytes).digest("hex");
  if (
    sourceSha256 !==
    FROZEN_V141_FIXTURE_SHA256["legacy-default-120s-1.41.golden.json"]
  ) {
    throw new Error(
      `Frozen 1.41 default fixture changed: received ${sourceSha256}.`,
    );
  }
  return JSON.parse(sourceBytes.toString("utf8")) as LegacyDefaultV141Fixture;
}

function loadFrozenLegacyDefaultV140Fixture(): typeof goldenV139 {
  if (process.env.UPDATE_LEGACY_DEFAULT_V139_GOLDEN === "1") {
    throw new Error(
      "legacy-default-120s-1.39.golden.json is frozen; create only a new versioned fixture.",
    );
  }
  if (process.env.UPDATE_LEGACY_DEFAULT_V140_GOLDEN === "1") {
    throw new Error(
      "legacy-default-120s-1.40.golden.json is frozen; create only the versioned 1.41 fixture.",
    );
  }
  const sourceBytes = readFileSync(LEGACY_DEFAULT_V140_FIXTURE_URL);
  const sourceSha256 = createHash("sha256").update(sourceBytes).digest("hex");
  if (
    sourceSha256 !==
    FROZEN_V140_FIXTURE_SHA256["legacy-default-120s-1.40.golden.json"]
  ) {
    throw new Error(
...
function loadOrCreateLegacyDefaultV142Fixture(): LegacyDefaultV142Fixture {
  if (process.env.UPDATE_LEGACY_DEFAULT_V142_GOLDEN === "1") {
    throw new Error(
      "legacy-default-120s-1.42.golden.json is frozen; create only the versioned 1.44 fixture.",
    );
  }
  const sourceBytes = readFileSync(LEGACY_DEFAULT_V142_FIXTURE_URL);
  const sourceSha256 = createHash("sha256").update(sourceBytes).digest("hex");
  if (
    sourceSha256 !==
    FROZEN_V142_FIXTURE_SHA256["legacy-default-120s-1.42.golden.json"]
  ) {
    throw new Error(
      `Frozen 1.42 default fixture changed: received ${sourceSha256}.`,
    );
  }
  return JSON.parse(sourceBytes.toString("utf8")) as LegacyDefaultV142Fixture;
}
...
function loadOrCreateLegacyDefaultV144Fixture(
  ...
  const outputUrl = options.outputUrl ?? LEGACY_DEFAULT_V144_FIXTURE_URL;
  if (updateRequested) {
    const sourceBytes = readFileSync(LEGACY_DEFAULT_V142_FIXTURE_URL);
    const sourceSha256 = createHash("sha256").update(sourceBytes).digest("hex");
    if (
    ...
    if (
      JSON.stringify(
        projectLegacyDefaultCompatibilitySemantics(generatedFixture),
      ) !==
      JSON.stringify(projectLegacyDefaultCompatibilitySemantics(frozenV142))
    ) {
      throw new Error(
    ...
    const parsedGeneratedFixture =
      legacyDefault120sGoldenFixtureV144Schema.parse(generatedFixture);
    const generatedBytes = serializeJsonFixture(parsedGeneratedFixture);
    const generatedSha256 = byteSha256(generatedBytes);
    if (
      generatedSha256 !==
      FROZEN_V144_FIXTURE_SHA256["legacy-default-120s-1.44.golden.json"]
    ) {
      throw new Error(
...
function loadOrCreateLegacyDefaultV145Fixture(
  ...
  const outputUrl = options.outputUrl ?? LEGACY_DEFAULT_V145_FIXTURE_URL;
  const sourceBytes = readFileSync(LEGACY_DEFAULT_V144_FIXTURE_URL);
  ...
  const validateCompatibility = (fixture: LegacyDefaultV145Fixture): void => {
    const projected = projectLegacyDefaultV145ToFrozenV144(fixture);
    if (JSON.stringify(projected) !== JSON.stringify(frozenV144)) {
      throw new Error(
  ...
  if (updateRequested) {
    const parsedGeneratedFixture =
      legacyDefault120sGoldenFixtureV145Schema.parse(generatedFixture);
  ...
  const parsedFrozenFixture = legacyDefault120sGoldenFixtureV145Schema.parse(
    JSON.parse(frozenBytes.toString("utf8")),
  );
...
function makeLegacyDefaultV145CreationProbeFixture(
  ...
  return legacyDefault120sGoldenFixtureV145Schema.parse({
    ...
    reproducibilityKey: LEGACY_DEFAULT_V145_REPRODUCIBILITY_KEY,
...
function stripV131QuickenLifecycleAudit(value: unknown): unknown {
  if (Array.isArray(value)) {
    return value.map((entry) => stripV131QuickenLifecycleAudit(entry));
  }
  if (value !== null && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value as Record<string, unknown>)
        .filter(([key]) => key !== "quickenStateMutation")
        .map(([key, entry]) => [key, stripV131QuickenLifecycleAudit(entry)]),
    );
  }
  return value;
}
 ...
 */
function stripV132BurningSkipAuditCorrection(value: unknown): unknown {
  if (Array.isArray(value)) {
    return value.map((entry) => stripV132BurningSkipAuditCorrection(entry));
  }
  if (value !== null && typeof value === "object") {
    const record = value as Record<string, unknown>;
    return Object.fromEntries(
...
function v130CompatibilityResult(
  ...
  const frozenDamageEvents =
    frozenConfig === undefined
      ? result.damageEvents
      : projectCurrentDamageEventsToFrozenV146(result, frozenConfig);
  const frozenHitResolutionLog = result.hitResolutionLog.map((entry) => {
    const {
  ...
  const {
    ...
    totalReactionSelfDamageTaken: _totalReactionSelfDamageTaken,
  ...
  return stripV131QuickenLifecycleAudit(
    stripV132BurningSkipAuditCorrection({
      ...
      config: {
        ...Object.fromEntries(
          Object.entries(frozenConfig ?? preDendroCoreResult.config).filter(
            ([key]) =>
...
function expectContiguousTargetStateTimelineIds(
  ...
  expect(result.targetStateTimeline.points.map((point) => point.id)).toEqual(
    Array.from(
...
function expectNoDendroCoreOutput(result: ReturnType<typeof simulate>): void {
...
describe("frozen versioned fixture integrity", () => {
  it("keeps every pre-1.37 foundation fixture byte-for-byte frozen", () => {
    for (const [fileName, expectedSha256] of Object.entries(
      ...
      const bytes = readFileSync(
        new URL(`../../../test-vectors/fixtures/${fileName}`, import.meta.url),
      );
  ...
  it("keeps the 1.44 release fixtures byte-for-byte frozen", () => {
    for (const [fileName, expectedSha256] of Object.entries(
      ...
      if (
        fileName === "legacy-default-120s-1.44.golden.json" &&
        process.env.UPDATE_LEGACY_DEFAULT_V144_GOLDEN === "1" &&
  ...
  it("keeps the 1.45 default release fixture byte-for-byte frozen", () => {
    ...
    expect(byteSha256(readFileSync(LEGACY_DEFAULT_V145_FIXTURE_URL))).toBe(
...
describe("default 1.44 Golden creation gate", () => {
  it("does not create a missing fixture without explicit UPDATE", () => {
    ...
    const generated = makeLegacyDefaultV144CreationProbeFixture(frozenV142);
    ...
    try {
      expect(() =>
        loadOrCreateLegacyDefaultV144Fixture(generated, frozenV142, {
  ...
  it("validates the frozen SHA before an atomic UPDATE create", () => {
    ...
    try {
      ...
      expect(existsSync(fileURLToPath(driftedOutputUrl))).toBe(false);
    } finally {
      rmSync(probeDirectory, {
        recursive: true,
        force: true,
...
describe("default 1.45 Golden creation gate", () => {
  function makeProbe(): {
    ...
    const frozenV142 = loadOrCreateLegacyDefaultV142Fixture();
    const generatedV144 = makeLegacyDefaultV144CreationProbeFixture(frozenV142);
    ...
    return {
      generated: makeLegacyDefaultV145CreationProbeFixture(result, frozenV144),
      frozenV144,
    };
  }
  ...
  it("does not create a missing fixture without explicit UPDATE", () => {
    ...
    try {
      expect(() =>
        loadOrCreateLegacyDefaultV145Fixture(generated, frozenV144, {
          updateRequested: false,
          outputUrl,
        }),
      ).toThrow();
...
describe("1.44 identity migration release gate", () => {
  function makeHistoricalV141Config(
    ...
    const frozenV146 = projectCurrentConfigToFrozenV146(current);
    const {
      reactionFormulaModel: _reactionFormulaModel,
      directDamageGroupModel: _directDamageGroupModel,
      ...historical
  ...
  for (const propagation of [
    ...
    it(`migrates exact 1.41 ${propagation.mode} by adding the pinned formula profile`, () => {
      const historical = makeHistoricalV141Config(propagation);
      ...
      expect(migrateConfig(historical)).toEqual({
        ...
        directDamageGroupModel: makeConfig().directDamageGroupModel,
        elementalApplicationIcdModel: makeConfig().elementalApplicationIcdModel,
        reactionOwnedElementalApplicationModel: {
...
describe("Vanilla v0.1 golden compatibility", () => {
  it("matches the full default 120-second baseline", () => {
    ...
    const goldenV140 = loadFrozenLegacyDefaultV140Fixture();
    const goldenV141 = loadFrozenLegacyDefaultV141Fixture();
    const goldenV142 = loadOrCreateLegacyDefaultV142Fixture();
    ...
    expect(
      projectLegacyDefaultCompatibilitySemantics(parsedGeneratedV144Fixture),
    ).toEqual(projectLegacyDefaultCompatibilitySemantics(goldenV142));
    expect(
      byteSha256(serializeJsonFixture(parsedGeneratedV144Fixture)),
      "generated default 1.44 fixture bytes",
    ).toBe(FROZEN_V144_FIXTURE_SHA256["legacy-default-120s-1.44.golden.json"]);
    const goldenV144 =
      process.env.UPDATE_LEGACY_DEFAULT_V144_GOLDEN === "1"
    ...
    const parsedGeneratedV145Fixture =
      makeLegacyDefaultV145CreationProbeFixture(result, goldenV144);
    const goldenV145 =
      process.env[UPDATE_LEGACY_DEFAULT_V145_GOLDEN] === "1"
    ...
    expect(legacyDefault120sGoldenFixtureV142Schema.parse(goldenV142)).toEqual(
      goldenV142,
    );
    expect(legacyDefault120sGoldenFixtureV144Schema.parse(goldenV144)).toEqual(
      goldenV144,
    );
    expect(legacyDefault120sGoldenFixtureV145Schema.parse(goldenV145)).toEqual(
      goldenV145,
    );
    ...
    expect(goldenV139).toMatchObject({
      ...
      reproducibilityKey: expect.stringMatching(/^gdl-v2-fnv1a32-[0-9a-f]{8}$/),
    ...
    expect(result.config.engineVersion).toBe(CURRENT_ENGINE_VERSION);
    ...
    expect(result.runManifest).toMatchObject({
      ...
      plugins: [],
      reactionFormulaRoot: goldenV145.runManifest.reactionFormulaRoot,
      directDamageGroupRoot: GCSIM_DAMAGE_GROUP_ROOT,
      reproducibilityKey: result.reproducibilityKey,
    });
    expect(
      reactionFormulaRootSchema.parse(result.runManifest.reactionFormulaRoot),
    ...
    expect(result.config.targetClockModel).toEqual({ mode: "disabled" });
    ...
    expect(result.targetClockLog).toEqual(goldenV145.targetClock.clockLog);
    expect(result.targetHitlagLog).toEqual(goldenV145.targetClock.hitlagLog);
    expect(result.config.targetTaskModel).toEqual(goldenV145.targetTask.config);
    expect(result.targetTaskPhaseLog).toEqual(goldenV145.targetTask.phaseLog);
    expect(result.targetPhaseLog).toEqual(goldenV145.targetPhaseLog);
    ...
    expect(result.config.reactionEngine?.mode).not.toBe("aura-v9");
    expect(projectLegacyDefaultCompatibilitySemantics(goldenV145)).toEqual(
      projectLegacyDefaultCompatibilitySemantics(goldenV144),
    );
    expect(projectLegacyDefaultCompatibilitySemantics(goldenV144)).toEqual(
    ...
    expect(projectLegacyDefaultCompatibilitySemantics(goldenV142)).toEqual(
      projectLegacyDefaultCompatibilitySemantics(goldenV141),
    );
    expect(projectLegacyDefaultCompatibilitySemantics(goldenV141)).toEqual(
      projectLegacyDefaultCompatibilitySemantics(goldenV140),
    );
    expect(goldenV141.targetTask).toEqual(goldenV140.targetTask);
    expect(goldenV141.targetPhaseLog).toEqual(goldenV140.targetPhaseLog);
    ...
    expect(goldenV140.targetPhaseLog).toEqual(goldenV139.targetPhaseLog);
    ...
    expect(goldenV139.targetPhaseLog).toEqual(goldenV138.targetPhaseLog);
    ...
    ).toBe(LEGACY_V130_COMPATIBILITY_SHA256);
    ...
    expect(
      result.damageCurve.every(
        (point) =>
          point.targetId === "enemy-0" && point.targetName === "敌人 0",
      ),
    ).toBe(true);
    ...
    expect(result.damageEvents).toHaveLength(goldenV145.hitCount);
    expect(result.reactedHits).toBe(goldenV145.reactedHits);
    expect(result.skippedActions).toHaveLength(goldenV145.skippedActionCount);
    expect(result.byCharacter).toEqual(goldenV145.byCharacter);
    ...
    expect(goldenV145.skippedActionCount).toBe(goldenV144.skippedActionCount);
    expect(goldenV145.byCharacter).toEqual(goldenV144.byCharacter);
    ...
    expect(goldenV144.reactedHits).toBe(goldenV142.reactedHits);
    expect(goldenV144.skippedActionCount).toBe(goldenV142.skippedActionCount);
    expect(goldenV144.byCharacter).toEqual(goldenV142.byCharacter);
    ...
    expect(goldenV142.reactedHits).toBe(goldenV141.reactedHits);
    expect(goldenV142.skippedActionCount).toBe(goldenV141.skippedActionCount);
    expect(goldenV142.byCharacter).toEqual(goldenV141.byCharacter);
    ...
    expect(goldenV141.reactedHits).toBe(goldenV140.reactedHits);
    expect(goldenV141.skippedActionCount).toBe(goldenV140.skippedActionCount);
    expect(goldenV141.byCharacter).toEqual(goldenV140.byCharacter);
    ...
    expect(goldenV140.skippedActionCount).toBe(goldenV139.skippedActionCount);
    expect(goldenV140.byCharacter).toEqual(goldenV139.byCharacter);
    ...
    expect(goldenV139.skippedActionCount).toBe(goldenV138.skippedActionCount);
    expect(goldenV139.byCharacter).toEqual(goldenV138.byCharacter);
    ...
    expect(goldenV138.skippedActionCount).toBe(goldenV137.skippedActionCount);
    expect(goldenV138.byCharacter).toEqual(goldenV137.byCharacter);
    ...
    expect(goldenV137.skippedActionCount).toBe(goldenV136.skippedActionCount);
    expect(goldenV137.byCharacter).toEqual(goldenV136.byCharacter);
    ...
    expect(goldenV136.skippedActionCount).toBe(goldenV135.skippedActionCount);
    expect(goldenV136.byCharacter).toEqual(goldenV135.byCharacter);
    ...
    expect(goldenV135.skippedActionCount).toBe(goldenV134.skippedActionCount);
    expect(goldenV135.byCharacter).toEqual(goldenV134.byCharacter);
    ...
    expect(goldenV134.skippedActionCount).toBe(goldenV133.skippedActionCount);
    expect(goldenV134.byCharacter).toEqual(goldenV133.byCharacter);
    ...
    expect(
      result.damageCurve.every(
        (point) => (point.cumulativeByReaction.burning ?? 0) === 0,
      ),
    ).toBe(true);
    ...
    for (const [characterId, expectedDamage] of Object.entries(
      ...
      expectRelativeClose(result.byCharacter[characterId] ?? 0, expectedDamage);
    }
    ...
    golden.bySkill.forEach((expectedSkill, index) => {
      ...
      expectRelativeClose(actualSkill?.damage ?? 0, expectedSkill.damage);
    });

    expect(
      result.skippedActions.map(({ time, actorId, action, reason, cycle }) => ({
        time,
        actorId,
        action,
        reason,
    ...
    expect(repeated.targetStateTimeline).toEqual(result.targetStateTimeline);
    expectContiguousTargetStateTimelineIds(repeated);
    if (process.env.UPDATE_LEGACY_DEFAULT_V144_GOLDEN === "1") {
      expect(
        loadOrCreateLegacyDefaultV144Fixture(
          parsedGeneratedV144Fixture,
          goldenV142,
    ...
    if (process.env[UPDATE_LEGACY_DEFAULT_V145_GOLDEN] === "1") {
      expect(
        loadOrCreateLegacyDefaultV145Fixture(
          parsedGeneratedV145Fixture,
          goldenV144,
...
describe("Burning aura-v4 provisional golden", () => {
  it("matches the fixed-gcsim-code-cross-check lifecycle and damage vector", () => {
    ...
    const frozenV146Config = projectCurrentConfigToFrozenV146(
      result.config,
      input,
    );
    const frozenV146Manifest = projectCurrentManifestToFrozenV146(
      result,
      frozenV146Config,
    );
    const frozenV146DamageEvents = projectCurrentDamageEventsToFrozenV146(
      result,
      frozenV146Config,
    );
    expect(playerDamageResultReferencesSchema.parse(result)).toEqual(result);
    expect(simulationResultSchema.parse(result)).toEqual(result);
    expect(assertTrustedSimulationResult(result)).toBe(result);
    expect(frozenV146Manifest.configHash).toBe(BURNING_V146_CONFIG_HASH);

    expect(burningGolden.config.schemaVersion).toBe("1.30.0");
    expect(burningGolden.config.engineVersion).toBe("1.30.0-burning-reaction");
    expect(result.schemaVersion).toBe(CURRENT_SCHEMA_VERSION);
    expect(result.engineVersion).toBe(CURRENT_ENGINE_VERSION);
    expect(result.config.schemaVersion).toBe(CURRENT_SCHEMA_VERSION);
    expect(result.config.engineVersion).toBe(CURRENT_ENGINE_VERSION);
    ...
    expect(result.targetClockAudit).toEqual(goldenV139.targetClock.audit);
    expect(result.targetClockLog).toEqual([]);
    expect(result.targetHitlagLog).toEqual([]);
    expect(result.config.targetTaskModel).toEqual(goldenV139.targetTask.config);
    expect(result.targetTaskPhaseLog).toEqual([]);
    expect(result.targetPhaseLog).toEqual([]);
    expect(result.config.reactionDeliveryModel).toEqual(
      goldenV139.reactionDeliveryModel,
    );
    expect(result.dataVersion).toBe(burningGolden.config.dataVersion);
    expect(burningGolden.reproducibilityKey).toBe("gdl-37da25f5");
    ...
    expect(result.runManifest).toMatchObject({
      ...
      plugins: [],
      reactionFormulaRoot: result.runManifest.reactionFormulaRoot,
      directDamageGroupRoot: GCSIM_DAMAGE_GROUP_ROOT,
      reproducibilityKey: result.reproducibilityKey,
    });
    expect(frozenV146Manifest).toMatchObject({
      ...
      reactionFormulaRoot: result.runManifest.reactionFormulaRoot,
      directDamageGroupRoot: GCSIM_DAMAGE_GROUP_ROOT,
      reproducibilityKey: BURNING_V146_REPRODUCIBILITY_KEY,
    });
    expect(sha256(frozenV146DamageEvents)).toBe(BURNING_DAMAGE_EVENTS_SHA256);
    expect(sha256(result.burningStateLog)).toBe(BURNING_STATE_LOG_SHA256);
    ...
    expect(sha256(v130Probe)).toBe(BURNING_V130_COMPATIBILITY_SHA256);
    expectNoDendroCoreOutput(result);
    expectContiguousTargetStateTimelineIds(result);
    expectRelativeClose(result.totalDamage, burningGolden.totalDamage);
    expectRelativeClose(result.dps, burningGolden.dps);
    expect(result.damageEvents).toHaveLength(burningGolden.hitCount);
    ...
    expect(burningDamageEvents.map((event) => event.frame)).toEqual(
      burningGolden.cadence.tickFrames,
    );
    expect(
      burningDamageEvents.every(
        (event) =>
          Math.abs(event.finalDamage - burningGolden.burningDamagePerTick) <
            1e-10 &&
          event.sourceActorId === burningGolden.owners.damageSourceActorId &&
          event.creditOwnerId === burningGolden.owners.creditOwnerId,
      ),
    ).toBe(true);
    ...
    expect(start).toMatchObject({
      operation: "start",
      damageSourceActorId: burningGolden.owners.damageSourceActorId,
      fuelSourceActorId: burningGolden.owners.fuelSourceActorId,
      fuelGaugeUnitsAfter: burningGolden.fuel.startGaugeUnits,
      fuelExpiresAtFrame: burningGolden.fuel.expiresAtFrame,
    });
    expect(skipped).toMatchObject({
      tickIndex: burningGolden.cadence.skippedTickIndex,
      fuelGaugeUnitsAfter: burningGolden.fuel.skippedTickGaugeUnitsAfter,
    });
    ...
    expect(expiry).toMatchObject({
      ...
      fuelGaugeUnitsBefore: burningGolden.fuel.expiryGaugeUnitsBefore,
      fuelGaugeUnitsAfter: burningGolden.fuel.expiryGaugeUnitsAfter,
      reason: "FUEL_EXPIRED",
    });
    expect(result.auraInitialStates).toEqual([
      {
    ...
    expect(
      result.burningStateLog.every(
        (entry) =>
          ...
          entry.fuelSourceActorId === burningGolden.owners.fuelSourceActorId,
      ),
    ).toBe(true);
    ...
    expect(result.bySkill).toHaveLength(burningGolden.bySkill.length);
    ...
    expect(result.targetSummaries).toHaveLength(burningGolden.byTarget.length);
    burningGolden.byTarget.forEach((expectedTarget, index) => {
      ...
      expectRelativeClose(actualTarget?.damage ?? 0, expectedTarget.damage);
      expectRelativeClose(
    ...
    expect(finalCurve).toMatchObject({
      ...
      sourceActorId: burningGolden.owners.damageSourceActorId,
      creditOwnerId: burningGolden.owners.creditOwnerId,
    });
    expectRelativeClose(finalCurve.finalDamage, burningGolden.curve.lastDamage);
    expectRelativeClose(
    ...
    expectRelativeClose(
      finalCurve.cumulativeByComponent.transformativeReaction,
    ...
    const repeated = simulate(makeBurningGoldenConfig(), options);
    expect(repeated.reproducibilityKey).toBe(result.reproducibilityKey);
    expect(repeated.damageEvents).toEqual(result.damageEvents);
    expect(repeated.burningStateLog).toEqual(result.burningStateLog);
    expect(repeated.targetStateTimeline).toEqual(result.targetStateTimeline);
    expectContiguousTargetStateTimelineIds(repeated);
  });
});
