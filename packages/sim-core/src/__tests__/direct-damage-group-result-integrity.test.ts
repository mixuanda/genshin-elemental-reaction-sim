...
describe("direct-damage-group result wire and trusted replay", () => {
  ...
  it("resolves repeated and explicit/automatic hit ID collisions by the stable hitGroupId index", () => {
    ...
    for (const [result, multipliers, expectedHitIds] of [
      ...
      expect(() => assertTrustedSimulationResult(result)).not.toThrow();
      expect(
        result.directDamageGroupLog.map((entry) => entry.configuredMultiplier),
      ).toEqual(multipliers);
      ...
      expectRejectedByPublicAndTrusted(result, (forged) => {
        ...
        forged.hitResolutionLog[event.targetResolutionId]!.hitGroupId =
          event.hitGroupId;
      });
    }
  });
  ...
  it.each([
    ...
    [
      "generic OnEnemyHit gate",
      (result: SimulationResult) => {
        result.directDamageGroupLog[1]!.damageGroupOnEnemyHitAllowed = true;
      },
    ],
  ])(
    "rejects %s tampering through public and trusted boundaries",
    (_label, mutate) => {
  ...
  it("rejects log deletion, duplication, and stable-order tampering", () => {
    ...
    expectRejectedByPublicAndTrusted(result, (forged) => {
      ...
      forged.directDamageGroupLog[0] = forged.directDamageGroupLog[1]!;
  ...
  it("rejects non-finite multiplier and damage-factor values at the trusted boundary", () => {
    const result = resultVector();
    const mutations: Array<(forged: SimulationResult, poison: number) => void> =
      [
        (forged, poison) => {
  ...
  it("binds every plugin multiplier step, including fixed zero slots", () => {
    ...
    expect(result.directDamageGroupLog[1]!.pluginTraceVerification).toBe(
    ...
    for (const mutate of [
      ...
      (forged: SimulationResult) => {
        forged.directDamageGroupLog[1]!.pluginMultiplierTrace[0]!.pluginManifestIndex = 1;
      },
      (forged: SimulationResult) => {
      ...
      (forged: SimulationResult) => {
        forged.directDamageGroupLog[1]!.pluginMultiplierTrace[0]!.inputMultiplier = 99;
      },
      (forged: SimulationResult) => {
        forged.directDamageGroupLog[1]!.pluginMultiplierTrace[0]!.outputMultiplier = 99;
      },
      (forged: SimulationResult) => {
  ...
  it("keeps V145 exact and rejects the future top-level log", () => {
    ...
    expect(simulationResultV145Schema.safeParse(projected).success).toBe(true);
    projected.directDamageGroupLog = [];
    expect(simulationResultV145Schema.safeParse(projected).success).toBe(false);
  });
});
