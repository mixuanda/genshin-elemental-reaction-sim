...
const UPDATE_FLAG =
  "UPDATE_LEGACY_DEFAULT_V142_GOLDEN";
...
const sourcePath = resolve(
  scriptDirectory,
  "../fixtures/legacy-default-120s-1.41.golden.json"
);
...
if (process.env[UPDATE_FLAG] !== "1") {
  throw new Error(
    `Refusing to create the 1.42 default fixture without ${UPDATE_FLAG}=1.`
  );
}
...
if (run.status !== 0) {
  throw new Error(
    `Vitest refused the 1.42 default Golden with exit code ${String(run.status)}.`
  );
}
if (!existsSync(outputPath)) {
  throw new Error(
